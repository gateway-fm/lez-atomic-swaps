...
bind_claim_sweep() {
  ...
  "$agreement_actor_binary" bind-finalized-claim-sweep --private-root "${agreement_root}/material/taker" --own-public-packet "${agreement_root}/exchange/taker.json" --peer-public-packet "${agreement_root}/exchange/maker.json" --agreement-stage-a "$agreement_stage_a" --activation-stage-b "$agreement_stage_b" --journal "${agreement_root}/stage-b/private/taker.sqlite" --run-id "$MONERO_RUN_ID" --finalized-claim "$tag15_finality_result" --observed-final-signature "$observed_final_signature" --extracted-maker-adaptor-scalar "$extracted_maker_scalar" --monero-sweep-evidence "$monero_sweep_evidence" --monero-receipt-evidence "$monero_verification_evidence" --output-binding-evidence "$claim_sweep_binding"
