...
for role in maker taker; do
  if [[ "$role" == maker ]]; then identity="$M4_ONBOARD_MAKER_IDENTITY"; else identity="$M4_ONBOARD_TAKER_IDENTITY"; fi
...
readonly maker_allocation taker_allocation
...
readonly runner_sha stack_sha deployment_sha maker_identity_sha taker_identity_sha vault_claim_sha
...
claim_role() {
  local role="$1" key identity allocation account vault owner_hex vault_hex role_state request evidence partial
  local start tx block owner_output vault_output summary
  if [[ "$role" == maker ]]; then
    key="$M4_ONBOARD_MAKER_PRIVATE_KEY"; identity="$M4_ONBOARD_MAKER_IDENTITY"; allocation="$maker_allocation"
  else
    key="$M4_ONBOARD_TAKER_PRIVATE_KEY"; identity="$M4_ONBOARD_TAKER_IDENTITY"; allocation="$taker_allocation"
  fi
  ...
  "$M4_ONBOARD_VAULT_CLAIM_BIN" --role "$role" --run-id "$M4_ONBOARD_RUN_ID" \
  ...
  chmod 0600 "$partial"
  jq -e --arg role "$role" --arg run "$M4_ONBOARD_RUN_ID" --arg request "$request" \
...
for role in maker taker; do claim_role "$role"; done
...
jq -n --arg run "$M4_ONBOARD_RUN_ID" --arg channel "$channel" --arg program "$pinned_program_id" \
  ...
  --argjson taker "$(<"$M4_ONBOARD_EVIDENCE_ROOT/taker-summary.json")" --argjson raw "$raw" '
  {schema_version:1,kind:"m4_lez_actor_onboarding",result:"passed",flow:"flow_0_fresh_vault_claims",
   ...
   vault_claim_binary_sha256:$binary_sha,actors:{maker:$maker,taker:$taker},raw_evidence:$raw,
   total_submission_count:2,automatic_submission_retry:false,
