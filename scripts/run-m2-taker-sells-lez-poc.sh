...
apply_m6_refund_parent_handoff() {
  ...
  local zcash_block_hash zcash_block_height
  ...
  jq -e '
    ...
    and (
      ((.m6_zcash_refund_mined // false) == false
        ...
        and (.m6_zcash_refund_block_hash // "") == ""
        and (.m6_zcash_refund_block_height // null) == null)
      or
      (.m6_zcash_refund_mined == true
        and (.m6_zcash_refund_txid | strings | test("^[0-9a-f]{64}$"))
        and (.m6_zcash_refund_block_hash | strings | test("^[0-9a-f]{64}$"))
        and (.m6_zcash_refund_block_height | numbers) >= 1
        and .m6_zcash_refund_block_height == (.m6_zcash_refund_block_height | floor))
    )
  ' <<<"$output" >/dev/null || {
  ...
  zcash_block_hash="$(jq -er '(.m6_zcash_refund_block_hash // "") | strings' <<<"$output")"
  zcash_block_height="$(jq -r '.m6_zcash_refund_block_height // empty' <<<"$output")"
  ...
  if (( ${m6_zcash_refund_mined:-0} == 1 )); then
    [[ "$zcash_mined" == true
      ...
      && "$m6_zcash_refund_block_height" == "$zcash_block_height" ]] || {
...
emit_m6_refund_parent_handoff() {
  ...
  jq -c --argjson generation "$m6_refund_generation" \
    ...
    --arg zcash_block_hash "${m6_zcash_refund_block_hash:-}" \
    --arg zcash_block_height "${m6_zcash_refund_block_height:-}" \
    ...
    (.result // {}) + {m6_refund_parent_handoff:true,m6_refund_admitted:true,
      ...
      m6_zcash_refund_block_hash:$zcash_block_hash,
      m6_zcash_refund_block_height:
        (if $zcash_mined == 1 then ($zcash_block_height | tonumber) else null end)}
  ' <<<"$response"
}
