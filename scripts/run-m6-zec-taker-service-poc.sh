...
if [[ "${POC_DIRECTION:-taker_sells_lez}" != taker_sells_lez ]]; then
  printf '%s\n' 'M6 ZEC Taker-service PoC requires POC_DIRECTION=taker_sells_lez' >&2
  exit 2
fi
if [[ -n "${M5_APPLICATION_MODE:-}" && "$M5_APPLICATION_MODE" != 1 ]]; then
  printf '%s\n' 'M6 ZEC Taker-service PoC fixes M5_APPLICATION_MODE=1' >&2
  exit 2
fi
if [[ -n "${M6_TAKER_SERVICE_MODE:-}" && "$M6_TAKER_SERVICE_MODE" != 1 ]]; then
  printf '%s\n' 'run-m6-zec-taker-service-poc.sh fixes M6_TAKER_SERVICE_MODE=1' >&2
  exit 2
fi
if [[ -n "${M6_ZEC_JOURNEY:-}" && "$M6_ZEC_JOURNEY" != claim ]]; then
  printf '%s\n' 'run-m6-zec-taker-service-poc.sh fixes M6_ZEC_JOURNEY=claim' >&2
  exit 2
fi
