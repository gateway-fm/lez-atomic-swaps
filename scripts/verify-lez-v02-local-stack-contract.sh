...
rg -Fqx 'contract_status = "binaries_built_distroless_cli_smoked_stack_not_executed"' "$contract"
rg -Fqx "stack_kind = \"bedrock_settled_non_standalone\"" "$contract"
rg -Fqx 'compose_project_template = "lez-atomic-swaps-lez-v02-{run_id}"' "$contract"
rg -Fqx 'state_root_template = ".e2e/{run_id}/lez-v02"' "$contract"
rg -Fqx "host_port_policy = \"dynamic_zero_only\"" "$contract"
rg -Fqx "container_name_policy = \"forbidden\"" "$contract"
...
rg -Fqx 'binary_sha256_status = "locked_source_build_hashes_bound_warm_rerun_stable_not_container_executed"' "$contract"
...
rg -Fqx 'runtime_smoke_status = "distroless_cli_smoked_not_service_started"' "$contract"
...
rg -Fqx 'channel_id = "0101010101010101010101010101010101010101010101010101010101010101"' "$contract"
...
rg -Fq '"http:GET_/channel/0101010101010101010101010101010101010101010101010101010101010101_returns_200",' "$contract"
rg -Fq '"rpc:getBlock(1)_returns_genesis",' "$contract"
rg -Fq '"rpc:getProgramIds_contains_required_builtins",' "$contract"
rg -Fq '"rpc:getLastFinalizedBlockId_returns_some",' "$contract"
rg -Fq '"rpc:getBlockById(last_finalized)_returns_same_block_id",' "$contract"
rg -Fq '"cross_check:indexer_finalized_block_matches_sequencer_block_at_same_id",' "$contract"
...
printf 'LEZ v0.2 local-stack contract verified: commit=%s tag=%s toolchain=%s project=%s status=stack-not-executed\n' "$actual_commit" "$local_tag_state" "$rust_toolchain" "$project_name"
