...
use lez_node_common::owner_rpc_server::{bind_owner_socket, validate_runtime_directory};
...
async fn owner_socket_refuses_every_preexisting_endpoint_kind() {
    ...
    assert!(
        error
            .to_string()
            .contains("refusing to replace existing owner RPC socket path")
    );
...
async fn runtime_rejects_wrong_mode_and_symlink_alias() {
    ...
    assert!(
        error
            .to_string()
            .contains("owner RPC runtime directory must have mode 0700")
    );
    ...
    assert!(
        error
            .to_string()
            .contains("owner RPC runtime path must be a real directory")
    );
