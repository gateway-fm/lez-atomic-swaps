...
fn startup_requires_private_config_and_rejects_invalid_or_relative_socket_before_bind() {
    ...
    let help = Command::new(env!("CARGO_BIN_EXE_lez-taker-node"))
        .arg("--help")
        .output()
        .unwrap();
    assert!(help.status.success());
    let help = String::from_utf8(help.stdout).unwrap();
    assert!(help.contains("--role-config"));
    assert!(help.contains("/run/lez/taker/node.sock"));

    let missing_config_socket = runtime.join("missing-config.sock");
    let missing = Command::new(env!("CARGO_BIN_EXE_lez-taker-node"))
        .arg("--socket")
    ...
    let failed = Command::new(env!("CARGO_BIN_EXE_lez-taker-node"))
        .arg("--role-config")
        .arg(&invalid)
        .arg("--socket")
        .arg(&invalid_socket)
    ...
    let relative = Command::new(env!("CARGO_BIN_EXE_lez-taker-node"))
...
impl TestService {
    fn spawn(config: &Path, socket: &Path) -> Self {
        let node_config = socket.with_extension("node-config.json");
        let ready_file = socket.with_extension("ready");
        fs::write(
            &node_config,
        ...
        .unwrap();
        fs::set_permissions(&node_config, fs::Permissions::from_mode(0o600)).unwrap();
        let child = Command::new(env!("CARGO_BIN_EXE_lez-taker-node"))
            .arg("--config")
            .arg(node_config)
            .stdin(Stdio::null())
            .stdout(Stdio::null())
