use lez_swap_core::{
    Chain, ChainPosition, ClockBasis, Error, Pair, RefundSchedule, SwapDirection, TimelockSafety,
};
...
fn reverse_direction_maps_role_deadlines_to_the_correct_chains() {
    ...
    assert!(
        !schedule
            .maker_refund_reached(ChainPosition::block_height(Chain::Bitcoin, 499))
            .unwrap()
    );
    assert!(
        schedule
            .maker_refund_reached(ChainPosition::block_height(Chain::Bitcoin, 500))
            .unwrap()
    );
    assert!(
        schedule
    ...
    assert_eq!(
        schedule.maker_refund_reached(ChainPosition::block_height(Chain::Lez, 500)),
...
fn block_height_and_timestamp_are_never_compared_as_raw_numbers() {
    ...
    assert_eq!(
        schedule.maker_refund_reached(ChainPosition::block_height(Chain::Lez, 1_800_000_000)),
        Err(Error::WrongDeadlineClock {
