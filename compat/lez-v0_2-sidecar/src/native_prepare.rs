...
use lez_bridge_protocol::{
    ...
    RuntimeCompatibility, RuntimeDescriptor, SubmitNativeXmrClaimAuthorizationV3Request,
    SubmitTransactionRequest, TransactionId, WitnessedAssetPrepareStepV2,
    WitnessedAssetPreparedEffectV2, XmrNativeEffectV3, XmrNativeEscrowTermsV3,
};
use nssa::{
...
#[derive(Clone)]
struct ActiveXmrClaimPrepareV3 {
    request: PrepareNativeXmrClaimV3Request,
    result: PrepareNativeXmrClaimV3Result,
}
...
/// preparations. Native-XMR init, fund, and tag-16 refund submissions additionally
/// use their transaction IDs as canonical one-attempt request identities.
...
impl NativeEscrowPlanner {
    ...
    /// Durably reserves the exact unsigned generated tag-15 XMR claim message.
    ///
    /// The Maker process binds the complete v3 terms while the aggregate claim
    ...
    pub async fn prepare_native_xmr_claim_v3(
        &self,
        request: &PrepareNativeXmrClaimV3Request,
    ) -> Result<PrepareNativeXmrClaimV3Result, NativePrepareError> {
        let authority = self.validate_xmr_claim_v3_request(request)?;
        let mut state = self.state.lock().await;
        if let Some(active) = state.active_xmr_claim_v3.as_ref() {
            return if &active.request == request {
                self.validate_prepared_native_xmr_claim_v3(&active.request, &active.result)?;
                Ok(active.result.clone())
            } else {
        ...
        #[cfg(target_os = "linux")]
        if let Some(recovered) = self.recover_durable_xmr_claim_v3(request)? {
            state.active_xmr_claim_v3 = Some(ActiveXmrClaimPrepareV3 {
                request: request.clone(),
                result: recovered.clone(),
            });
            return Ok(recovered);
        ...
        let message = self.xmr_claim_v3_message(request, nonce)?;
        let result = PrepareNativeXmrClaimV3Result::new(
            request.context.clone(),
            request.terms,
            prepared_witnessed_from_message(request.context.request_id.clone(), &message)?,
        )?;
        self.validate_prepared_native_xmr_claim_v3(request, &result)?;
        #[cfg(target_os = "linux")]
        if let Some(store) = self.durable_store.as_ref()
            && let Err(error) = store.create(ReservationKind::XmrNativeClaimV3, request, &result)
        {
            if error == DurableReservationError::AlreadyReserved {
                let recovered = self
                    .recover_durable_xmr_claim_v3(request)?
                    .ok_or(DurableReservationError::Filesystem)?;
                state.active_xmr_claim_v3 = Some(ActiveXmrClaimPrepareV3 {
                    request: request.clone(),
                    result: recovered.clone(),
    ...
    /// Completes one exact durable tag-15 reservation with the aggregate witness.
    ///
    ...
    pub async fn complete_native_xmr_claim_v3(
        &self,
        request: &CompleteNativeXmrClaimV3Request,
    ) -> Result<CompleteNativeXmrClaimV3Result, NativePrepareError> {
        let mut state = self.state.lock().await;
        #[cfg(target_os = "linux")]
        if state.active_xmr_claim_v3.is_none()
            && let Some(recovered) = self.load_durable_xmr_claim_v3_for_completion(request)?
        {
            state.active_xmr_claim_v3 = Some(recovered);
        }
        let active = state
            .active_xmr_claim_v3
            .clone()
            .ok_or(NativePrepareError::ActiveWitnessedClaimPrepare)?;
        self.validate_xmr_claim_v3_completion_request(&active, request)?;
        if let Some(completed) = state.completed_xmr_claim_v3.as_ref() {
            return if &completed.request == request {
                self.validate_completed_native_xmr_claim_v3(&active, request, &completed.result)?;
        ...
        #[cfg(target_os = "linux")]
        if let Some(recovered) = self.recover_durable_xmr_claim_v3_completion(&active, request)? {
        ...
        let result =
            CompleteNativeXmrClaimV3Result::new(request.context.clone(), request.terms, prepared);
        self.validate_completed_native_xmr_claim_v3(&active, request, &result)?;
        #[cfg(target_os = "linux")]
        if let Some(store) = self.durable_store.as_ref()
            && let Err(error) = store.create(
            ...
            )
        {
            if error == DurableReservationError::AlreadyReserved {
                let recovered = self
                    .recover_durable_xmr_claim_v3_completion(&active, request)?
                    .ok_or(DurableReservationError::Filesystem)?;
                state.completed_xmr_claim_v3 = Some(ActiveXmrClaimCompletionV3 {
                    request: request.clone(),
                    result: recovered.clone(),
    ...
    /// Restores one exact tag-15 preparation from existing durable state only.
    ///
    /// This startup path never consults the nonce source and never regenerates
    /// a missing reservation. The journaled result and planner record must both
    /// independently revalidate and agree byte-for-byte.
    ///
    /// # Errors
    ///
    ...
    pub async fn restore_native_xmr_claim_v3(
        &self,
        request: &PrepareNativeXmrClaimV3Request,
        expected: &PrepareNativeXmrClaimV3Result,
    ) -> Result<(), NativePrepareError> {
        self.validate_prepared_native_xmr_claim_v3(request, expected)?;
        let mut state = self.state.lock().await;
        if let Some(active) = state.active_xmr_claim_v3.as_ref() {
            self.validate_prepared_native_xmr_claim_v3(&active.request, &active.result)?;
    ...
    /// Restores one exact tag-15 completion from existing durable state only.
    ///
    /// The corresponding preparation must already have been restored. This
    /// startup path never recreates a missing completion from the journaled
    /// signature.
    ///
    /// # Errors
    ///
    ...
    pub async fn restore_completed_native_xmr_claim_v3(
        &self,
        request: &CompleteNativeXmrClaimV3Request,
        expected: &CompleteNativeXmrClaimV3Result,
    ) -> Result<(), NativePrepareError> {
        let mut state = self.state.lock().await;
        let active = state
            .active_xmr_claim_v3
            .clone()
            .ok_or(NativePrepareError::InvalidTransactionBytes)?;
        self.validate_completed_native_xmr_claim_v3(&active, request, expected)?;
        if let Some(completed) = state.completed_xmr_claim_v3.as_ref() {
    ...
    pub(crate) async fn validate_submission_request(
        ...
        if let (Some(active), Some(completed)) = (
            state.active_xmr_refund_v3.as_ref(),
            state.completed_xmr_refund_v3.as_ref(),
        ) && completed.result.refund == request.transaction
        {
            self.validate_completed_native_xmr_refund_v3(
                active,
                &completed.request,
            ...
            #[cfg(target_os = "linux")]
            {
                self.validate_durable_owned_completed_native_xmr_refund_v3(
                    active,
                    completed,
                    &request.transaction,
                )?;
                return Ok(OwnedSubmission::XmrRefund);
            }
            #[cfg(not(target_os = "linux"))]
            return Err(NativePrepareError::InvalidTransactionBytes);
        }
    ...
    /// claim and request-bound tag-16 refund are admitted; tag 14 remains
    /// exclusive to its dedicated release route. Native-XMR tag-13 effects use
    /// the request-aware validator above.
    pub async fn validate_owned_submission(
        ...
        if self.validate_completed_xmr_submission(&state, prepared)? {
            return Ok(());
        }
        if let (Some(active), Some(completed)) = (
    ...
    fn validate_completed_xmr_submission(
        &self,
        state: &PlannerState,
        prepared: &PreparedTransaction,
    ) -> Result<bool, NativePrepareError> {
        if let (Some(active), Some(completed)) = (
        ...
        ) && &completed.result.claim == prepared
        {
            ...
            #[cfg(target_os = "linux")]
            self.validate_durable_owned_completed_native_xmr_claim_v3(active, completed, prepared)?;
            #[cfg(not(target_os = "linux"))]
            return Err(NativePrepareError::InvalidTransactionBytes);
            return Ok(true);
        }
        if let (Some(active), Some(completed)) = (
            state.active_xmr_refund_v3.as_ref(),
            state.completed_xmr_refund_v3.as_ref(),
        ) && &completed.result.refund == prepared
        {
            self.validate_completed_native_xmr_refund_v3(
                active,
                &completed.request,
    ...
    #[cfg(target_os = "linux")]
    fn recover_durable_xmr_claim_v3(
        &self,
        request: &PrepareNativeXmrClaimV3Request,
    ) -> Result<Option<PrepareNativeXmrClaimV3Result>, NativePrepareError> {
        let Some(store) = self.durable_store.as_ref() else {
            return Ok(None);
        };
        let Some((stored_request, stored_result)) = store
        ...
        self.validate_prepared_native_xmr_claim_v3(&stored_request, &stored_result)?;
        if &stored_request != request {
    ...
    #[cfg(target_os = "linux")]
    fn load_durable_xmr_claim_v3_for_completion(
        &self,
        completion: &CompleteNativeXmrClaimV3Request,
    ) -> Result<Option<ActiveXmrClaimPrepareV3>, NativePrepareError> {
        let Some(store) = self.durable_store.as_ref() else {
            return Ok(None);
        };
        let Some((stored_request, stored_result)) = store
        ...
        Ok(Some(ActiveXmrClaimPrepareV3 {
            request: stored_request,
            result: stored_result,
        }))
    }
    ...
    #[cfg(target_os = "linux")]
    fn recover_durable_xmr_claim_v3_completion(
        &self,
        active: &ActiveXmrClaimPrepareV3,
        request: &CompleteNativeXmrClaimV3Request,
    ) -> Result<Option<CompleteNativeXmrClaimV3Result>, NativePrepareError> {
        let Some(store) = self.durable_store.as_ref() else {
            return Ok(None);
        };
        let Some((stored_request, stored_result)) = store
        ...
        self.validate_xmr_claim_v3_completion_request(active, &stored_request)?;
        self.validate_completed_native_xmr_claim_v3(active, &stored_request, &stored_result)?;
        if &stored_request != request {
    ...
    /// Revalidates one durable unsigned tag-15 reservation against its request.
    ///
    /// # Errors
    ///
    ...
    pub fn validate_prepared_native_xmr_claim_v3(
        &self,
        request: &PrepareNativeXmrClaimV3Request,
        result: &PrepareNativeXmrClaimV3Result,
    ) -> Result<(), NativePrepareError> {
        self.validate_xmr_claim_v3_request(request)?;
        if result.context != request.context
            || result.terms != request.terms
        ...
        if message != self.xmr_claim_v3_message(request, u128::from(*nonce))? {
            return Err(NativePrepareError::InvalidTransactionBytes);
        }
        Ok(())
    }

    fn validate_xmr_claim_v3_completion_request(
        &self,
        active: &ActiveXmrClaimPrepareV3,
        request: &CompleteNativeXmrClaimV3Request,
    ) -> Result<(), NativePrepareError> {
    ...
    fn validate_completed_native_xmr_claim_v3(
        &self,
        active: &ActiveXmrClaimPrepareV3,
        request: &CompleteNativeXmrClaimV3Request,
        result: &CompleteNativeXmrClaimV3Result,
        ...
        let expected_authority = AccountId::new(*terms.claim_authority_account_id.as_bytes());
        ...
        if signature.value != *request.aggregate_signature.as_bytes()
            || public_key.value() != terms.claim_aggregate_x_only_public_key.as_bytes()
        {
            return Err(NativePrepareError::InvalidSignature);
        }
        Ok(())
    ...
    #[cfg(target_os = "linux")]
    fn validate_durable_owned_completed_native_xmr_refund_v3(
        &self,
        active: &ActiveXmrRefundPrepareV3,
        completed: &ActiveXmrRefundCompletionV3,
        prepared: &PreparedTransaction,
    ) -> Result<(), NativePrepareError> {
        let store = self
            .durable_store
        ...
        if active.request != stored_prepare_request
            ...
            || completed.request != stored_completion_request
            || completed.result != stored_completion_result
            || &stored_completion_result.refund != prepared
        {
            return Err(NativePrepareError::InvalidTransactionBytes);
        }
        let stored_active = ActiveXmrRefundPrepareV3 {
            request: stored_prepare_request,
            result: stored_prepare_result,
        };
        self.validate_completed_native_xmr_refund_v3(
    ...
    fn validate_xmr_claim_v3_request(
        &self,
        request: &PrepareNativeXmrClaimV3Request,
    ) -> Result<AccountId, NativePrepareError> {
        ...
        self.validate_xmr_terms_v3_binding(&request.context, &request.runtime, &request.terms)?;
        let public_key = PublicKey::try_new(*terms.claim_aggregate_x_only_public_key.as_bytes())
            .map_err(|_| NativePrepareError::WrongAggregateAuthority)?;
        let authority = AccountId::from(&public_key);
        if authority.into_value() != *terms.claim_authority_account_id.as_bytes() {
            return Err(NativePrepareError::WrongAggregateAuthority);
        }
        Ok(authority)
    }

    fn xmr_claim_v3_message(
        &self,
        request: &PrepareNativeXmrClaimV3Request,
        nonce: u128,
    ) -> Result<Message, NativePrepareError> {
        ...
        let authority = AccountId::new(*terms.claim_authority_account_id.as_bytes());
        Message::try_new(
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            match effect {
                ...
                XmrNativeEffectV3::Claim => {
                    self.validate_owned_xmr_claim_v3(store, context, runtime, terms, target)?;
                }
    ...
    #[cfg(target_os = "linux")]
    fn validate_owned_xmr_claim_v3(
        &self,
    ...
    #[cfg(target_os = "linux")]
    fn validate_owned_xmr_refund_v3(
        &self,
        store: &DurableReservationStore,
        context: &MessageContext,
        runtime: &RuntimeDescriptor,
        ...
        let (completion_request, completion_result) = store
            ...
            .ok_or(NativePrepareError::InvalidTransactionBytes)?;
        let active = ActiveXmrRefundPrepareV3 {
            request: prepare_request,
            result: prepare_result,
        };
        self.validate_completed_native_xmr_refund_v3(
        ...
        if active.request.context.run_id != context.run_id
            ...
            || &active.request.runtime != runtime
            || &active.request.terms != terms
            || &completion_result.refund != target
        {
            return Err(NativePrepareError::InvalidTransactionBytes);
        }
        Ok(())
    ...
    fn xmr_escrow_v3_messages(
        ...
        self.validate_xmr_escrow_v3_request(request)?;
        xmr_escrow_v3_messages_for_public_binding(
            self.escrow_program_id,
            self.authenticated_transfer_program_id,
            request,
...
/// Validates a durable native-XMR escrow pair without possessing the Taker key.
///
/// This is the same exact request, generated-ABI, account-order, instruction,
/// canonical-byte, signature, signer, and consecutive-nonce predicate used by
/// the native escrow planner. An optional nonce binds recovery to a separately
/// finalized account snapshot.
pub(crate) fn validate_prepared_native_xmr_escrow_v3_public(
    expected_runtime: &RuntimeDescriptor,
    authenticated_transfer_program_id: [u32; 8],
    request: &PrepareNativeXmrEscrowV3Request,
    result: &PrepareNativeXmrEscrowV3Result,
    expected_initialization_nonce: Option<u128>,
) -> Result<(), NativePrepareError> {
    validate_xmr_escrow_v3_request_public(
        expected_runtime,
        authenticated_transfer_program_id,
        request,
    )?;
    ...
    let signer = AccountId::new(*expected_runtime.signer_account_id.as_bytes());
    let initialization = decode_prepared_for_signer(&result.initialization, signer)?;
    let funding = decode_prepared_for_signer(&result.funding, signer)?;
    let [initialization_nonce] = initialization.message().nonces.as_slice() else {
        return Err(NativePrepareError::InvalidTransactionBytes);
    };
    let initialization_nonce = u128::from(*initialization_nonce);
    if expected_initialization_nonce.is_some_and(|expected| expected != initialization_nonce) {
        return Err(NativePrepareError::InvalidTransactionBytes);
    }
    let funding_nonce = initialization_nonce
        .checked_add(1)
    ...
    let escrow_program_id = program_id_from_hex(expected_runtime.escrow_program_id);
    let (expected_initialization, expected_funding) = xmr_escrow_v3_messages_for_public_binding(
        escrow_program_id,
        authenticated_transfer_program_id,
        request,
        initialization_nonce,
...
fn validate_xmr_escrow_v3_request_public(
    expected_runtime: &RuntimeDescriptor,
    authenticated_transfer_program_id: [u32; 8],
    request: &PrepareNativeXmrEscrowV3Request,
) -> Result<(), NativePrepareError> {
    if expected_runtime.sidecar_role != Participant::Taker
        || request.context.sidecar_role != Participant::Taker
        || request.runtime.sidecar_role != Participant::Taker
        || request.terms.to_input().depositor != Participant::Taker
    {
        return Err(NativePrepareError::WrongRole);
    }
    ...
    let terms = request.terms.to_input();
    if request.runtime.signer_account_id != terms.depositor_account_id {
        return Err(NativePrepareError::WrongSigner);
    }
    let escrow_program_id = program_id_from_hex(expected_runtime.escrow_program_id);
    if escrow_program_id == [0; 8]
        || request.runtime.escrow_program_id != program_id_to_hex(escrow_program_id)
        || terms.escrow_program_id != program_id_to_hex(escrow_program_id)
    {
        return Err(NativePrepareError::WrongEscrowProgram);
    }
    if authenticated_transfer_program_id == [0; 8]
        || authenticated_transfer_program_id == escrow_program_id
        || terms.authenticated_transfer_program_id
            != program_id_to_hex(authenticated_transfer_program_id)
    {
        return Err(NativePrepareError::WrongAuthenticatedTransferProgram);
    }
    let swap_id = *terms.swap_id.as_bytes();
    if terms.metadata_account_id
        != Hex32::from_bytes(compute_metadata_pda(&escrow_program_id, &swap_id).into_value())
        || terms.custody_account_id
            != Hex32::from_bytes(compute_custody_pda(&escrow_program_id, &swap_id).into_value())
    {
        return Err(NativePrepareError::InvalidTransactionBytes);
    }
    let claim_key = PublicKey::try_new(*terms.claim_aggregate_x_only_public_key.as_bytes())
...
fn xmr_escrow_v3_messages_for_public_binding(
    escrow_program_id: [u32; 8],
    authenticated_transfer_program_id: [u32; 8],
    request: &PrepareNativeXmrEscrowV3Request,
    initialization_nonce: u128,
    ...
    let terms = request.terms.to_input();
    let swap_id = *terms.swap_id.as_bytes();
    let metadata = compute_metadata_pda(&escrow_program_id, &swap_id);
    let custody = compute_custody_pda(&escrow_program_id, &swap_id);
    let depositor = AccountId::new(*terms.depositor_account_id.as_bytes());
    let claimant = AccountId::new(*terms.claimant_account_id.as_bytes());
    ...
    let initialization = Message::try_new(
        ...
        ZecEscrowInstruction::InitializeNativeXmr {
            ...
            claim_aggregate_x_only_public_key: *terms.claim_aggregate_x_only_public_key.as_bytes(),
            refund_aggregate_x_only_public_key: *terms
                .refund_aggregate_x_only_public_key
                .as_bytes(),
            maker_dleq_transcript_commitment: *terms.maker_dleq_transcript_commitment.as_bytes(),
            taker_dleq_transcript_commitment: *terms.taker_dleq_transcript_commitment.as_bytes(),
            ...
            authenticated_transfer_program: authenticated_transfer_program_id,
        },
    )
    .map_err(|_| NativePrepareError::InstructionEncoding)?;
    let funding = Message::try_new(
