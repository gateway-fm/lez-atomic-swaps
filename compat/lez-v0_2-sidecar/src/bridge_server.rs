...
type RestoredRequests = (
    ...
    Option<
        Box<(
            ...
            lez_bridge_protocol::PrepareNativeXmrClaimV3Result,
        )>,
    >,
    Option<
        Box<(
            lez_bridge_protocol::CompleteNativeXmrClaimV3Request,
            lez_bridge_protocol::CompleteNativeXmrClaimV3Result,
        )>,
    >,
    Option<
        Box<(
            lez_bridge_protocol::PrepareNativeXmrRefundV3Request,
            lez_bridge_protocol::PrepareNativeXmrRefundV3Result,
        )>,
    >,
    Option<
        Box<(
    ...
    fn restore_requests(&self) -> Result<RestoredRequests, BridgeServerError> {
        ...
        for entry in self.persisted.entries.values() {
            ...
            match entry.method.as_str() {
                ...
                METHOD_PREPARE_NATIVE_XMR_CLAIM_V3 if xmr_claim_v3.is_none() => {
                    xmr_claim_v3 = Some(Box::new((
                        serde_json::from_value(request)
                            .map_err(|_| BridgeServerError::InvalidDurableState)?,
                        serde_json::from_value(value.clone())
                            .map_err(|_| BridgeServerError::InvalidDurableState)?,
                ...
                METHOD_COMPLETE_NATIVE_XMR_CLAIM_V3 if completed_xmr_claim_v3.is_none() => {
                    completed_xmr_claim_v3 = Some(Box::new((
                        serde_json::from_value(request)
                            .map_err(|_| BridgeServerError::InvalidDurableState)?,
                        serde_json::from_value(value.clone())
                            .map_err(|_| BridgeServerError::InvalidDurableState)?,
                ...
                METHOD_PREPARE_NATIVE_XMR_REFUND_V3 if xmr_refund_v3.is_none() => {
                    xmr_refund_v3 = Some(Box::new((
                        serde_json::from_value(request)
                            .map_err(|_| BridgeServerError::InvalidDurableState)?,
                        serde_json::from_value(value.clone())
                            .map_err(|_| BridgeServerError::InvalidDurableState)?,
                ...
                METHOD_COMPLETE_NATIVE_XMR_REFUND_V3 if completed_xmr_refund_v3.is_none() => {
                    completed_xmr_refund_v3 = Some(Box::new((
                        serde_json::from_value(request)
                            .map_err(|_| BridgeServerError::InvalidDurableState)?,
                        serde_json::from_value(value.clone())
                            .map_err(|_| BridgeServerError::InvalidDurableState)?,
...
async fn restore_runtime_requests(
    ...
    if let Some(restored) = restored_xmr_claim_v3 {
        let (request, expected) = *restored;
        runtime
            .restore_native_xmr_claim_v3(&request, &expected)
            .await
            .map_err(|_| BridgeServerError::InvalidDurableState)?;
    }
    if let Some(restored) = restored_xmr_claim_completion_v3 {
        let (request, expected) = *restored;
        runtime
    ...
    if let Some(restored) = restored_xmr_refund_v3 {
        let (request, expected) = *restored;
        runtime
    ...
    if let Some(restored) = restored_xmr_refund_completion_v3 {
        let (request, expected) = *restored;
        runtime
        ...
        |params, state, _| async move {
            ...
            state
                .execute(
                    ...
                    || async move {
                        runtime
                            .classify_finalized_witnessed_claim(&operation)
                            .await
                            .map_err(Into::into)
                            .and_then(to_value)
                    },
        ...
        |params, state, _| async move {
            ...
            state
                .execute(
                    ...
                    || async move {
                        runtime
                            .classify_finalized_witnessed_initialization(&operation)
                            .await
                            .map_err(Into::into)
                            .and_then(to_value)
                    },
...
fn register_xmr_v3_methods(
    ...
    module.register_async_method(
        METHOD_PREPARE_NATIVE_XMR_CLAIM_V3,
        |params, state, _| async move {
            let request: lez_bridge_protocol::PrepareNativeXmrClaimV3Request = params.one()?;
            state.validate_xmr_v3_request(
                &request.context,
                &request.runtime,
                &request.terms,
            ...
            state
                .execute(
                    ...
                    || async move {
                        runtime
                            .prepare_native_xmr_claim_v3(&operation)
                            .await
    ...
    module.register_async_method(
        METHOD_COMPLETE_NATIVE_XMR_CLAIM_V3,
        |params, state, _| async move {
            let request: lez_bridge_protocol::CompleteNativeXmrClaimV3Request = params.one()?;
            state.validate_xmr_v3_request(
                &request.context,
                &request.runtime,
                &request.terms,
            ...
            state
                .execute(
                    ...
                    || async move {
                        runtime
                            .complete_native_xmr_claim_v3(&operation)
                            .await
                            .map_err(OperationFailure::from)
                            .and_then(to_value)
                    },
    ...
    module.register_async_method(
        METHOD_PREPARE_NATIVE_XMR_REFUND_V3,
        |params, state, _| async move {
            let request: lez_bridge_protocol::PrepareNativeXmrRefundV3Request = params.one()?;
            state.validate_xmr_v3_request(
                &request.context,
                &request.runtime,
                &request.terms,
            ...
            state
                .execute(
                    ...
                    || async move {
                        runtime
                            .prepare_native_xmr_refund_v3(&operation)
                            .await
                            .map_err(OperationFailure::from)
                            .and_then(to_value)
                    },
    ...
    module.register_async_method(
        METHOD_COMPLETE_NATIVE_XMR_REFUND_V3,
        |params, state, _| async move {
            let request: lez_bridge_protocol::CompleteNativeXmrRefundV3Request = params.one()?;
            state.validate_xmr_v3_request(
                &request.context,
                &request.runtime,
                &request.terms,
            ...
            state
                .execute(
                    ...
                    || async move {
                        runtime
                            .complete_native_xmr_refund_v3(&operation)
                            .await
                            .map_err(OperationFailure::from)
                            .and_then(to_value)
                    },
