...
| U6 | Basecamp taker mini-app browses and executes swaps | Prototype/browser, seven-method DTO, owner service, prepared acceptance, receipt-bound reads, and terminal authorization are GREEN. Certificate `m6cert20260803164006` proves the service-driven actual-local-node `TakerSellsLez` Claim and exact durable replay with one unchanged Zebra mempool transaction. Equivalent Refund and the actor-real Basecamp journey remain | M6 active under ADRs 0128 and 0130 through 0137; Maker/Taker `ui_qml`, QtRO, actor-real UI E2E, and owner signoff remain pending |
...
| S6 | README covers deployment, addresses, prerequisites, and maker/taker CLI/mini-app use | Fresh-machine documentation tests plus link/command validation | Partial M6: prototype review, service configuration, local resources, flakiness, prepared acceptance, locked monitoring, and the actual-node service Claim/replay command are documented. Basecamp package build/load, production assets, actor-real GUI use, equivalent Refund, and final link/command validation remain |
| S7 | Write-up covers protocols, escrow, atomicity, timelocks, assumptions, limitations | M1 design packet reviewed, then updated from executable evidence and audit findings | Living architecture contains pair component and sequence diagrams plus conditional atomicity arguments. ADR 0137 records authorization-before-effect, one terminal branch, lock scope, actor journal replay, and the fresh local Claim evidence. It explicitly does not claim a distributed cross-chain transaction. Equivalent Refund and actor-real UI composition remain for M6; durable rollback fencing is deferred production hardening and M7 retains independent review |
