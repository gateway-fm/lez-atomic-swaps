...
| F8 | Pluggable local and Logos-module C-API price sources | Price-port contract suite; config/CLI mutation E2E; fake and real C-ABI adapter tests including stale/unavailable feeds | Partial M5: local daemon/CLI quoting is GREEN across restart. The provisional C-API v1 one-shot worker is actual-C GREEN for exact quotes, typed missing/unavailable, malformed ABI/route/ratio/revision/time/reserved fields, and native abort. Parent timeout/artifact/output controls, daemon/store anti-rollback, signed-offer binding, and eventual upstream compatibility remain |
