...
| F4 | LEZ-transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors, profiles, agreement validation, canonical funding observations, and fresh maker eligibility pass. Separate role-fixed maker/taker SDKs and schema-v10 SQLite stores complete and reopen the direction-fixed claim or refund lifecycles. The authenticated eight-method client protocol, official native/revealing-claim preparation, official native and revealing-claim owner/discovery observation, executable role-isolated sidecars, main escrow agreement validator, role-correct both-direction Zcash funding submission, and production Zebra claim/refund boundaries pass isolated suites; actual Zebra fork/restart and local LEZ custody lifecycles still pass separately, not as one composed flow. | Deterministic SDK/SQLite lifecycles plus isolated production bridge/sidecar/Zebra boundaries implemented; native LEZ refund, main claim/refund SDK composition, Zcash unknown-ID funding observation, margin cases, reference actors, and public-testnet evidence remain M2 work |
