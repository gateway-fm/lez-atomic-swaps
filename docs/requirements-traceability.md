...
is GREEN under ADR 0115. A 2026-07-30 audit withdrew role-correct user-flow certification from H because the runner used provisioner funding, a Taker-hosted shared-wallet RPC, and a Maker destination. Corrected code requires Maker funding, a neutral shared-wallet RPC, and Taker receipt; fresh replay is pending and the historical evidence remains immutable. Refund attempt `m5xmrrefund8c10cd7a` subsequently reached finalized tag 13 and verified Monero funding, then failed closed when the actual local finalized identity did not advance through empty-block polling. ADR 0123 records the contract-GREEN one-reservation, one-attempt terms-sealed Taker-to-Maker unit transfer, followed only by read-only verification and authoritative Maker finality classification. Its protocol, planner, client, and runner contracts pass 325 Rust tests and the complete repository quality/security gate; the live runtime/server path lacks a focused behavioral test, and a fresh actual-node replay is still required and no literal M5 output or tag is claimed. Pushed replay `m5xmrrefund827a5d4a` reached the signed refund threshold after both devnets, finalized tag 13, and Maker-funded Monero verification, then failed before any clock effect because a prefixed preparation request ID exceeded the 64-character protocol bound. The TDD repair uses versioned, operation-domain-separated SHA-256 IDs; 215 sidecar tests and strict gates pass, but fresh actual-node replay remains open.
...
| F5 | Risc0 LEZ escrow validates pair proof and supports claim/refund | Per-pair guest instruction tests plus standalone-sequencer initialise/claim/refund E2E | Historical M2/M3 checked and deployed artifacts remain certified. M4 tags 13–17 retain the twice-reproduced checked ELF/ImageID and private local deployment. The working-tree checkpoint executed finalized Initialize/Fund at heights 3953/3960, finalized tag 14 at 4107, and finalized tag 15 at 4208 with terminal custody zero. Exact clean-commit replay remains. Tag-16 exact submission and finalized classification are component-GREEN. The actual refund attempt exposed idle finalized-clock liveness before tag 16; its single sealed local clock effect is contract-GREEN, while actual refund recovery and punishment paths are still absent. Public deployment stays deferred under ADR 0023; Logos-owned disclosures remain tracked under ADR 0018 |
| F6 | Atomic outcome: both claim or both refund | Six scenario tests and 512 generated transition sequences pass. M2/M3 retain their certified conditional claim/refund evidence. For M4, Stage A/B precommitted exact claim, signed-refund, and punishment messages; the Taker partial stayed private until exact XMR funding; finalized Maker claim disclosed the share used for the Taker sweep | Partial: the M4 successful claim branch executed conditionally atomically, not as one distributed cross-chain transaction. The claim binder directly cross-checks finalized LEZ height 4208/tip 4220 against Monero receipt height 121/tip 130, but is a historical snapshot and not future-reorg immunity. The refund runner/binder now requires finalized tag 16 in the signed half-open window, derives the Taker share only for the Maker after that effect, and verifies the Maker-directed Monero receipt with honest roles and exact accounting. The first clean refund attempt failed closed before tag 16 on unchanged local finality. The contract-GREEN one-unit tick leaves escrow byte-identical and does not itself authorize refund; literal both-refund conformance awaits a clean actual-node replay. Tag 17, adversarial races, process-kill, reorg, and concurrency remain. GW-M4-003 preserves the punishment/economic-safety discrepancy |
...
| U3 | Maker CLI configures pairs/prices, controls daemon, queries history, and triggers claim/refund over IPC/RPC | Black-box CLI-to-daemon command matrix under owner/wrong-owner roles plus fixed-system-service contract | Partial M5: configure/price/offers/create/status/history/monitor and supported generation-fenced actions pass through the real daemon/CLI with replay and restart evidence. Literal start/stop controls only the packaged system unit with bounded exact-state verification. Receipt-only XMR Taker monitoring is intentionally a separate CLI boundary and does not satisfy the missing daemon-owned XMR effect actions; complete the supported-pair lifecycle matrix |
| U4 | Taker CLI covers discovery, initiation, monitoring, claim, and refund | Actual taker CLI drives each pair’s happy and abandoned-counterparty journey | Partial M5: ZEC discovery/initiation/receipt-bound monitor/claim/refund, BTC discovery/initiation/offline monitor, and XMR discovery/initiation/receipt-only application monitor are process-GREEN; clean actual-node BTC and composed XMR application corridors exist. The role-correct XMR refund effect is composed in the application runner but not yet exposed through the literal Taker lifecycle CLI or clean-replayed. XMR monitor validates pre-effect application authority under the per-swap lock and explicitly does not infer chain progress. Abandonment journeys remain hardening unless required to expose a PoC defect |
...
| S7 | Write-up covers protocols, escrow, atomicity, timelocks, assumptions, limitations | M1 design packet reviewed, then updated from executable evidence and audit findings | Living architecture contains dedicated Mermaid components and sequences for both BTC directions, both transparent-ZEC directions, and the supported LEZ-first XMR successful and recovery branches. ADR 0122 adds the full local-node/RPC topology, role-correct tag-16-to-Monero sequence, and exact conditional atomicity argument for the reproducible refund runner. ADR 0123 records the observed empty-block finality stall and the contract-GREEN terms-sealed, durably reserved, one-attempt local clock effect with read-only accounting and escrow-identity proof while finalized classification remains authoritative. It explains why neither the tick nor binder is a distributed commit or future-reorg guarantee and why clean actual execution, punishment, and hardening remain. Stable markers remain CI-guarded; M7 retains independent formal review |
| S8 | Every pair SDK public API has docs/errors/examples for full lifecycle | `cargo doc -D warnings`, doctests, and public-API coverage check for all three SDKs | Partial across all pairs: ZEC and BTC expose mature role-fixed/full-lifecycle surfaces with strict Rustdoc. XMR exposes the cryptographic protocol, agreement, actor, release, and clean composed claim corridor, but daemon/Taker lifecycle APIs and a full external wiring example remain incomplete |
