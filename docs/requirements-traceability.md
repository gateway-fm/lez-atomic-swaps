...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors and immutable profiles pass; the typed validator re-decodes bytes and binds network/branch/block/outpoint/value/scripts/depth, and actual Zebra E2E populates it from a stable genesis/NU6.2/raw-tx/canonical-height snapshot before core projection; Zebra also proves a conflicting four-over-three-block replacement; durable watcher events, cross-chain refund ordering, and both-direction testnet role E2E remain | ZEC consensus/deeper reorg, profile, validator, and actual snapshot implemented; watcher/margin composition and actor delivery remain M2 |
