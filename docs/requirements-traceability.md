...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors and immutable profiles pass; the validator retains decoded raw bytes and a stable tip while binding network/branch/block/outpoint/value/scripts/depth; actual Zebra populates it and proves a conflicting four-over-three-block replacement; the two-phase watcher emits only stable canonical, affirmative detach, or atomic replacement evidence and never treats RPC absence as removal; actual two-Zebra evidence now crosses immutable binding, schema-v4 commit, close/reopen, fresh-query dedupe, fork removal, second restart, and exact retry; primitive observation and immutable profile/output binding records are revalidated and persisted; runtime requires binding/policy/event agreement and atomically journals post-dependent conflicting replacements while retaining the committed ID/reorg phase; cross-chain refund ordering and both-direction testnet role E2E remain | ZEC consensus/deeper reorg, profile, validator, actual snapshot, watcher, records, schema-v4 binding/journal/alert enforcement, core role semantics, and actual-node restart/requery runtime composition implemented; full SDK, testnet, and actor delivery remain M2 |
...
| R4 | Persisted state survives crash/restart without fund loss | SQLite reopen and actual daemon kill/restart pass; schema-v4 ZEC journal/binding/alert storage proves migration without inferred terms, future-version rejection, atomic swap+binding and event+aggregate+alert rollback, immutable rebinding, stale revisions, role isolation, and revalidated loading; both-direction runtime tests probe unknown-outcome replay before mutation, replay ordered records into the exact historical tracker head, reject impossible sequence history, require every new event to advance that head before projection, and require fresh reconciliation after close/open; conflict/terminal alerts deduplicate and preserve swap-scoped acknowledgment across replay/restart; authenticated CLI status/list/ack retains phase and history; the actual two-Zebra fixture proves canonical commit, two close/reopen cycles, fresh requery, affirmative fork removal, and no duplicate on retry; kill-at-every-effect-transition, corruption, encryption, and general effect outbox remain | Partial |
