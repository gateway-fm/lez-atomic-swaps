...
| F8 | Pluggable local and Logos-module C-API price sources | Price-port contract suite; config/CLI mutation E2E; fake and real C-ABI adapter tests including stale/unavailable feeds | Partial M5: local daemon/CLI quoting is GREEN. The provisional actual-C worker and bounded parent are GREEN for validation, typed unavailability, abort/hang containment, exact timeout reap, owner/path/mode/link/hash checks, and bounded output. Daemon/store anti-rollback, retry identity, atomic signed-offer binding, and eventual upstream compatibility remain |
