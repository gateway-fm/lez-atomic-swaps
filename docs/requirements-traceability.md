...
| R4 | Persisted state survives crash/restart without fund loss | Schema-v7 SQLite retains agreement, canonical observation history, separate taker and maker intents, exact confirmed transitions, aggregate revision, bindings, and alerts. Eleven SDK-recovery tests cover role isolation, rollback, corruption/torn history, stale catch-up, exact replay, and both-direction close/reopen at `BothLegsLocked`; union replay rejects holes and duplicate predecessor slots. Actual daemon and two-Zebra restart evidence also passes. Maker-effect fault matrices, later-effect kill points, encryption, and the general M5 outbox remain. | Partial |
