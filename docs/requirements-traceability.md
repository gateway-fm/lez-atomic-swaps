...
|---|---|---|---|
| F1 | No central server; Delivery advertisements and Chat coordination | Role E2E discovers and negotiates through real Delivery/Chat adapters; central services absent; UJ-006 repeats with both stopped after lock | Partial M5: signed bounded daemon-to-taker run-local Delivery publication/discovery plus restart reconciliation and durable expiring offers, one-winner reservation, and atomic coordinator consumption are component GREEN; maker-first ZEC proposal/countersigning and bounded BTC unsigned draft handoff are GREEN; schema 19 now stages the exact BTC Maker signature before response and atomically commits the final wire, coordinator, consumed offer, and Bitcoin Maker actor, with pre-provisioning lost-response replay; the disjoint Chat process now authenticates, signs, stages before response, countersigns as the taker role, atomically accepts with daemon-local protected authority, isolates methods, and survives kill/reopen. The actual taker command durably persists the countersigned wire before completion, validates its private agreement/draft and both roles on rerun, and retries only exact completion. The daemon holds startup-pinned per-swap Maker authority, durably publishes one Maker-only actor bundle, atomically registers its schema-v16 manifest, and preflights the exact committed request/negotiation/actor before any current-wall-clock parse or provisioning. Actor-bearing systemd lifecycle, exact-snapshot BTC/ZEC role/swap/state admission, and one bounded pair-neutral supervisor cycle are GREEN. The cycle retains one kernel lock through sealed status/effect processes and durable fenced resolution. The real daemon/taker journey proves that retry after a three-second TTL returns the original result; changed inputs or missing actor fail closed, and a stale consumed Delivery projection reports ready plus degraded/unavailable until reconciliation. Long-running daemon/systemd supervisor execution, disjoint concurrent application swaps, actual-node supervisor composition, and other-pair composition remain |
...
| F9 | Headless maker covers configuration, pricing, advertisement, execution, monitoring, and full CLI operation | UJ-007 actual CLI/daemon suite plus restart, history, manual claim/refund, pricing, and advertisement cases | Partial M5: owner-local configure/list/quote, offer publish/list/withdraw, create/status/history, durable alerts, and allowlisted actor monitor survive restart. Generation-fenced ZEC claim/refund and BTC refund admission use exact global request replay; BTC claim is deliberately unsupported. Fresh actual-node supervisor execution, Taker controls, concurrent composition, and XMR remain |
...
| U2 | Long-running autonomous maker daemon plus systemd unit/install guide | Packaged daemon runs under hardened systemd in an isolated VM, restarts, advertises, prices, and executes without GUI | Partial M5: hardened `Type=notify` operation, credentials, exact installed actor identity, staged verification, and user-systemd readiness/SIGKILL-restart/SIGTERM-cleanup are GREEN. Node-free run `lez-m5-systemd-1000-3497452-2505` additionally proves fenced scheduler recovery, one unchanged durable local effect, and disjoint peer progress across a ten-second daemon restart. A real-daemon two-row journey now proves one timed-out actor enters durable backoff without preventing a disjoint actor from reaching terminal; both exact manifests and one-attempt counts survive restart, invocation logs stay unchanged during the 300-millisecond observation window, and health remains responsive while the timed-out actor is leased. This is sequential process failure isolation, not simultaneous worker or chain evidence. Actual-chain actor crash recovery, actual-node supervisor routing, simultaneous multi-pair composition, and immutable live Logos Core integration remain |
| U3 | Maker CLI configures pairs/prices, controls daemon, queries history, and triggers claim/refund over IPC/RPC | Black-box CLI-to-daemon command matrix under owner/wrong-owner roles | Partial M5: create/status/auth/restart plus allowlisted actor monitor and generation-fenced claim/refund routing pass through the real daemon and CLI. Exact replay, payload conflict, missing actor, durable queued-action visibility, and restart-stable monitoring are GREEN. ZEC supports claim/refund; BTC supports refund only. Service lifecycle uses the separate systemd path; actual-node supervisor execution and all-pair composition remain |
| U4 | Taker CLI covers discovery, initiation, monitoring, claim, and refund | Actual taker CLI drives each pair's happy and abandoned-counterparty journey | Partial M5: key-pinned discovery and ZEC initiation through exact maker-proposal validation, local countersigning, atomic Chat completion, deterministic replay, and no-clobber agreement persistence are process-GREEN. Accepted ZEC swaps now no-clobber provision a role-fixed Taker bundle before completion and publish a digest-pinned receipt only after durable Maker completion. Receipt-bound monitor/claim/refund, exact artifact replay, tamper rejection, transport-free monitor, and Delivery-independent persisted completion replay are process-GREEN. A real Unix-HTTP fault boundary now drops only the already-successful Maker completion response, proves no premature receipt, and recovers through exact completion and artifact replay. The composed runner now prevalidates the effect-bearing Maker/Taker pair, pins the receipt around every receipt-based monitor or claim command, routes the happy claim only from exact claim evidence through `claim --receipt`, and binds its traces to the accepted swap. BTC now has symmetric role-fixed no-clobber Maker/Taker provisioning with exact replay, but the real BTC Taker CLI handoff is still open. This runner contract is GREEN; fresh actual-node Taker effects, BTC/XMR process composition, and abandonment remain |
...
| R8 | Delivery/Chat outage is retried/buffered/degraded and documented | Pre-lock outage/recovery matrix plus post-lock UJ-006 proves on-chain independence and user-visible degraded state | M5 local output GREEN: real maker Delivery failure leaves one durable offer, health reports ready plus degraded dependency state, exact repair replay republishes once, Chat loss after durable proposal creates no final output, daemon restart retains the reserved envelope for deterministic taker replay and one atomic completion, an observed successful completion response can be dropped before the Taker sees it without publishing a receipt and exact retry recovers inode-stably, and three actual-node runs remove both transports after first lock. LOGOS-020 remains upstream production parity |
