...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors and immutable profiles pass; a bounded dual-signed agreement binds actors, LEZ custody, BIP-199 output, transaction policy, actual LEZ deadline bound, and Zcash CLTV; concrete activation/resume, exact first-lock intent, primitive recovery-record revalidation, and production SQLite atomic projection/replay are integrated; funding observation is durable and actual Zebra proves fork replacement/restart; bounded spend recognition accepts every pinned-consensus sighash/high-S/nonminimal form without hiding the preimage and reports SDK policy separately; maker-independent observation, later-effect persistence, durable spend reorgs, cross-chain ordering, and both-direction testnet actor E2E remain | Concrete agreement, activation/resume, observe-before-rebroadcast plus unknown-commit first-lock contract, production first-lock recovery adapter, ZEC transaction/consensus/funding reconciliation, spend recognition, profiles, schema-v5 funding journal/alerts/recovery, core role semantics, and actual-node restart/requery are implemented; later SDK effects, chain adapters, testnet, and actor delivery remain M2 |
