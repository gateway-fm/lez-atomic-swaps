...
| R4 | Persisted state survives crash/restart without fund loss | SQLite reopen and actual daemon kill/restart pass; schema-v5 retains the ZEC journal/binding/alert guarantees and adds role/revision/foreign-key constrained SDK recovery tables, with future-version rejection, atomic swap+binding and event+aggregate+alert rollback, immutable rebinding, stale revisions, role isolation, and revalidated loading; both-direction runtime tests probe unknown-outcome replay before mutation, replay ordered records into the exact historical tracker head, reject impossible sequence history, require every new event to advance that head before projection, and require fresh reconciliation after close/open; conflict/terminal alerts deduplicate and preserve swap-scoped acknowledgment across replay/restart; authenticated CLI status/list/ack retains phase and history; the actual two-Zebra fixture proves canonical commit, two close/reopen cycles, fresh requery, affirmative fork removal, and no duplicate on retry; SDK recovery adapter, kill-at-every-effect-transition, corruption, encryption, and general effect outbox remain | Partial |
