...
| U2 | Long-running autonomous maker daemon plus systemd unit/install guide | Packaged daemon runs under hardened systemd in an isolated VM, restarts, advertises, prices, and executes without GUI | Partial M5: hardened `Type=notify` operation, credentials, exact installed actor identity, staged verification, and user-systemd readiness/SIGKILL-restart/SIGTERM-cleanup are GREEN. Node-free run `lez-m5-systemd-1000-3497452-2505` additionally proves fenced scheduler recovery, one unchanged durable local effect, and disjoint peer progress across a ten-second daemon restart. Actual-chain actor crash recovery, actual-node supervisor routing, multi-pair composition, and immutable live Logos Core integration remain |
