...
| U3 | Maker CLI configures pairs/prices, controls daemon, queries history, and triggers claim/refund over IPC/RPC | Black-box CLI-to-daemon command matrix under owner/wrong-owner roles | Partial M5: create/status/auth/restart pass. The raw ZEC actor now exposes role-fixed ordered recovery with typed bounded outcomes, but durable manual-action intent, service start/stop control, and maker CLI claim/refund routing remain |
