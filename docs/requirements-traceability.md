...
| U2 | Long-running autonomous maker daemon plus systemd unit/install guide | Packaged daemon runs under hardened systemd in an isolated VM, restarts, advertises, prices, and executes without GUI | Partial M7: hardened systemd lifecycle, exact identity, staged verification, crash recovery, and bounded 1..=32 worker supervision are GREEN. Checked run `m7btcconc-272788c-a` composes two direction-bound accepted BTC applications through one daemon/database, restarts before activation without replay, proves four isolated role states and signing journals at the shared revision-two barrier, completes both actual-chain claim paths, and replays all four terminal roles with zero resubmission. Actual-chain actor process-kill/crash recovery remains. Immutable live Logos Core integration stays an upstream production-parity exception |
