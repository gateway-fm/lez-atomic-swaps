...
| U6 | Basecamp taker mini-app browses and executes swaps | Prototype and isolated browser proof are GREEN. The seven-method DTO contract, authenticated backend, owner-only process, replay-first registry, prepared ZEC authority, and real acceptance are GREEN. Commit `e9393cf` registers receipt-bound swap list and monitor, resolves only exact service-owned admitted authority, reads typed actor status under the per-swap lock, and reproduces `NotActivated` generation zero with Delivery and Chat offline without artifact mutation or chain I/O. Hardening `3307dca` rejects same-byte receipt inode replacement and actor-lock contention and proves exact recovery. Commit `9cf1a34` adds fail-closed vanished/tampered custody, corrupt role-state, whole-list, and honest no-receipt restart evidence. Commit `c90b21d` adds real activated-SQLite `AwaitingFirstLock` projection and future/malformed row recovery without chain RPC. Actor driving, claim, refund, durable receipt/state rollback-incarnation hardening, standalone-service real-Chat E2E, and actor-real QML remain | M6 active under ADRs 0128 and 0130 through 0136; owner signoff still gates QML/QtRO |
...
| S6 | README covers deployment, addresses, prerequisites, and maker/taker CLI/mini-app use | Fresh-machine documentation tests plus link/command validation | Partial M6: prototype review, conditional service configuration, owner socket, authenticated reads, admission-first real ZEC Chat acceptance, exact private-authority replay, receipt-bound offline swap list/monitor, local resources, flakiness, and no-chain-effect limitations are documented. Basecamp package build/load, production assets, actor-real GUI use, terminal chain lifecycle flows, standalone-service real-Chat proof, and final link/command validation remain |
| S7 | Write-up covers protocols, escrow, atomicity, timelocks, assumptions, limitations | M1 design packet reviewed, then updated from executable evidence and audit findings | Living architecture contains pair component and sequence diagrams plus conditional atomicity arguments. ADR 0135 records pre-effect acceptance atomicity. ADR 0136 adds service-owned receipt-bound monitor and restart/offline sequences plus the lock-scoped read-atomicity argument: exact registry authority, reread under the shared actor lock, status-only unit ports, and no mutation or chain I/O. It adds process-incarnation receipt identity and lock-contention evidence plus vanished/tampered custody and corrupt-state whole-list evidence while leaving durable cross-restart receipt/state rollback fencing and terminal effects pending. M7 retains independent formal review |
