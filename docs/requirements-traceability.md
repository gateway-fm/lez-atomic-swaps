...
|---|---|---|---|
| F1 | No central server; Delivery advertisements and Chat coordination | Role E2E discovers and negotiates through real Delivery/Chat adapters; central services absent; UJ-006 repeats with both stopped after lock | Partial M5: signed bounded daemon-to-taker run-local Delivery publication/discovery plus restart reconciliation and durable expiring offers, one-winner reservation, and atomic coordinator consumption are component GREEN. ZEC proposal/countersigning remains process-GREEN. BTC now also runs the actual Maker CLI, maker daemon, and Taker CLI through signed Delivery, authenticated Chat proposal/completion, role-owned Schnorr signatures, schema-19 atomic final-wire/coordinator/offer/Maker-actor commit, role-only Maker/Taker provisioning, receipt-after-completion, Delivery-free exact replay, inode preservation, and offline monitor. The BTC process test uses no chain RPC, node, Docker, faucet, DNS, network, or public funds; isolated Core Regtest plus LEZ v0.2 execution remains. Actor-bearing systemd lifecycle, exact-snapshot BTC/ZEC admission, and one bounded pair-neutral supervisor cycle are GREEN. Long-running daemon/systemd supervisor execution, disjoint concurrent application swaps, actual-node supervisor composition, and other-pair composition remain |
...
| U4 | Taker CLI covers discovery, initiation, monitoring, claim, and refund | Actual taker CLI drives each pair's happy and abandoned-counterparty journey | Partial M5: key-pinned discovery and ZEC initiation through exact maker-proposal validation, local countersigning, atomic Chat completion, deterministic replay, and no-clobber agreement persistence are process-GREEN. Accepted ZEC swaps no-clobber provision a role-fixed Taker bundle before completion and publish a digest-pinned receipt only after durable Maker completion. Receipt-bound monitor/claim/refund, exact artifact replay, tamper rejection, transport-free monitor, and Delivery-independent persisted completion replay are process-GREEN. BTC discovery and initiation now also pass through the real Taker CLI: it validates the Maker proposal, contributes only the Taker Schnorr signature, persists the final wire before completion, provisions only the Taker actor, publishes a receipt only after durable Maker completion, repeats exact completion without Delivery or inode replacement, and monitors offline. This process PoC is GREEN; fresh actual-node BTC/ZEC Taker effects, XMR process composition, and abandonment remain |
