...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors and immutable profiles pass; a bounded dual-signed agreement binds actors, LEZ custody, BIP-199 output, transaction policy, actual LEZ deadline bound, and Zcash CLTV; funding observation is durable and actual Zebra proves fork replacement/restart; bounded spend recognition accepts every pinned-consensus sighash/high-S/nonminimal form without hiding the preimage and reports SDK policy separately; activation integration, durable spend reorgs, cross-chain ordering, and both-direction testnet actor E2E remain | Concrete agreement, ZEC transaction/consensus/funding reconciliation, spend recognition, profiles, schema-v4 funding journal/alerts, core role semantics, and actual-node restart/requery are implemented; full SDK effects, durable spend tracking, testnet, and actor delivery remain M2 |
...
| ID | Contract | Acceptance evidence | Status / milestone |
|---|---|---|---|
| U1 | Dedicated full-lifecycle SDK for each pair | Shared SDK contract suite instantiated for BTC/XMR/ZEC; public API doctests cover discovery through refund | Partial: ZEC role-fixed discovery/negotiation/activation/resume and the separate concrete dual-signed agreement validator pass; integration, typed active chain actions, and actor E2E remain M2 |
