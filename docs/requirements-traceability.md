...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Direction-aware LEZ-recipient/preimage and exact redeem/P2SH/scriptSig/V5 vectors pass; pinned Zebra accepts actor-funded claim/refund, rejects mutated signatures and early CLTV, and confirms both; real LEZ escrow plus both-direction testnet happy/refund/concurrency role E2E remain | ZEC chain transaction lifecycle implemented; cross-chain actor delivery remains M2 |
| F5 | Risc0 LEZ escrow validates pair proof and supports claim/refund | Per-pair guest instruction tests plus standalone-sequencer initialise/claim/refund E2E | ZEC custom-token instruction semantics pass; standalone sequencer/testnet remain M2 |
...
| F7 | Native and custom LEZ tokens through ATAs | Parameterized guest and sequencer E2E for native token plus two independent token mints/accounts | Two custom definitions pass pinned token-program execution; actual-user native transfer is blocked on the v0.1.2 surface |
...
| U7 | SPEL IDL for LEZ escrow program(s) | IDL generation/validation test against pinned compatible SPEL/LEZ versions | Generated IDL/client and custom-token custody semantics pass; sequencer/native-user evidence remains M2 |
