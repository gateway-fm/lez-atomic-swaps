...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors and immutable profiles pass; a bounded dual-signed concrete agreement cross-binds both actor roles, common digest, exact LEZ chain/custody, BIP-199 output, transaction policy, actual LEZ deadline bound, and Zcash CLTV; the chain validator retains decoded raw bytes and a stable tip while binding network/branch/block/outpoint/value/scripts/depth; actual Zebra proves a conflicting four-over-three-block replacement and restart/requery; activation integration, cross-chain refund ordering, and both-direction testnet role E2E remain | Concrete agreement, ZEC consensus/deeper reorg, profiles, chain validation, watcher, schema-v4 binding/journal/alerts, core role semantics, and actual-node restart/requery are implemented; full SDK effects, testnet, and actor delivery remain M2 |
