...
| F4 | LEZ-transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors, profiles, agreement validation, canonical funding observations, and fresh maker eligibility pass. Separate role-fixed maker/taker SDKs and schema-v9 SQLite stores complete the fixed LEZ-reveal then Zcash-follow-up happy path in both directions and independently reopen at `Completed` (`add5d98`). The authenticated LEZ bridge client and production Zebra owner-claim port now pass isolated contract suites; actual Zebra fork/restart and local LEZ custody lifecycles still pass separately, not yet as one composed claim flow. | Deterministic SDK/SQLite happy claim plus production bridge-client/owner-claim adapter boundaries implemented; sidecar server, LEZ claim planning, counterparty-spend discovery, refunds, margin composition, process actors, and public-testnet evidence remain M2 work |
...
| ID | Contract | Acceptance evidence | Status / milestone |
|---|---|---|---|
| U1 | Dedicated full-lifecycle SDK for each pair | Shared SDK contract suite instantiated for BTC/XMR/ZEC; public API doctests cover discovery through refund | Partial: ZEC role-fixed discovery, negotiation, separate persistence, both lock effects/observations, and agreement-directed claims to `Completed` are integrated in both directions. Schema v9 encrypts preimages/exact submissions and replays separate owner/observer claim journals; production node adapters, refund lifecycle, fault-hardening, and actor-process E2E remain M2. |
