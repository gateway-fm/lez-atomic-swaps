...
| U2 | Long-running autonomous maker daemon plus systemd unit/install guide | Packaged daemon runs under hardened systemd in an isolated VM, restarts, advertises, prices, and executes without GUI | Partial M5: hardened `Type=notify` unit, encrypted credential paths, staged install verification, real user-systemd crash restart, typed health, SIGTERM cleanup, process-lifetime database lease, and bounded future-Core lifecycle contract are GREEN. Autonomous multi-pair execution and an immutable live Logos Core API remain |
