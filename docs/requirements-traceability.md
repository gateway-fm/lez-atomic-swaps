...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact redeem/P2SH/scriptSig/V5 vectors, immutable network profiles, and a typed canonical observation validator pass; the validator re-decodes bytes and binds network/branch/block/outpoint/value/scripts/depth before core projection; pinned Zebra proves actor transactions and a conflicting four-over-three-block replacement; stable actual-RPC observation persistence, cross-chain refund ordering, and both-direction testnet role E2E remain | ZEC consensus/deeper reorg, profile, and observation validation implemented; watcher/margin composition and actor delivery remain M2 |
