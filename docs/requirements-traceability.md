...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Direction-aware exact redeem/P2SH/scriptSig/V5 vectors and immutable local/public profiles pass; profiles bind network/NU6.2/depths/horizons, require calibrated bounds, and preserve LEZ-before-ZEC in both directions; pinned Zebra proves actor transactions and a conflicting four-over-three-block replacement; typed canonical observation, actual cross-chain refund ordering, and both-direction testnet role E2E remain | ZEC consensus/deeper reorg and profile enforcement implemented; margin composition and actor delivery remain M2 |
