...
| S6 | README covers deployment, addresses, prerequisites, and maker/taker CLI/mini-app use | Fresh-machine documentation tests plus link/command validation | Partial M6: prototype review, service configuration, local resources, flakiness, prepared acceptance, locked monitoring, and actual-node service Claim/Refund replay are documented. Basecamp package build/load, production assets, actor-real GUI use, and final link/command validation remain |
