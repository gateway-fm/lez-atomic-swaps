...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors and immutable profiles pass; the validator retains decoded raw bytes and a stable tip while binding network/branch/block/outpoint/value/scripts/depth; actual Zebra populates it and proves a conflicting four-over-three-block replacement; the two-phase watcher emits only stable canonical, affirmative detach, or atomic replacement evidence and never treats RPC absence as removal; primitive v1 records and the atomic SQLite journal retain/revalidate evidence; participant-aware core tests cover taker-funded and reverse maker-funded ZEC removal/reappearance, claim suspension, conflict rejection, and refunds; runtime atomically journals post-dependent conflicting replacements while retaining the committed ID/reorg phase; immutable per-swap binding, cross-chain refund ordering, and both-direction testnet role E2E remain | ZEC consensus/deeper reorg, profile, validator, actual snapshot, watcher, record, journal, core role semantics, and initial runtime conflict composition implemented; full actor delivery remains M2 |
