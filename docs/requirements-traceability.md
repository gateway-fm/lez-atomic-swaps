...
| R4 | Persisted state survives crash/restart without fund loss | SQLite reopen and actual daemon kill/restart pass; schema-v2 ZEC journal proves migration, future-version rejection, atomic rollback, stale revisions, role isolation, and revalidated loading; initial both-direction runtime tests probe exact unknown-outcome replay before core mutation and reload after close/open; historical tracker replay, terminal alerts, kill-at-every-effect-transition, corruption, encryption, and general outbox remain | Partial |
