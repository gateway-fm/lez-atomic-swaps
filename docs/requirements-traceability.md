...
| U5 | Basecamp maker mini-app configures and monitors | Signed-off prototype first, then actor-real UI E2E through an allowlisted process backend to the existing owner Unix RPC; clean local build/load from the documented repo | M6 active under ADR 0128 |
