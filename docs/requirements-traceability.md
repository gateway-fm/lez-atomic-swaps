...
| U2 | Long-running autonomous maker daemon plus systemd unit/install guide | Packaged daemon runs under hardened systemd in an isolated VM, restarts, advertises, prices, and executes without GUI | Partial M5: hardened `Type=notify` unit, encrypted credentials, installed real ZEC actor, pinned authority/root/program/digest inputs, staged verification, and actual nine-second user-systemd readiness/SIGKILL-restart/SIGTERM-cleanup are GREEN. One bounded supervisor cycle plus prompt process-group cancellation are component-GREEN; long-running daemon/systemd wiring, abandoned-lease recovery, multi-pair composition, and an immutable live Logos Core API remain |
