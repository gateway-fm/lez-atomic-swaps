...
| U3 | Maker CLI configures pairs/prices, controls daemon, queries history, and triggers claim/refund over IPC/RPC | Black-box CLI-to-daemon command matrix under owner/wrong-owner roles plus fixed-system-service contract | Partial M5: configure/price/offers/create/status/history/monitor and supported generation-fenced actions pass through the real daemon/CLI with replay and restart evidence. Literal start/stop controls only the packaged system unit with bounded exact-state verification. XMR effect actions are not daemon-owned; complete the supported-pair lifecycle matrix |
