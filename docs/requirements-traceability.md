...
| U4 | Taker CLI covers discovery, initiation, monitoring, claim, and refund | Actual taker CLI drives each pair's happy and abandoned-counterparty journey | Partial M5: key-pinned discovery and ZEC initiation through exact maker-proposal validation, local countersigning, atomic Chat completion, deterministic replay, and no-clobber agreement persistence are process-GREEN. Accepted ZEC swaps now no-clobber provision a role-fixed Taker bundle before completion and publish a digest-pinned receipt only after durable Maker completion. Receipt-bound monitor/claim/refund, exact artifact replay, tamper rejection, transport-free monitor, and Delivery-independent persisted completion replay are process-GREEN. Actual-node Taker effects, BTC, XMR, and abandonment remain |
