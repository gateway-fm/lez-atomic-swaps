...
| R4 | Persisted state survives crash/restart without fund loss | Schema-v7 SQLite retains agreement, canonical history, separate taker/maker intents, exact transitions, revision, bindings, and alerts. Twelve SDK-recovery tests cover role isolation, rollback, torn history, stale catch-up, exact replay, both-direction close/reopen, an observation revision intervening between maker stage and close, and atomic maker-write rollback; union replay rejects holes and duplicate slots. Actual daemon/two-Zebra restart also passes. Maker submission/reorg/corruption faults, later-effect kill points, encryption, and the M5 outbox remain. | Partial |
