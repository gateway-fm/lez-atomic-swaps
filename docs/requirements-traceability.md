...
| ID | Contract | Acceptance evidence | Status / milestone |
|---|---|---|---|
| R1 | Taker locks first; maker waits for required confirmations | Core rejects early lock and revokes permission on confirmation regression for all pairs; ZEC watcher tests require stable positive evidence and affirmative changed-height removal proof, suppress exact replay, and reject stale/unproved replacement; role-aware persisted projection remains | Passing core and ZEC reconciliation boundary; composed adapter evidence M2–M4 |
