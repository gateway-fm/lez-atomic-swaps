...
|---|---|---|---|
| F1 | No central server; Delivery advertisements and Chat coordination | Role E2E discovers and negotiates through real Delivery/Chat adapters; central services absent; UJ-006 repeats with both stopped after lock | Partial M5: signed bounded daemon-to-taker run-local Delivery publication/discovery plus restart reconciliation and durable expiring offers, one-winner reservation, and atomic coordinator consumption are component GREEN; maker-first ZEC proposal/countersigning, bounded unsigned draft handoff, and one-winner durable proposal staging are GREEN; the disjoint Chat process now authenticates, signs, stages before response, countersigns as the taker role, atomically accepts with daemon-local protected authority, isolates methods, and survives kill/reopen. The actual taker command durably persists the countersigned wire before completion, validates its private agreement/draft and both roles on rerun, and retries only exact completion. The daemon holds startup-pinned Maker authority, durably publishes one Maker-only actor bundle, atomically registers its schema-v16 manifest, and preflights the exact committed request/negotiation/actor before any current-wall-clock parse or provisioning. The real daemon/taker journey proves that retry after a three-second TTL returns the original result; changed inputs or missing actor fail closed, and a stale consumed Delivery projection reports ready plus degraded/unavailable until reconciliation. Supervisor execution, per-swap authority for distinct/concurrent application swaps, actor-bearing systemd, actual nodes, and other-pair composition remain |
