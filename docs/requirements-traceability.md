...
| F4 | LEZ-transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors, profiles, agreement validation, canonical funding observations, and fresh maker eligibility pass. Separate role-fixed maker/taker SDKs and schema-v9 SQLite stores complete the fixed LEZ-reveal then Zcash-follow-up happy path in both directions and independently reopen at `Completed` (`add5d98`). Actual Zebra fork/restart and local LEZ custody lifecycles pass separately, not yet as one composed claim flow. | Deterministic SDK/SQLite happy claim implemented; canonical production maker-lock/claim adapters, claim persistence fault/corruption gates, refunds, margin composition, process actors, and public-testnet evidence remain M2 work |
