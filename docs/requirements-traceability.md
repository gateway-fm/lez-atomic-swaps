...
|---|---|---|---|
| F1 | No central server; Delivery advertisements and Chat coordination | Role E2E discovers and negotiates through real Delivery/Chat adapters; central services absent; UJ-006 repeats with both stopped after lock | Partial M5: signed bounded run-local Delivery discovery plus durable expiring offers, one-winner reservation, and atomic coordinator consumption are component GREEN; maker-first ZEC proposal/countersigning is SDK GREEN; durable Chat runtime, process wiring, and post-lock removal remain |
