...
| U6 | Basecamp taker mini-app browses and executes swaps | Prototype and isolated browser proof are GREEN. The seven-method DTO contract, authenticated backend, owner-only process, replay-first registry, and prepared ZEC authority are GREEN. Commit `5536dd0` service-connects admission-first real Maker Chat acceptance, countersigned agreement and role-fixed actor provisioning, returns `NotActivated`, rejects durable authority drift, and replays a completed receipt with Delivery and Chat offline. It deliberately performs no chain effect. Swap-list, monitor, actor driving, claim, refund, standalone-service real-Chat E2E, and actor-real QML remain | M6 active under ADRs 0128 and 0130 through 0135; owner signoff still gates QML/QtRO |
...
| R8 | Delivery/Chat outage is retried/buffered/degraded and documented | Pre-lock outage/recovery matrix plus post-lock UJ-006 proves on-chain independence and user-visible degraded state | M5 local output GREEN: real Maker Delivery and Chat failure/recovery, response-loss replay, restart, and three post-first-lock actual-node transport removals remain proven. M6 `5536dd0` additionally proves that after service-connected Maker completion and Taker receipt publication, an exact restart replay succeeds with the Delivery offer removed and Chat unavailable while agreement, actor config, and receipt bytes and inodes remain unchanged. LOGOS-020 remains upstream production parity |
...
| S6 | README covers deployment, addresses, prerequisites, and maker/taker CLI/mini-app use | Fresh-machine documentation tests plus link/command validation | Partial M6: prototype review, conditional service configuration, owner socket, authenticated reads, admission-first real ZEC Chat acceptance, exact private-authority replay, completed offline receipt replay, local resources, flakiness, and no-chain-effect limitations are documented. Basecamp package build/load, production assets, actor-real GUI use, chain lifecycle flows, standalone-service real-Chat proof, and final link/command validation remain |
| S7 | Write-up covers protocols, escrow, atomicity, timelocks, assumptions, limitations | M1 design packet reviewed, then updated from executable evidence and audit findings | Living architecture contains pair component and sequence diagrams plus conditional atomicity arguments. ADR 0135 adds the M6 service-connected ZEC fresh/restart flows and explains admission transaction, Maker atomic completion, Taker no-clobber publication, receipt ordering, and response-loss convergence. It explicitly classifies this as pre-effect acceptance atomicity rather than a distributed cross-chain commit because neither actor starts and no Zebra or LEZ RPC occurs. M7 retains independent formal review |
