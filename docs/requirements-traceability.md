...
| ID | Contract | Acceptance evidence | Status / milestone |
|---|---|---|---|
| R1 | Taker locks first; maker waits for required confirmations | Core rejects early lock and revokes permission on either participant's canonical funding removal; reverse ZEC proves maker-funded ZEC removal suspends claims, pins the exact ID, restores only exact reappearance, rejects conflict, and preserves refunds; watcher tests require stable affirmative evidence; committed-event runtime wiring remains | Passing core and ZEC reconciliation/persistence boundaries; composed adapter evidence M2–M4 |
...
| R4 | Persisted state survives crash/restart without fund loss | SQLite reopen and actual daemon kill/restart pass; schema-v2 ZEC journal tests additionally prove legacy migration, future-version rejection, atomic event-plus-aggregate rollback, stale revisions, unknown-outcome replay, role isolation, and revalidated restart loading; kill-at-every-effect-transition, corruption, encryption, and general outbox matrix remain | Partial |
