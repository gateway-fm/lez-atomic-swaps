...
| U4 | Taker CLI covers discovery, initiation, monitoring, claim, and refund | Actual taker CLI drives each pair’s happy and abandoned-counterparty journey | Partial M7: ZEC discovery/initiation/receipt-bound monitor/claim/refund, BTC discovery/initiation/offline monitor, and XMR discovery/initiation/receipt-only monitoring plus the receipt-v2 Tag14 marker claim are process-GREEN; clean actual-node BTC and composed XMR application corridors exist. ADRs 0152/0153 give the XMR sender and observer exact validated application bytes on sealed FDs 211..216 plus a canonical mode/step/identity/ABI/journal/evidence/RPC plan on FD 217. The role-correct XMR refund effect is composed in the application runner but not yet exposed semantically through the literal Taker lifecycle CLI. The literal receipt-v2 claim still constructs no semantic transaction and proves no actual chain evidence; semantic journal transitions and branch-produced signature/share/evidence custody remain. XMR monitor validates pre-effect application authority under the per-swap lock and explicitly does not infer chain progress. Abandonment journeys remain hardening unless required to expose a PoC defect |
