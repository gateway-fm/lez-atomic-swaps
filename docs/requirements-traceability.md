...
autonomous multi-worker composition, and unavailable-route behavior remain.
...
| U2 | Long-running autonomous maker daemon plus systemd unit/install guide | Packaged daemon runs under hardened systemd in an isolated VM, restarts, advertises, prices, and executes without GUI | Partial M5: hardened systemd lifecycle, exact identity, staged verification, crash recovery, bounded supervision, and sequential two-row failure isolation are GREEN. The daemon still starts one supervisor runtime; simultaneous multi-worker application execution and actual-chain actor crash recovery remain. Immutable live Logos Core integration stays an upstream production-parity exception |
| U3 | Maker CLI configures pairs/prices, controls daemon, queries history, and triggers claim/refund over IPC/RPC | Black-box CLI-to-daemon command matrix under owner/wrong-owner roles | Partial M5: configure/price/offers/create/status/history/monitor and supported generation-fenced actions pass through the real daemon/CLI with replay and restart evidence. The CLI has no literal start/stop subcommands, and XMR effect actions are not daemon-owned. Add black-box service control plus the supported-pair lifecycle matrix |
| U4 | Taker CLI covers discovery, initiation, monitoring, claim, and refund | Actual taker CLI drives each pair's happy and abandoned-counterparty journey | Partial M5: ZEC discovery/initiation/receipt-bound monitor/claim/refund and BTC discovery/initiation/offline monitor are process-GREEN; clean actual-node BTC and composed XMR application corridors exist. The Taker lifecycle loader/dispatch supports only ZEC/BTC, so XMR monitor/claim/refund remain absent. Abandonment journeys remain hardening unless required to expose a PoC defect |
...
| R3 | Missing chain dependency does not disable other pairs | Dependency matrix starts daemon with each node absent/unhealthy and completes unaffected-pair swaps with clear CLI/GUI status | Partial M5: route configuration and publication persist per pair, but quote selection currently does not reject `configuration.enabled == false`. First close explicit pre-publication disable/re-enable with unaffected-pair progress; automatic live-node health detection remains a distinct hardening gate unless added to the PoC |
...
| R5 | Concurrent swaps have independent state, escrow, and deadlines | M3 clean run `m3overlap-20260717a` proves the underlying opposite-direction actor/chain boundary with distinct agreements, stores, escrows, deadlines, effects, replay, and cleanup. M5 has one real-daemon sequential two-row failure-isolation journey but no simultaneous application composition | Carried M3 chain-level evidence only; M5 application concurrency remains Partial until one database/daemon runs simultaneous workers over distinct accepted application agreements and proves overlap, terminal isolation, restart/replay, and scoped cleanup |
