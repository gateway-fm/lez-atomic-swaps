...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors and immutable profiles pass; a bounded dual-signed agreement binds actors, LEZ channel/genesis/custody, BIP-199 output, role-local transaction construction policy, actual LEZ deadline bound, and Zcash CLTV; concrete activation/resume, exact taker first-lock intent, primitive recovery-record revalidation, and production SQLite atomic projection/replay are integrated; the maker independently selects its signed-direction node port; forward Zcash rejects weak ID/depth assertions and durably journals canonical evidence, depth changes, atomic same-tip replacement, and affirmative exact-head removal through ZcashObservationTracker against the signed HTLC output binding; both deterministic-local directions have a distinct non-cached fresh eligibility call that replays and re-queries without authorizing a maker effect; reverse checks signed depth; the public Finalized/typed Pending/Safe policy is unit-tested but public activation remains fail-closed pending reviewed deployment; actual Zebra proves durable fork replacement/restart; reverse LEZ rejects weak assertions and revalidates stable transaction/block/metadata/exact-custody evidence; canonical, duplicate, same-inclusion update, nonfinal removal, and atomic same-tip replacement fold through the active SDK and SQLite exact-head tracker across reopen, including historical payload-v1 upgrade and stale-old-head rejection; official-wire LEZ decoding, maker second-lock consumption of fresh eligibility, later-effect persistence, cross-chain ordering, and both-direction actor E2E remain | Concrete agreement, taker submission recovery, maker-independent ordered forward-Zcash and stable reverse-LEZ observation durability, ZEC transaction/consensus/funding reconciliation, spend recognition, profiles, schema-v6 journals/alerts/recovery, core role semantics, and actual-node restart/requery are implemented; later effects, production ports, and actor delivery are remaining M2 work |
...
| R4 | Persisted state survives crash/restart without fund loss | SQLite reopen and actual daemon kill/restart pass; schema-v6 retains the ZEC journal/binding/alert guarantees and now has a cloneable role-fixed SDK recovery adapter; primitive DTOs and adapter loads reject future schemas, unknown/substituted fields, oversized bytes, invalid evidence, malformed JSON, corrupt closed intents, torn active revisions, closed intents missing transitions, orphan taker rows, same-cardinality maker holes, duplicate polls, stale inclusion removal, and history-incompatible appends; nine adapter tests prove exact and historical replay, same-ID role isolation, taker and maker immediate-transaction rollback, retained intent, stale-instance catch-up, and canonical/depth/replacement/removal close/reopen recovery; existing store tests prove atomic swap+binding and event+aggregate+alert rollback, immutable rebinding, stale revisions, role isolation, and revalidated loading; both-direction runtime tests probe unknown outcomes and exact historical replay; conflict/terminal alerts and authenticated CLI history survive restart; actual two-Zebra evidence proves canonical commit, two close/reopen cycles, fresh requery, affirmative fork removal, and no duplicate on retry; later-effect kill points, encryption, and the general M5 effect outbox remain | Partial |
