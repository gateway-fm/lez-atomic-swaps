...
refund-side actual-node absent-maker journeys. Live maker-lock admission,
survivor-specific behavior, two
...
| F2 | LEZ–BTC via BIP-340 adaptor signatures and Taproot key-path cooperative claim | Proposed GW-M3-001 evidence contract: official BIP-340/BIP-327 vectors, swap-specific adaptor fixtures, independent cross-check, tweak/parity-aware signatures under the P2TR output key, Core consensus, and both-direction actual-node happy/refund/concurrency role E2E. The nonexistent DLC `AdaptorSignature.md` is a nonblocking Gateway/Logos production-review erratum | Happy path is actual-node GREEN in both directions in `m3actor-20260716n`. Run `m3refund-20260716h` executes both ordered two-lock timeouts, and clean pushed-commit run `m3firstlock-20260716h` executes both absent-maker first-lock refunds through fresh-maker terminal reconstruction and zero replay resubmission. Live maker-lock admission at the cutoff boundary, direct post-reveal survivor continuation, two overlapping swaps, the full-lifecycle public BTC SDK, D1 artifacts, process-kill/reorg, production authority, Testnet4, formal review, and GW-M3-001 replacement acceptance remain open |
...
| F6 | Atomic outcome: both claim or both refund | Six scenario tests and 512 generated transition sequences pass. M3 run `m3actor-20260716n` retained exact funding/agreement/signer ordering, both canonical locks before reveal, actor-owned claims through revision four, and unchanged effect counts after replay. Run `m3refund-20260716h` executed both ordered two-lock refunds. Run `m3firstlock-20260716h` then proved both absent-maker directions: only the taker first lock existed, two fresh cross-chain reads admitted its exact refund after the signed boundary, both roles reconstructed revision 2 `Refunded`, and replay added zero effects. This proves conditional protocol recovery, not one distributed commit across Core, LEZ, and SQLite. Live maker-lock admission at the cutoff boundary, direct post-reveal survivor execution, reorg, process-kill, and overlapping concurrency remain | Partial |
...
| ID | Contract | Acceptance evidence | Status / milestone |
|---|---|---|---|
| U1 | Dedicated full-lifecycle SDK for each pair | Shared SDK contract suite instantiated for BTC/XMR/ZEC; public API doctests cover discovery through refund | Partial: ZEC role-fixed discovery, negotiation, separate schema-v10 persistence, both lock effects/observations, agreement-directed claims to `Completed`, and ordered refunds to `Refunded` are integrated in both directions. The same public SDK boundary now drives independent maker/taker processes through both private local actual-node happy paths. Signed public deployment activation and typed local/self-hosted/provider routes reuse that SDK and actor binary; their local nonconnecting contract suites pass. Protected claim/refund replay remains proven in lower lanes; composed actual-node refund/restart faults remain owner-gated. The BTC reference actor and lower typed components do not yet constitute the accepted full-lifecycle public BTC SDK with discovery, negotiation, SDK-owned escrow creation, public errors, examples, and lifecycle documentation. Current actual-node scripts submit both locks outside the actor/SDK. |
...
| R2 | After first submission, only local state and chain nodes are required | Core happy/refund paths have no peer handle. Runs `m3actor-20260716n`, `m3refund-20260716h`, and `m3firstlock-20260716h` reconstructed claims, ordered refunds, or the sole funded-leg refund from countersigned agreement, role-local journals, and Core/LEZ observations. In the first-lock run the maker stayed offline after activation; a fresh maker later reconstructed first lock and refund from chain state. No Delivery or Chat, public RPC, faucet, or public funds participated | Partial: actual-node happy, both-lock refund, and absent-maker first-lock on-chain-only continuation are proven. Direct post-reveal survivor continuation, outage behavior, and process-kill evidence remain |
...
| S5 | Complete reference integration for every chain | UJ-001/UJ-002/UJ-004 role E2E and runnable reference package for BTC, XMR, and ZEC | Partial: ZEC and BTC each have private local happy-path references in both directions. BTC packets `m3actor-20260716n`, `m3refund-20260716h`, and `m3firstlock-20260716h` cover happy, ordered two-lock refunds, and both absent-maker first-lock refunds with exact replay/cleanup. Live maker-lock admission at the cutoff boundary, direct survivor continuation, two overlapping swaps, the full public BTC SDK surface, process-kill/reorg, and XMR M4 work remain |
