...
| U5 | Basecamp maker mini-app configures and monitors | Signed-off prototype first, then actor-real UI E2E through an allowlisted process backend to the existing owner Unix RPC; the atomic schema-v22 local-route save is GREEN at `8c6a7db`; clean local build/load remains | M6 active under ADRs 0128 and 0129 |
| U6 | Basecamp taker mini-app browses and executes swaps | The prototype journey and isolated browser proof are GREEN. The exact seven-method DTO contract, authenticated backend, owner-only process, same-FD custody, replay-first registry, and prepared-ZEC authority are GREEN. Commit `1664c41` service-wires admission: configured health/list/initiate are real, exact restart replay survives Delivery removal, and new admission requires prepared plus currently authenticated public facts before one atomic registry commit. The process-proven ZEC Chat acceptance and actor-provisioning path is reusable at `0afb6da` but is not service-connected. The returned state is only `Initiating` generation zero; swap-list, monitor, claim, refund, the real ZEC execution worker, and actor-real QML E2E remain | M6 active under ADRs 0128 and 0130 through 0134; owner sign-off still gates QML/QtRO |
...
| S6 | README covers deployment, addresses, prerequisites, and maker/taker CLI/mini-app use | Fresh-machine documentation tests plus link/command validation | Partial M6: prototype review, conditional Taker service config/start/health/list/initiate, exact process-restart replay, registry and prepared-authority proofs, local resources, and admission-only limitations are documented. Basecamp package build/load, downloadable production assets, actor-real Maker/Taker GUI use, real ZEC execution/lifecycle flows, and final link/command validation remain |
| S7 | Write-up covers protocols, escrow, atomicity, timelocks, assumptions, limitations | M1 design packet reviewed, then updated from executable evidence and audit findings | Living architecture contains dedicated Mermaid components and sequences for both BTC directions, both transparent-ZEC directions, and the supported LEZ-first XMR successful and recovery branches. ADR 0122 adds the full local-node/RPC topology, role-correct tag-16-to-Monero sequence, and exact conditional atomicity argument for the reproducible refund runner. ADR 0123 records the observed liveness RED and exact pushed replay `m5xmrrefund45924caa`: one terms-sealed clock effect, authoritative finalized tag 16, Maker extraction and recovery sweep, conditional binding, and exact cleanup. It explains why neither the tick nor binder is a distributed commit or future-reorg guarantee and why punishment and hardening remain. Stable markers remain CI-guarded; M7 retains independent formal review |
...
| S11 | Figma designs or equivalent for both mini-apps | Clickable HTML prototypes and isolated 6/6 actor-browser rehearsal exist for both roles; the review record must contain an explicit owner decision before production QML is accepted | M6 active under ADR 0128; owner sign-off remains pending |
