...
| U5 | Basecamp maker mini-app configures and monitors | Signed-off prototype first, then actor-real UI E2E through an allowlisted process backend to the existing owner Unix RPC; the atomic schema-v22 local-route save is GREEN at `8c6a7db`; locked tutorial package/LGX/build/install is GREEN, while the Maker package and Basecamp load remain | M6 active under ADRs 0128 and 0129 |
| U6 | Basecamp taker mini-app browses and executes swaps | Prototype/browser, seven-method DTO, owner service, prepared acceptance, receipt-bound reads, and terminal authorization are GREEN. Fresh certificates `m6claim0ba41aba` and `m6refund8f76d87a` prove service-driven actual-local-node Claim and Refund with finalized LEZ effects, exact canonical Zcash effects, and no-effect replay. The actor-real Basecamp journey remains | M6 active under ADRs 0128 and 0130 through 0145; Maker/Taker `ui_qml`, process-isolated backends, actor-real UI E2E, prototype signoff, and actual Basecamp load remain pending |
...
| S6 | README covers deployment, addresses, prerequisites, and maker/taker CLI/mini-app use | Fresh-machine documentation tests plus link/command validation | Partial M6: prototype review, service configuration, local resources, flakiness, prepared acceptance, locked monitoring, and actual-node service Claim/Refund replay are documented. Tutorial package/LGX/install is GREEN; Maker/Taker package build and Basecamp load, production assets, actor-real GUI use, and final link/command validation remain |
| S7 | Write-up covers protocols, escrow, atomicity, timelocks, assumptions, limitations | M1 design packet reviewed, then updated from executable evidence and audit findings | Living architecture contains pair component and sequence diagrams plus conditional atomicity arguments. ADRs 0137 through 0145 record authorization-before-effect, one terminal branch, lock scope, finalized Refund ordering, and exact Claim/Refund replay. They explicitly do not claim a distributed cross-chain transaction. Actor-real UI composition remains for M6; durable rollback fencing is deferred production hardening and M7 retains independent review |
