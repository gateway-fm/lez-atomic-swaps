...
| U6 | Basecamp taker mini-app browses and executes swaps | The prototype journey and isolated browser proof are GREEN. The exact seven-method DTO contract, authenticated health/list backend, owner-only read process, replay-first standalone ZEC initiation registry, and same-FD private-file custody are component-GREEN through `0ef38b0`. Only health and offer list are registered; the registry is not service-wired, initiate/list-swaps/monitor/claim/refund are method-not-found, and actor-real QML E2E remains | M6 active under ADRs 0128, 0130, 0131, and 0132; owner sign-off still gates QML/QtRO |
...
| S6 | README covers deployment, addresses, prerequisites, and maker/taker CLI/mini-app use | Fresh-machine documentation tests plus link/command validation | Partial M6: prototype review and actual read-only Taker service config/start/health/list/manual isolation are documented. Basecamp package build/load, downloadable production assets, actor-real Maker/Taker GUI use, mutation flows, and final link/command validation remain |
