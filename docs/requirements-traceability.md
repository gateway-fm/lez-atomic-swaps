...
| R4 | Persisted state survives crash/restart without fund loss | Schema-v7 SQLite retains agreement, canonical observation history, separate taker and maker intents, exact confirmed transitions, aggregate revision, bindings, and alerts. Twelve SDK-recovery tests cover role isolation, rollback, corruption/torn history, stale catch-up, exact replay, both-direction close/reopen, and atomic rollback of maker transition/revision/intent closure; union replay rejects holes and duplicate predecessor slots. Actual daemon and two-Zebra restart evidence also passes. Maker submission/reorg/corruption faults, later-effect kill points, encryption, and the M5 outbox remain. | Partial |
