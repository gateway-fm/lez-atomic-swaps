...
|---|---|---|---|
| F1 | No central server; Delivery advertisements and Chat coordination | Role E2E discovers and negotiates through real Delivery/Chat adapters; central services absent; UJ-006 repeats with both stopped after lock | M7 automatic-control component GREEN: the real daemon periodically runs bounded hash-pinned semantic commands off its async RPC loop; unavailable Zcash withdraws only active advertisements, rejects new quote/publication, preserves a reserved negotiation, and leaves Bitcoin offer/quote state available. Existing ZEC, BTC, and XMR paths retain signed Delivery, durable Chat/application state, role-only authority and Delivery-free post-lock replay. One actual unaffected-pair chain swap while another real local node is absent remains |
...
| R3 | Missing chain dependency does not disable other pairs | Dependency matrix starts daemon with each node absent/unhealthy and completes unaffected-pair swaps with clear CLI/GUI status | M7 automatic-control component GREEN under ADR 0150: strict configuration can require LEZ plus foreign semantic commands for each exact route; executable identity, exit, timeout, and mutation fail closed. The real daemon withdraws an active unhealthy Zcash offer without a health request; component evidence preserves a reserved Zcash negotiation, reports route degradation, rejects only new Zcash work, and leaves an independent Bitcoin offer and quote available. CAS revisions make reservation versus withdrawal one-winner and never revoke accepted terms. The remaining literal R3 gate is an actual unaffected-pair chain swap while another real local node is absent |
