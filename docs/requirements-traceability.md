...
| S6 | README covers deployment, addresses, prerequisites, and maker/taker CLI/mini-app use | Fresh-machine documentation tests plus link/command validation | M6 documentation now covers prototype review, package/LGX/installer builds, pinned Basecamp install, separate role user directories and sockets, real Maker/Taker UI steps, external build/runtime resources, flakiness, exact cleanup, prepared acceptance/replay/monitor, and layered actual-node Claim/Refund. Final link/command gate remains before tag |
| S7 | Write-up covers protocols, escrow, atomicity, timelocks, assumptions, limitations | M1 design packet reviewed, then updated from executable evidence and audit findings | Living architecture includes deployed Basecamp/QtRO/owner-service/local-node components plus Maker, Taker, BTC, XMR, and ZEC sequences and conditional atomicity arguments in ADR 0147. It explicitly separates local database atomicity, protocol conditional atomicity, and layered chain evidence; durable rollback fencing and independent M7 review remain production hardening |
