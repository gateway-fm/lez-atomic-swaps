...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors and immutable profiles pass; the validator retains decoded raw bytes and a stable tip while binding network/branch/block/outpoint/value/scripts/depth; actual Zebra populates it and proves a conflicting four-over-three-block replacement; the two-phase watcher emits only stable canonical, affirmative detach, or atomic replacement evidence and never treats RPC absence as removal; primitive observation and immutable profile/output binding records are revalidated and persisted; participant-aware core tests cover both ZEC roles; runtime atomically journals post-dependent conflicting replacements while retaining the committed ID/reorg phase; runtime binding enforcement, cross-chain refund ordering, and both-direction testnet role E2E remain | ZEC consensus/deeper reorg, profile, validator, actual snapshot, watcher, records, schema-v3 binding/journal, core role semantics, and initial runtime conflict composition implemented; full actor delivery remains M2 |
...
| R4 | Persisted state survives crash/restart without fund loss | SQLite reopen and actual daemon kill/restart pass; schema-v3 ZEC journal/binding storage proves migration without inferred terms, future-version rejection, atomic swap+binding and event+aggregate rollback, immutable rebinding, stale revisions, role isolation, and revalidated loading; both-direction runtime tests probe unknown-outcome replay before mutation, replay ordered records into the exact historical tracker head, reject impossible sequence history, require every new event to advance that head before projection, and require fresh reconciliation after close/open; terminal removal/replacement retains the absorbing outcome and returns an explicit durable classification; alert delivery, kill-at-every-effect-transition, corruption, encryption, and general outbox remain | Partial |
