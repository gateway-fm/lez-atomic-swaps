...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Direction-aware LEZ-recipient/preimage and exact redeem/P2SH/scriptSig/V5 vectors pass; pinned Zebra accepts actor-funded claim/refund, concurrent independent swaps, invalidation/rebroadcast/reconsideration, and a two-node conflicting four-over-three-block canonical replacement; typed observation/profile binding, actual cross-chain refund ordering, and both-direction testnet role E2E remain | ZEC consensus transaction and deeper-reorg lifecycle implemented; margin composition and actor delivery remain M2 |
