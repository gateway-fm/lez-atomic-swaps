...
| F7 | Native and custom LEZ tokens through ATAs | Parameterized guest and sequencer E2E for native token plus two independent token mints/accounts | Source audit invalidated direct native/token holding proof; canonical authenticated-transfer and official ATA RED-GREEN replacement is in progress |
