...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors and immutable profiles pass; a bounded dual-signed agreement binds actors, LEZ custody, BIP-199 output, transaction policy, actual LEZ deadline bound, and Zcash CLTV; concrete activation/resume and exact first-lock intent staging are integrated; funding observation is durable and actual Zebra proves fork replacement/restart; bounded spend recognition accepts every pinned-consensus sighash/high-S/nonminimal form without hiding the preimage and reports SDK policy separately; atomic active evidence projection, durable spend reorgs, cross-chain ordering, and both-direction testnet actor E2E remain | Concrete agreement, activation/resume, observe-before-rebroadcast first-lock seam, ZEC transaction/consensus/funding reconciliation, spend recognition, profiles, schema-v4 funding journal/alerts, core role semantics, and actual-node restart/requery are implemented; full SDK effects, durable active projection, testnet, and actor delivery remain M2 |
