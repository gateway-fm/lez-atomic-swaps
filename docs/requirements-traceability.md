...
|---|---|---|---|
| F1 | No central server; Delivery advertisements and Chat coordination | Role E2E discovers and negotiates through real Delivery/Chat adapters; central services absent; UJ-006 repeats with both stopped after lock | M7 GREEN under ADR 0150. Clean pushed run `m7outage-2c63218-a` provisioned and authenticated Bitcoin Core 31.1 Regtest, stopped its exact labelled container, reported only that route unavailable, rejected its quote, and completed the ordinary signed Delivery/Chat Zcash Maker/Taker application through actual local Zebra and LEZ nodes in 36.920 seconds. Maker restart preserved both route states; accepted role actors continued after transport removal to completed journals. The observed order was confirmed Zcash funding, revealing LEZ claim, then confirmed Zcash claim. The checked certificate is `docs/evidence/m7-unaffected-pair-outage-2c63218-20260804.json`; no public RPC, faucet, public funds, peer, central coordinator, or runtime external resource participated |
...
| F3 | LEZ–XMR via Ed25519 adaptor signatures, cross-curve DLEQ, and spend-key share | LEZ-first COMIT/DLEQ vectors; `monerod`/wallet RPC stagenet happy/refund/concurrency role E2E; spend-key recovery fault case; XMR-first rejected | Partial across milestones: M4 clean-certified the local successful-claim protocol, and M5 run `m5-xmr-app-20260730-2c6aec1-h` clean-certified its real Maker/daemon/Taker application composition through finalized Claim, extraction, reconstructed-key sweep, binding, and exact cleanup. Exact run `m5xmrrefund45924caa` finalized tag 16 and a ten-confirmation Maker recovery sweep with conditional binding and exact cleanup. ADR 0154 adds a real sealed receipt-v2 Tag16 child that requires the live durable presignature to equal Stage B and adapts it with least-privilege FD 218. The literal CLI now performs prepare-only preflight before CAS, then one authenticated prepare/complete/submit invocation; rejected preflight leaves the CAS retryable, and restart reconciliation never re-sends. ADR 0155 prepares the existing safe Tag14 release service for supervisor composition through sealed FDs 220..222 and stable directory FD 223 while preserving its finalized-Fund plus confirmed-Monero-output gate, post-CAS clock recheck, and no-ambiguous-retry rule. ADR 0158 and checked run `m7tag17a23a314a` make Tag17 component and actual-node execution GREEN: exact claimant-signed bytes were durable before exposure, released once under transaction identity, then classified identically by Maker and Taker at finalized height 124 with zero custody. ADR 0163 now joins schema-3 Maker Tag17 to the normal supervisor with safe inherited-lock custody, one send, restart observation, and terminal operator-action completion. ADR 0164 removes hard-coded recovery selection: the validated durable Refund branch invokes the Maker Monero sweep route once with FD 218 and observes without it, while Punish retains Tag17 preflight/invoke/observe; Claim or no branch fails closed. The real supervisor/actor test proves both process routes without RPC; the semantic Maker refund sender/observer is GREEN under ADRs 0165 through 0167; ADR 0168 adds the no-branch, finalized-evidence activation gate. Joined run `m7abandon-a742c9f-a` binds the funded Stage-A Monero output to terminal Tag17, and exact run `m7lose16-930e3b4-a` injects the valid late Tag16 once and proves its Refund effect absent through finalized height 175 while Tag17 facts remain unchanged. Exact run `m7lose17-63a9496-b` makes ADR 0176 actual-node GREEN: Tag16 finalized Refunded/0, one transport-accepted late Tag17 produced no Punish effect through height 226, and the bounded retried observation retained identical Tag16 facts; the checked certificate and exact cleanup passed. ADR 0189 pins exact run `m7claim-2cff48d-a`, where owner-exact Tag14 and finalized Tag15 enabled the ten-confirmation Taker Monero claim sweep. Accepted-application concurrency and fault recovery, Stagenet trust, and production hardening remain |
| F4 | LEZ-transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors, profiles, agreement validation, canonical funding observations, and fresh maker eligibility pass. Separate role-fixed maker/taker SDKs and schema-v10 SQLite stores complete and reopen the direction-fixed claim or refund lifecycles. The authenticated eight-method client/server protocol, official native/revealing-claim/refund preparation and observation, executable role-isolated sidecars, main escrow/claim/refund validation adapters, role-correct both-direction Zcash funding submission and unknown-ID discovery, and production Zebra claim/refund boundaries pass isolated suites. The context-owning LEZ SDK ports persist caller-owned IDs/windows and share one role-local journal. The fresh-client factory rereads a private capability without retaining it, and the actor context source generates fresh OS-random IDs with only protocol-required bounded windows. Reopened SQLite now derives the exact LEZ claim funding in both directions without primitive caller IDs; reverse direction additionally refuses until the durable Zcash second lock exists. The zeroizing production Zcash signer produces byte-identical canonical funding/claim/refund transactions and rejects foreign roles or keys. The agreement-committed exact-outpoint planner validates every candidate and stable Zebra identity before producing the only durable funding plan. A one-shot maker/taker CLI and private-config boundary now proves separate roles, runs, stores, journals, capabilities, keys, and config files without path aliasing. The local v0.2/Zebra corridor now completes both claim directions through independent role processes: run 14o finalized LEZ blocks 264/265/266 and Zcash heights 106/108; reverse 14c finalized LEZ blocks 641/642/643 and Zcash heights 113/115. In both runs Zcash funding had two confirmations before the LEZ reveal, the exact `:0` HTLC output was spent afterward, and both stores reached revision 4 `Completed`. Signed public LEZ identities, schema-v3 Zebra routes, exact official-public sidecar routing, and pre-persistence runtime checks pass local nonconnecting contract suites with the same binaries and validators. Current-schema runs `m2cert-schema3-forward-2d09997-20260714a` and `m2cert-schema3-reverse-2d09997-20260714a` repeat both role/effect orderings through actual local nodes. Canonical Docker-target runs `m2cert-canonical-forward-bb53daf-20260714a` and `m2cert-canonical-reverse-bb53daf-20260714a` then bind that same order to ProgramId `5cf8...29c1`, finalized LEZ blocks 2594/2595/2596 and 2605/2606/2607, Zcash heights 122/124 and 125/127, zero terminal LEZ custody, and four revision-4 `Completed` actor stores. | M7 local-functional GREEN under ADR 0203: the private actual-node happy-claim and configuration-portability gates are GREEN in both directions, including direction-correct funding and LEZ-before-Zcash reveal order. Lower lanes retain ordered refund, restart, and fault evidence, but their composed actual-node repetition and the remaining post-lock/margin matrix are owner-gated hardening. The M2 local-functional PoC is certified under `m2-complete`; reorganization and timing stress remain tracked by R1 and R6, and public-testnet execution is deferred to production readiness under ADR 0023 |
| F5 | Risc0 LEZ escrow validates pair proof and supports claim/refund | Per-pair guest instruction tests plus standalone-sequencer initialise/claim/refund E2E | M7 local-functional GREEN. Historical M2/M3 checked and deployed artifacts remain certified. The privately deployed historical M4 v0.2 identity is ELF `dc370bc3...b7292` / ImageID `4d659033...12c82`; tags 13–15 and the later clean refund replay used that deployed program. Exact pushed run `m7tag17a23a314a` freshly built and deployed the advisory-remediated current v0.2 guest at ELF `ade4af84...bbcee` / ImageID `b7f87278...b0433`; all five recursive cases passed. The deployment occurred exactly once and reached Bedrock `Finalized`. After finalized tag 13 and a pre-boundary negative observation, one exact Tag17 punishment finalized at height 124 under tip 127; Maker and Taker independently retained identical canonical facts, `Claimed` metadata and zero custody. The checked certificate is `docs/evidence/m7-actual-tag17-a23a314-20260804.json`. The v0.1.2 compatibility re-attestation remains GREEN, while historical tag 16 recovery remains valid evidence for the former deployment. Public deployment stays deferred under ADR 0023; Logos-owned disclosures remain tracked under ADR 0018 |
| F6 | Atomic outcome: both claim or both refund | Six scenario tests and 512 generated transition sequences pass. M2/M3 retain their certified conditional claim/refund evidence. For M4, Stage A/B precommitted exact claim, signed-refund, and punishment messages; the Taker partial stayed private until exact XMR funding; finalized Maker claim disclosed the share used for the Taker sweep | M7 local-functional GREEN under ADR 0203: the M4 successful claim branch executed conditionally atomically, not as one distributed cross-chain transaction. The claim binder directly cross-checks finalized LEZ height 4208/tip 4220 against Monero receipt height 121/tip 130, but is a historical snapshot and not future-reorg immunity. The refund runner/binder now requires finalized tag 16 in the signed half-open window, derives the Taker share only for the Maker after that effect, and verifies the Maker-directed Monero receipt with honest roles and exact accounting. Exact run `m5xmrrefund45924caa` proved the clock effect leaves escrow byte-identical, finalized tag 16 in the signed window, disclosed the refund share only afterward, and confirmed the Maker Monero sweep. This is conditional both-refund evidence, not a distributed commit or future-reorg guarantee. Checked run `m7tag17a23a314a` separately proved the exclusive post-`punish_at` Tag17 LEZ terminal transition: claimant-only signature, no aggregate signature, `Claimed` state and zero custody. ADR 0163 adds durable application-owned Tag17 preflight, one-send, restart observation, and terminal supervisor reconciliation. ADR 0164 proves that the same real actor and supervisor select Refund versus Punish only from the mutually exclusive durable branch, give FD 218 only to the Refund invocation, and never resend either route. ADRs 0165 and 0166 bind finalized Tag16 to FD 219, reconstruct only in memory, and submit the Maker-directed Monero sweep once without mining or finality wait. ADR 0167 adds spend-authority-free typed Monero finality observation, strict Pending classification, and atomic restart evidence. ADR 0168 permits the durable Refund CAS only after exact funding and finalized Maker-side Tag16 evidence; no operator branch input exists. Joined run `m7abandon-a742c9f-a` proves the disclosed penalty economics on one fresh two-devnet agreement. Exact run `m7lose16-930e3b4-a` then proves a transport-accepted valid late Tag16 produces no finalized Refund from heights 168 through 175 and cannot mutate canonical Tag17 facts. Exact run `m7lose17-63a9496-b` makes the symmetric ADR 0176 ordering actual-node GREEN: Punish was absent from heights 219 through 226 after one late accepted Tag17, and canonical Tag16 remained unchanged; the checked certificate binds exact cleanup. Concurrent boundary schedules, process-kill, fee, and reorg cases remain under R1, R2, R6, and R7; no distributed transaction or future-reorg immunity is claimed. GW-M4-003 preserves the punishment/economic-safety discrepancy |
...
| F9 | Headless maker covers configuration, pricing, advertisement, execution, monitoring, and full CLI operation | UJ-007 actual CLI/daemon suite plus restart, history, manual claim/refund, pricing, advertisement, and fixed-service lifecycle cases | M7 local-functional GREEN under ADR 0203: owner-local configuration/pricing/offers/create/status/history/alerts, generation-fenced actions, simultaneous workers, fixed service control, and automatic route-scoped health/withdrawal are process-GREEN. Health workers are bounded, hash-pinned, nonoverlapping and isolated from the RPC loop; accepted state is not revoked. Clean run `m7outage-2c63218-a` completes the actual unaffected-pair dependency-loss composition across Maker restart. ADRs 0152–0157 give receipt-v2 XMR children sealed application inputs, a canonical plan, least-privilege branch material, real Tag16 and Tag14 semantic sender control flows, and literal CLI preflight/invoke/observe/Complete process proofs. ADR 0163 additionally makes schema-3 Maker Tag17 recovery normal-supervisor GREEN through one send and finalized-only terminal reconciliation. ADR 0164 extends that application boundary to durable Refund selection and one-shot Monero-refund route composition with invocation-only FD 218. ADRs 0165 and 0166 add FD 219, transcript-bound in-memory reconstruction, and the real non-mining Maker wallet-RPC sender. ADR 0167 adds the read-only Maker refund finality observer and atomic replay receipt. ADR 0168 adds the exact-evidence import and Refund-only activation gate. ADR 0189 continuously checks the joined actual-node Tag14, finalized Tag15 and Taker Monero claim sweep. ADR 0198 makes both signed ZEC directions real CLI/Chat process-GREEN with direction-correct first-claim custody and atomic lost-response replay. ADR 0199 and checked run `m7zecfirstrefund8981e32a` additionally prove that the Maker daemon can be absent before any chain effect while the owner Taker service alone funds and canonically refunds the reverse ZEC first lock with no LEZ effect or replayed submission. The literal headless-Maker feature is complete; adverse schedules remain under Reliability |

Current M7 F3 closure: exact pushed run `m7xmrconc-d8efb7ca` accepted two
same-direction XMR applications through one Maker daemon/database and one
shared local LEZ v0.2/official Monero 0.18.5.1 Regtest topology. Separate
Taker LEZ signer/nonce domains, agreements, escrows, outputs, stores, and
journals preserved per-swap authority. Both ten-confirmation locks, finalized
LEZ claims, Monero sweeps, zero-submission replay, and exact cleanup passed.
The CI-pinned certificate is
`docs/evidence/m7-actual-xmr-accepted-concurrency-d8efb7c-20260812.json`.
This makes F3 GREEN for private-local functional acceptance without claiming
public-network reliability or future-reorganization immunity.
...
ADR 0177 and exact pushed-source run `m7refundkill-f8bee63-d` now close that
specific daemon-restart gap for F3/F6/R2/R4. After finalized Tag16 and one
durable Monero refund submission, the harness killed the daemon then actor
before stdout. Restart transferred abandoned lease generation four to six,
published `maker_recovery_available` revision one through ObserveOnly without
resending, mined exactly ten local Regtest blocks only afterward, and reached
terminal Refunded revision two with the manual action Completed. Source status
zero, exact cleanup, and foreign-sentinel preservation passed. The checked
packet is
`docs/evidence/m7-actual-maker-refund-process-kill-f8bee63-20260808.json`.
This is partial hardening evidence: it does not close other kill seams,
...
| U2 | Long-running autonomous maker daemon plus systemd unit/install guide | Packaged daemon runs under hardened systemd in an isolated VM, restarts, advertises, prices, and executes without GUI | M7 GREEN: hardened systemd lifecycle, exact identity, staged verification, crash recovery, and bounded 1..=32 worker supervision pass. Checked run `m7btcconc-272788c-a` composes two direction-bound accepted BTC applications through one daemon/database, restarts before activation without replay, proves four isolated role states and signing journals at the shared revision-two barrier, completes both actual-chain claim paths, and replays all four terminal roles with zero resubmission. Exact accepted-ZEC run `m7zecpk820001ba` then kills the owning daemon and actor after one accepted funding submission and before stdout, transfers lease generation 15 to 16 over the same database and singleton transaction at unchanged tip 104, completes both user roles at tip 107 and terminal generation 27, and leaves the final mempool empty. The CI-pinned certificate is `docs/evidence/m7-actual-zec-accepted-process-kill-820001b-20260811.json`. Immutable live Logos Core integration stays an upstream production-parity exception rather than blocking local RFP certification. |
| U3 | Maker CLI configures pairs/prices, controls daemon, queries history, and triggers claim/refund over IPC/RPC | Black-box CLI-to-daemon command matrix under owner/wrong-owner roles plus fixed-system-service contract | M7 GREEN under ADR 0197. Existing process suites retain configuration, price, offer, create, status, history, monitor, fixed-service controls, and wrong-owner/generation rejection. The joined six-case matrix crosses BTC/XMR/ZEC with Claim/Refund through the real `lez-maker` CLI, owner Unix RPC, a clean daemon restart, production supervisor, validated pair actor, atomic SQLite resolution, exact replay, and post-terminal rejection. BTC Claim maps to `drive`; XMR/ZEC Claim map to `claim`; all Refund actions map to `recover`. Every case terminalizes at generation 1/attempt 1 and reopens with no child identity or second invocation. No chain-effect claim is inferred from the marker actors; pair actual-node evidence remains separate. |
| U4 | Taker CLI covers discovery, initiation, monitoring, claim, and refund | Actual taker CLI drives each pair’s happy and abandoned-counterparty journey | M7 local-functional GREEN under ADR 0203: ZEC discovery/initiation/receipt-bound monitor/claim/refund, BTC discovery/initiation/offline monitor, and XMR discovery/initiation/receipt-only monitoring plus the receipt-v2 Tag14 marker claim are process-GREEN; clean actual-node BTC and composed XMR application corridors exist. ADRs 0152–0154 give XMR children sealed validated application bytes on FDs 211..216, the canonical plan on FD 217, and the private share only to share-consuming Tag16/Monero routes on FD 218. Literal `lez-taker refund` now performs prepare-only preflight before CAS, invokes the actual Tag16 child once, observes/reconciles on restart, and returns Complete without a process. ADRs 0155 and 0157 add and select the sealed finalized-gated Tag14 release service. ADRs 0163 and 0164 join Maker recovery to the normal supervisor and select durable Refund or Punish without operator-controlled effect routing. ADRs 0165 and 0166 seal finalized Tag16, reconstruct only in memory, and submit the Maker Monero refund once without mining or retrying. ADR 0167 adds the spend-authority-free Maker refund finality observer; ADR 0168 derives application Refund activation from exact funding and finalized Tag16 rather than an operator branch. ADR 0189 continuously checks the receipt-v2 actual-node Tag14, finalized Tag15 and Taker Monero claim sweep. ADR 0198 makes ZEC initiation process-GREEN through the real CLI and signed Chat in both directions; reverse gives the Taker alone first-claim/funding authority and leaves zero Maker claim rows. ADR 0199 and checked run `m7zecfirstrefund8981e32a` now drive that reverse direction through the actual owner Taker service after the Maker disappears, preserving the signed deadline and returning the only funded Zcash leg without a LEZ effect. All literal Taker journeys are retained; accepted-XMR concurrency remains under F3 and adverse schedules under Reliability. XMR monitor validates pre-effect application authority under the per-swap lock and explicitly does not infer chain progress |
...
| U7 | SPEL IDL for LEZ escrow program(s) | IDL generation/validation test against pinned compatible SPEL/LEZ versions | M7 GREEN under ADR 0151: the deployment package and runtime sidecar generate from one pinned v0.2 `PROGRAM_IDL_JSON`; the exact IDL and generated-client SHA-256 values are frozen in tests, verifier, and deployment manifest. Build-time semantic assertions preserve all 18 instruction tags, native/XMR/token account order, owner and aggregate-authority signer roles, permissionless refund/custody creation, and official Token/ATA custody. The fast CI contract, isolated artifact verifier, and full locked-offline runtime-sidecar tests/Clippy/Rustdoc/dependency policy are GREEN; the sidecar lock now records its existing local SDK-core edge without registry-version churn. A future ABI or generator change must fail until explicitly reviewed and repinned. |
...
| ID | Contract | Acceptance evidence | Status / milestone |
|---|---|---|---|
| R1 | Taker locks first; maker waits for required confirmations | Core rejects early lock and uses independent immutable policies for each funded leg; reverse ZEC proves maker-funded ZEC below-threshold accumulation, promotion, 10→9-style depth regression suspension, exact depth recovery, removal pinning, conflict rejection, and preserved refunds. Both actual-node happy directions now show the direction-derived taker effect first and prohibit the LEZ reveal until the Zcash lock has two confirmations. Watcher tests still require stable affirmative evidence; runtime requires both role policies to match the immutable named profile and commits conflicting chain replacements without substituting the protocol-pinned ID. | M7 GREEN under ADR 0207: the aggregate gate binds actual BTC/XMR/ZEC ordering and ZEC canonical-reappearance evidence. Each direction-derived Taker effect precedes the Maker effect, and the Maker effect remains absent while required confirmation is missing or detached. Broader future-fork calibration remains production work, not a missing R1 acceptance criterion |
| R2 | After first submission, only local state and chain nodes are required | BTC retains happy/refund/survivor evidence. XMR run `m5-xmr-app-20260730-2c6aec1-h` archives authenticated reconciliation, installs an empty Delivery outage, replays from durable application state, proves process cutoff, and completes using only role-local journals plus local LEZ/Monero nodes. Run `m7refundkill-f8bee63-d` additionally recovers a submitted Maker refund from the same local database and local nodes after daemon/actor SIGKILL without Delivery or resubmission | M7 GREEN under ADR 0207: both BTC directions complete from persisted role state without post-lock transports; XMR Claim/Refund process-kill paths recover from journals and local nodes without resend; ZEC Claim survives owner-process loss after first submission, and ZEC Refund completes through only the owner Taker service and local nodes while the Maker and transports remain absent |
| R3 | Missing chain dependency does not disable other pairs | Dependency matrix starts daemon with each node absent/unhealthy and completes unaffected-pair swaps with clear CLI/GUI status | M7 GREEN under ADR 0150. Hash-pinned bounded semantic probes, active-offer CAS withdrawal, accepted-negotiation preservation and route-scoped quote rejection remain component-GREEN. Clean pushed run `m7outage-2c63218-a` adds the literal composition: a real stopped Bitcoin Core node stayed unavailable before and after Maker restart while the independent actual-node Zcash route stayed available and completed both users' claim journey. The 36.920-second corridor used three deterministic Zebra blocks, actual finalized LEZ effects, zero same-run retries and no public or external runtime resource. Certificate: `docs/evidence/m7-unaffected-pair-outage-2c63218-20260804.json` |
| R4 | Persisted state survives crash/restart without fund loss | Schema v9 retains the schema-v8 lock journal and adds protected claim material, encrypted exact claim intents, owner/observer transitions, and two-direction replay to `Completed` (`add5d98`). Coordinator snapshots retain only a SHA-256 claim marker; AAD binds agreement/role/step/revision/expected identity. The crash-safe v8→v9 migration rewrites exact legacy plaintext arrays, enables secure deletion, truncates WAL, and reopens the marker (`5ed04ec`). The dedicated R4 baseline gate executes exact wrong-key/material, protected ciphertext/nonce/fingerprint/future-version, orphan/duplicate/drift, coupled-SQL rollback, and accepted-before-transport-failure recovery tests. Exact run `m7refundkill-f8bee63-d` proves the actual-node Maker refund Started journal survives daemon/actor SIGKILL and can only ObserveOnly the unchanged submission to terminal. ADR 0194 and exact run `m7claimkill507a38ba` add the complementary real Taker Tag14 seam. ADR 0195 and exact run `m7claimsweep997bd6bb` prove the subsequent real Taker Monero sweep was accepted once before CLI stdout, survived SIGKILL with identical durable evidence, recovered only through observation with zero pre-restart confirmations, and finalized without another send. ADR 0196 and exact run `m7tag15kille455deca` complete the Maker Tag15 seam: generation 4 submitted once before actor stdout, generation 7 terminalized through owner-exact observation over unchanged durable evidence, the independent Taker classifier found the same finalized transaction, and the Monero sweep reached ten confirmations. | GREEN: generic corruption/rollback/unknown-submission recovery plus actual-process Maker refund, Taker Tag14, Taker Monero claim-sweep, and Maker Tag15 accepted-before-stdout restarts are certified by the R4 baseline. Every ambiguous effect retains unchanged identity and zero second send; exact cleanup and the foreign sentinel pass |
| R5 | Concurrent swaps have independent state, escrow, and deadlines | M3 clean run `m3overlap-20260717a` proves the underlying opposite-direction actor/chain boundary. Checked M7 run `m7btcconc-272788c-a` adds two authenticated accepted applications under one daemon/database and one post-acceptance restart | GREEN: the retained certificate binds distinct swap agreements, funding outpoints, four actor databases, eight signing journals, signer sessions, LEZ escrows and deadlines; both swaps were simultaneously revision two before either settlement, both reached terminal revision four, all four replays resubmitted zero effects, and exact cleanup left foreign resources untouched. This is bounded opposite-direction BTC concurrency, not arbitrary-N or same-direction scheduler proof |
| R6 | Timelocks cover variance, congestion, and clock drift per chain | Typed pair-specific domains and ordering pass; LEZ seconds/milliseconds conversion is explicit, conservatively rounded, overflow-checked, and boundary-tested; named runtime profiles and public calibration remain | M7 GREEN under ADR 0207: the named profile documents confirmation depths, recovery horizons, clock domains, and explicit variance/congestion/drift/reorg/inclusion/reaction budgets. Typed exact/short/overflow/cross-domain tests and actual signed-boundary refunds pass. Public calibration and mainnet value-at-risk tuning remain honest production work, not a missing documentation-and-rationale criterion |
| R7 | Bitcoin refund construction is chosen and justified with failure analysis | ADR 0009 selects script-path CSV; Bitcoin Core tests cover exact boundary, key, reorg, current fees, and RBF/CPFP | M7 GREEN under ADR 0207: ADR 0009 chooses the RFP-permitted consensus-enforced Taproot CSV script path, documents its visible-refund privacy cost, and contrasts every named pre-signed failure mode. Actual Core runs refund the exact Bitcoin output in both directions with paired LEZ refunds and zero-effect replay. The RFP makes the extended fixed-fee mitigation inventory conditional on choosing pre-signed key-path refunds, which this implementation does not |
...
| S5 | Complete reference integration for every chain | UJ-001/UJ-002/UJ-004 role E2E and runnable reference package for BTC, XMR, and ZEC | Partial across all pairs: ZEC and BTC retain certified private local references, and checked run `m7btcconc-272788c-a` certifies two accepted BTC applications overlapping through one daemon/database and both local chains. XMR has clean exact-commit application claim and refund corridors through real local LEZ v0.2 and official Monero Regtest processes, role-correct funding/recovery, conditional binding, manual repetition, and scoped cleanup. ADR 0198 adds real ZEC CLI/Chat process integration for both directions with role-correct claim custody and replay. ADR 0199 and checked run `m7zecfirstrefund8981e32a` add the reverse actual-node absent-Maker reference: the owner Taker service funds and canonically refunds the sole Zcash leg while both LEZ submission journals stay empty. Complete remaining daemon-owned Maker/Taker journeys, unavailable-route composition, public execution, and functional hardening remain |
...
| ID | Contract | Acceptance evidence | Status / milestone |
|---|---|---|---|
| D1 | Happy, abandonment/refund, and concurrent-swap recorded demo video for each pair | Nine videos generated from passing role E2E runs, with commit/testnet/version metadata | GREEN. BTC retains the three M3 actual-node videos and sealed bundle. Exact pushed commit `1b47a15` adds XMR and ZEC happy, refund/timeout, and concurrent H.264 1280x720 MP4s, regenerated-source verification, full decode probes, private modes, and bundle SHA-256 `a23b7d32b11ce91a44875750c82bccc470659cac380de90d61c9e7b2e743bf5b`. Five M7 scenarios bind joined actual-node evidence. ZEC concurrency honestly layers the real one-daemon overlap regression with separately bound actual-node Claim and Refund effects and explicitly denies a joined concurrent chain run. No runtime public RPC, peer, faucet, funds, or deployment participated; the private bytes remain ignored and the secret-free certificate is CI-pinned. |
...
| Taker CLI | F3, U4 | Retained pair corridors plus receipt-v2 Tag16 refund 1/1 in 106.26s with rejected-preflight retry, prepare-only preflight, send-once, observe/reconcile, Complete, and losing-claim exclusion |
