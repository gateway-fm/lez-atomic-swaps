...
| F3 | LEZ–XMR via Ed25519 adaptor signatures, cross-curve DLEQ, and spend-key share | LEZ-first COMIT/DLEQ vectors; `monerod`/wallet RPC stagenet happy/refund/concurrency role E2E; spend-key recovery fault case; XMR-first rejected | M4 component partial: DLEQ-bound share reconstruction, two-stage SDK, checked tags 13–17, official Monero 0.18.5.1 Regtest topology/reconstructed spend, origin-retaining observation, strict nine-method v3 boundary, sealed release worker, canonical tag-13 executor, exact synthetic Initialize/Fund classifier, typed pre-Fund barrier, and pure exact Stage-A tag-15/tag-16/tag-17 future-message/hash planning are GREEN. The three-test planner derives claim/refund authorities and the complete nonce schedule from a caller-supplied stable finalized snapshot; existing tag-15 prepare/complete accepts its claim bytes exactly, while it performs no RPC, reservation, persistence, signing, or send. Four of seven builders now durably prepare Initialize/Fund and tag 14, then Maker tag-15 prepare/complete with exact nonce/ABI/accounts/hash, aggregate BIP340 verification, restore-only startup revalidation. Generic submission now admits only the exact completed durable tag-15 claim; its authenticated restart test makes one accepted send, while tag 14 remains dedicated-release-only. The pinned sidecar gate, strict Clippy/Rustdoc, 96 non-doc adapter tests plus 3 doctests, and 35-test release authority pass. The exact checked M4 ELF is now actually deployed once on a fresh isolated LEZ v0.2 finalized history, and two independent deterministic-genesis actor identities have finalized Vault onboarding. Actual tag-13 Initialize/Fund execution/classification is GREEN in blocks 3008/3023; tag-14/tag-15 execution/finality, lifecycle actors, actor-owned aggregation/extraction, official-wallet claim, refund/punishment, Stagenet trust, concurrency hardening, and the actual swap remain |
...
| F5 | Risc0 LEZ escrow validates pair proof and supports claim/refund | Per-pair guest instruction tests plus standalone-sequencer initialise/claim/refund E2E | Historical v0.1.2 and canonical M2/M3 checked/deployed artifacts remain certified. M4 appends tags 13–17 without changing legacy digests; two fresh builds reproduce ELF `dc370bc34b432317730c51b49342760dbc675fca700e300b30b5fadefe5b7292` and ImageID `4d6590332948743c2db88a183755815354ef92560550cd206ac27bddeea12c82` across five recursive branches. `deploy-m4-local` is component-GREEN: 19 manifest/runtime mutations and three non-loopback endpoints make zero RPC calls, followed on the exact path by one deployment send and bounded canonical inclusion. Four of seven sidecar builders and exact Initialize/Fund classification are GREEN, including durable restore-only tag-15 completion. A pure three-test Stage-A planner additionally constructs exact generated tag-15 claim, tag-16 signed-refund, and tag-17 punishment messages and distinct hashes from the checked nonce schedule; tag-16/tag-17 builders remain unavailable. Exact checked local deployment is GREEN: transaction `8bb883f1...63f9` finalized in block 86 after a full finalized genesis-through-86 scan proved zero prior and one total exact-ELF occurrence, decoded ELF/ImageID equality, sequencer/indexer equality, and stable ID/hash/ID rereads. Two independent actor Vault Claims also finalized once in blocks 228 and 240. Actual tag-13 Initialize/Fund finalized once in blocks 3008/3023 with exact transaction and containing-block evidence. Actual tag-14/tag-15 swap effects/finality, lifecycle actors, refund/punishment, and an M4 swap remain; public deployment stays deferred under ADR 0023 and Logos-owned disclosures remain non-blocking under ADR 0018 |
| F6 | Atomic outcome: both claim or both refund | Six scenario tests and 512 generated transition sequences pass. Run `m3schema4-20260717d` retained presign-before-effect, Taker-first, both canonical locks before reveal, actor-owned claims through revision four, and unchanged replay counts. Runs `m3refund-20260716h` and `m3firstlock-20260716h` execute both ordered two-lock and absent-maker first-lock refunds. Run `m3survivor-20260716c` proves a fresh maker can complete from canonical reveal after taker disappearance. Run `m3overlap-20260717a` proves those per-swap gates remain isolated for two opposite-direction swaps simultaneously at revision two before either settlement; both terminate with disjoint effects and zero replay. This is conditional protocol recovery, not one distributed commit across Core, LEZ, and SQLite. Adversarial cutoff/refund races, reorg, process-kill, and arbitrary-N/same-direction scheduling remain. For M4, GW-M4-003 records that the cited XMR construction's later Maker punishment and hidden-partial grief model are economic safety, not literal F5/F6 both-refund conformance; the local PoC may proceed only with that limitation explicit. The pure Stage-A planner now commits exact generated claim, signed-refund, and punishment messages/hashes before locks and proves existing tag-15 byte compatibility without performing I/O; tag-16/tag-17 builders and chain effects remain absent. The typed issuer derives the journal's exclusive end from the same signed refund time enforced by the checked guest, while the post-CAS finalized-time sample narrows the publication race and Suppressed proves no send. Generic tag-14 submission remains rejected; the dedicated release-intended route proves its 3-of-3 official-type matrix: accepted at one lookup/one send, exact `AlreadyKnown` at one lookup/zero sends, and wrong official returned ID as `UnknownSubmissionOutcome` at one lookup/one send, with same-request replay counters unchanged, not release-service-gated actual-chain publication. The tag-13 route contributes at-most-once exact-effect admission without secret reveal, but finalized Initialize remains an actor barrier. Atomicity remains unproved until actual submission, authorization finality, canonical claim/recovery effects, and role flows execute. | Partial |
