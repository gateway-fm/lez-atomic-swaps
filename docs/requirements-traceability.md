...
| F4 | LEZ–transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors and immutable profiles pass; a bounded dual-signed agreement binds actors, LEZ custody, BIP-199 output, transaction policy, actual LEZ deadline bound, and Zcash CLTV; concrete activation/resume, exact first-lock intent, primitive recovery-record revalidation, and in-memory atomic projection/replay contracts are integrated; funding observation is durable and actual Zebra proves fork replacement/restart; bounded spend recognition accepts every pinned-consensus sighash/high-S/nonminimal form without hiding the preimage and reports SDK policy separately; production active persistence, maker-independent observation, durable spend reorgs, cross-chain ordering, and both-direction testnet actor E2E remain | Concrete agreement, activation/resume, observe-before-rebroadcast plus unknown-commit first-lock contract, ZEC transaction/consensus/funding reconciliation, spend recognition, profiles, schema-v5 funding journal/alerts and constrained SDK recovery tables/DTOs, core role semantics, and actual-node restart/requery are implemented; full SDK effects, production recovery adapter, testnet, and actor delivery remain M2 |
...
| R4 | Persisted state survives crash/restart without fund loss | SQLite reopen and actual daemon kill/restart pass; schema-v5 retains the ZEC journal/binding/alert guarantees and adds role/revision/foreign-key constrained SDK recovery tables; primitive SDK intent/transition DTOs reject future schemas, unknown/substituted fields, oversized bytes, invalid evidence, and corrupt closed intents before rebuilding trusted state; existing store tests prove future-version rejection, atomic swap+binding and event+aggregate+alert rollback, immutable rebinding, stale revisions, role isolation, and revalidated loading; both-direction runtime tests probe unknown-outcome replay before mutation, replay ordered records into the exact historical tracker head, reject impossible sequence history, require every new event to advance that head before projection, and require fresh reconciliation after close/open; conflict/terminal alerts deduplicate and preserve swap-scoped acknowledgment across replay/restart; authenticated CLI status/list/ack retains phase and history; the actual two-Zebra fixture proves canonical commit, two close/reopen cycles, fresh requery, affirmative fork removal, and no duplicate on retry; production SDK recovery adapter, kill-at-every-effect-transition, SQLite-row corruption, encryption, and general effect outbox remain | Partial |
