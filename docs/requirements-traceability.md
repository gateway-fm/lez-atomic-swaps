...
| F4 | LEZ-transparent-ZEC via BIP-199 HTLC; LEZ claim/refund precedes ZEC and the ZEC refund strictly outlives LEZ by the documented margin | Exact vectors, profiles, agreement validation, canonical observations, and fresh maker eligibility pass. Separate role-fixed maker/taker SDKs drive both lock directions from chain evidence, persist schema-v8 journals, and independently restart at `BothLegsLocked`; actual Zebra fork/restart and local LEZ custody lifecycles pass separately. | Lock convergence and durable replay implemented; canonical production maker-lock adapters, LEZ-reveal/Zcash-follow-up claims to `Completed`, refunds, margin composition, process actors, and public-testnet evidence remain M2 work |
...
| ID | Contract | Acceptance evidence | Status / milestone |
|---|---|---|---|
| U1 | Dedicated full-lifecycle SDK for each pair | Shared SDK contract suite instantiated for BTC/XMR/ZEC; public API doctests cover discovery through refund | Partial: ZEC role-fixed discovery, negotiation, separate persistence, both lock effects/observations, and restart at `BothLegsLocked` are integrated in both directions. Protected preimage and exact-payload cryptography plus contract-double first-claimant activation exist, but SQLite claim persistence/actions, production adapters, and actor-process E2E remain M2. |
...
| R4 | Persisted state survives crash/restart without fund loss | Schema-v8 SQLite retains agreement, canonical history, taker/maker intents, taker-observed maker funding, exact transitions, revision, bindings, and alerts. Fifteen SDK-recovery tests cover rollback, torn/corrupt/future rows, stale catch-up, mixed revisions, exact replay, both-direction unknown submissions, and independent taker replay to `BothLegsLocked`; union replay rejects holes/duplicates. Actual daemon/two-Zebra restart also passes. Protected claim envelopes are tested but not yet wired into SQLite. | Partial |
