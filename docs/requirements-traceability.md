...
| U6 | Basecamp taker mini-app browses and executes swaps | The prototype journey and isolated browser proof are GREEN. The exact seven-method DTO contract, authenticated health/list backend, owner-only read process, same-FD private-file custody, replay-first two-connection ZEC initiation registry, and strict prepared-ZEC authority loading are component-GREEN through `28006dc`. Only health and offer list are registered; the registry is not service-wired, initiate/list-swaps/monitor/claim/refund are method-not-found, and actor-real QML E2E remains | M6 active under ADRs 0128 and 0130 through 0133; owner sign-off still gates QML/QtRO |
...
| S6 | README covers deployment, addresses, prerequisites, and maker/taker CLI/mini-app use | Fresh-machine documentation tests plus link/command validation | Partial M6: prototype review, actual read-only Taker service config/start/health/list/manual isolation, registry proof, and prepared-ZEC authority proof are documented. Basecamp package build/load, downloadable production assets, actor-real Maker/Taker GUI use, mutation flows, and final link/command validation remain |
