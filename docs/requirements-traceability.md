...
|---|---|---|---|
| F1 | No central server; Delivery advertisements and Chat coordination | Role E2E discovers and negotiates through real Delivery/Chat adapters; central services absent; UJ-006 repeats with both stopped after lock | Partial M5: signed bounded daemon-to-taker run-local Delivery publication/discovery plus restart reconciliation and durable expiring offers, one-winner reservation, and atomic coordinator consumption are component GREEN; maker-first ZEC proposal/countersigning, bounded unsigned draft handoff, and one-winner durable proposal staging are GREEN; the disjoint Chat process now authenticates, signs, stages before response, countersigns as the taker role, atomically accepts with daemon-local protected authority, isolates methods, and survives kill/reopen. The actual taker command durably persists the countersigned wire before completion, validates its private agreement/draft and both roles on rerun, and retries only exact completion. The daemon holds startup-pinned per-swap Maker authority, durably publishes one Maker-only actor bundle, atomically registers its schema-v16 manifest, and preflights the exact committed request/negotiation/actor before any current-wall-clock parse or provisioning. Actor-bearing systemd lifecycle and exact-snapshot BTC/ZEC role/swap/state admission are GREEN. The real daemon/taker journey proves that retry after a three-second TTL returns the original result; changed inputs or missing actor fail closed, and a stale consumed Delivery projection reports ready plus degraded/unavailable until reconciliation. Bounded supervisor execution, disjoint concurrent application swaps, actual-node supervisor composition, and other-pair composition remain |
...
| U2 | Long-running autonomous maker daemon plus systemd unit/install guide | Packaged daemon runs under hardened systemd in an isolated VM, restarts, advertises, prices, and executes without GUI | Partial M5: hardened `Type=notify` unit, encrypted credentials, installed real ZEC actor, pinned authority/root/program/digest inputs, staged verification, and actual nine-second user-systemd readiness/SIGKILL-restart/SIGTERM-cleanup are GREEN. Autonomous supervisor execution, multi-pair composition, and an immutable live Logos Core API remain |
