...
as complete. Tag17 is separately actual-node GREEN under ADR 0158 and normal Maker-supervisor GREEN under ADR 0163.
...
| U3 | Maker CLI configures pairs/prices, controls daemon, queries history, and triggers claim/refund over IPC/RPC | Black-box CLI-to-daemon command matrix under owner/wrong-owner roles plus fixed-system-service contract | Partial M5: configure/price/offers/create/status/history/monitor and supported generation-fenced actions pass through the real daemon/CLI with replay and restart evidence. Literal start/stop controls only the packaged system unit with bounded exact-state verification. Schema-3 Maker Tag17 Refund is now daemon-supervisor owned under ADR 0163. Receipt-v2 Taker Tag14 and Tag16 remain separate CLI boundaries; complete the remaining supported-pair lifecycle matrix |
| U4 | Taker CLI covers discovery, initiation, monitoring, claim, and refund | Actual taker CLI drives each pair’s happy and abandoned-counterparty journey | Partial M7: ZEC discovery/initiation/receipt-bound monitor/claim/refund, BTC discovery/initiation/offline monitor, and XMR discovery/initiation/receipt-only monitoring plus the receipt-v2 Tag14 marker claim are process-GREEN; clean actual-node BTC and composed XMR application corridors exist. ADRs 0152–0154 give XMR children sealed validated application bytes on FDs 211..216, the canonical plan on FD 217, and the private share only to share-consuming Tag16/Monero routes on FD 218. Literal `lez-taker refund` now performs prepare-only preflight before CAS, invokes the actual Tag16 child once, observes/reconciles on restart, and returns Complete without a process. ADRs 0155 and 0157 add and select the sealed finalized-gated Tag14 release service. ADR 0163 separately joins Maker Tag17 to the normal supervisor. Joined actual-node finality/reconciliation, Tag14 claim and Tag16 refund composition, both Monero sweep workers, and abandoned-counterparty journeys remain. XMR monitor validates pre-effect application authority under the per-swap lock and explicitly does not infer chain progress |
