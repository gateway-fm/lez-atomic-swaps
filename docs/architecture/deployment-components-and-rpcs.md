...
| Official-wire LEZ sidecar | Native escrow and claim paths plus M3 witnessed preparation/completion and finalized funding/claim observers are actual-node GREEN. The strict refund wire and durable exact v0.2 refund planner are component GREEN | Actor-facing server is capability-authenticated literal loopback; outbound `local` is uncredentialed loopback HTTP and `official_public` remains exact pinned HTTPS | Capability, run, fixed role, signer, runtime, channel, genesis, program, destination, and M3 aggregate authority are checked independently of transport | Refund preparation produces official `[metadata, custody, depositor]` bytes with zero nonce/witness and admits only the retained reservation. Finalized refund observation and its server registration are not implemented yet | Actor claim revisions completed in both actual-node directions in `m3actor-20260716n`. Refund execution, historical account proof/snapshot, crash/reorg, and production trust remediation remain open |
...
| M3 v0.2 native-refund planner | Durable exact planner GREEN; no actual-node refund claim yet | No RPC, node, Docker, faucet, or public endpoint is used by the planner tests. A future authenticated loopback bridge method will expose the same retained result | The sidecar role, complete runtime, escrow and transfer programs, immutable depositor, and witnessed aggregate key/account are revalidated. The official transaction is permissionless and has no signer, nonce, or witness | Five focused tests cover exact ABI, legacy/witnessed terms, identity and transaction mutation, restart replay, distinct-request exclusion, owned-submission admission, and zero nonce-source calls. Finalized eligibility, server wiring, actor one-attempt submission, and actual-node evidence remain |
| M3 witnessed LEZ v0.2 stack | Repository-owned happy-path actor composition GREEN in both directions; exact durable refund preparation component GREEN | Successful services: Bedrock `127.0.0.1:32914`, sequencer `127.0.0.1:32915`, indexer `127.0.0.1:32916`; final sidecars: foreign maker/taker `52895/48941`, LEZ maker/taker `60737/48599`, all literal loopback and never defaults | Separate capabilities, signer keys, stores, and journals. Sequencer admission is not finality. Bounded finalized scans use ID/hash equality and finite 30-second bridge calls. Refund bytes have immutable depositor and zero signer authority | Guest `a199c5be...e293` / ProgramId `39b6a4db...4dec` deployed in block 6. Happy claims finalized in 16/19/25 and 31/34/42 with zero custody. The refund planner itself performs no RPC; finalized refund observer/server/actor and actual-node timeout evidence remain |
