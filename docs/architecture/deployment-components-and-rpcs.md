...
| SDK-facing LEZ bridge client and adapter | Fourteen-method client, signed-agreement adapters, crash-safe context-owning SDK ports, witnessed prepare/complete, distinct finalized observers, historical M2 actor wiring, and M3 actor funding observation through revision two | Literal-IP run-owned loopback HTTP; fresh client per attempt; no redirects or proxy settings | Capability plus exact run/role/runtime/ProgramId and caller-owned request IDs. The BTC actor derives terms from the signed agreement and uses each role own sidecar | Prepare, complete, submit, progress observation, finalized funding, and finalized claim. The first two actor transitions use only the read-only finalized funding method | Funding validates the covered window, canonical transaction, historical `Funded` metadata, custody, accounts, and finalized tip before CAS. Pre-funding errors are retryable unavailability, not false absence. Exact retries retain the deterministic request ID; a deliberate window change receives a distinct evidence-bound ID. Claim actor revisions and actual-node E2E remain open |
