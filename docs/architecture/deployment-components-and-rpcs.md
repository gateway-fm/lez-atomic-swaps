...
| Bitcoin Core | Core 31.1 role infrastructure and a known-key P2TR funding/claim composition are GREEN; complete swap pending | Bitcoin Core 31.1 Regtest built from the official checksum/signature/Guix-verified archive; allocated loopback RPC port, no published P2P port | Full cookie and wallet/mining RPC belong only to the run provisioner. Taker submits funding and maker submits the claim with distinct `rpcauth` credentials; the current Regtest helper owns public fixture keys client-side | Exact-pinned `bitcoin` 0.32.101 constructs a normal 1 BTC funding transaction to the aggregate-key plus CSV output. Core policy accepts and mines it at height 102; Core accepts and mines the tweaked-`Q`, one-item 64-byte key-path claim at height 103; the taker observes the exact outpoint spent once | Exact `RUN_ID` resources, critical-file hash manifest, cleanup proof, and bound attestation are retained per run. No public RPC, faucet, funds, or peers are used. MuSig2/adaptor/extraction, LEZ effect, independent processes/stores, refunds, and both complete directions remain |
