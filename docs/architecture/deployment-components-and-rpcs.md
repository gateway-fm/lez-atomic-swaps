...
| SDK-facing LEZ bridge client and adapter | Main-workspace client plus signed-agreement native first-lock preparation and observation conversion implemented; refund/claim composition and actor wiring pending under ADR 0022 | Literal-IP ephemeral owner-loopback HTTP for the M2 runner; owner-restricted Unix socket preferred for production; direct client follows no redirects or proxy settings | Sensitive high-entropy capability plus exact `RUN_ID` and fixed sidecar role; full run/role/runtime echoes and one-use caller-owned request IDs are validated. The adapter rechecks signed environment/channel/genesis/program/role/signer before preparation or observation and independently invokes the SDK canonical validator; official wire decoding remains inside the sidecar | All six typed `lez_bridge.v1.*` methods plus exact native initialize/fund conversion and exact-owner or bounded-discovery escrow observation; method names, identity headers, and body cap are owned once by the protocol crate | Nine client and fifteen adapter tests cover identity, capabilities, replay, malformed/oversized bodies, signed-term mutations, 41 primitive observation mutations, remote errors, response mismatch, and ambiguous transport outcomes. Each call is attempted once; neither layer speaks or duplicates official LEZ JSON-RPC |
| Official-wire LEZ sidecar | Native/revealing-claim planners, official native escrow observation, node-RPC core, authenticated server, and executable role runner implemented; revealing-claim observation, refund methods, and actor composition pending under ADR 0022 | Official LEZ RPC client and actor-facing server enforce literal loopback; each actor listener binds `127.0.0.1:0`; one connection and 5.5 MB bodies maximum. The process test starts maker and taker concurrently with distinct private files and state | Capability, exact `RUN_ID`, and fixed role are checked in middleware before JSON parsing. Planners bind runtime/role/signer/program; native preparation reserves one consecutive nonce pair and revealing claim binds the exact preimage plus request/cache funding identity. The 0600 durable store validates identity, rejects symlinks, and fsyncs file plus directory | All six local methods register. Describe, official native/revealing-claim prepare, exact cached submit, and native escrow observe are complete. Observation decodes official transaction/signature/instruction/account types, validates block/genesis links and identical tip brackets, and reports only full-window absence; revealing-claim observation remains `Unavailable`. Generated upstream RPC covers nonce, submit, tip/block/account, and bounded range scans | Twenty-five all-target tests prove server/planner/node/runner behavior, exact randomized restoration, owner/discovery observations, ambiguity/moving-tip rejection, role isolation, and the durable pre-submit marker. Strict format/Clippy/rustdoc/advisory/license/source gates pass; old or bounded misses never imply global absence |
