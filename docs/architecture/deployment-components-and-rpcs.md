...
| Deterministic LEZ/ZEC SDK lifecycle | Running library/test boundary | No socket, RPC, node, Docker, faucet, or public endpoint; bounded Borsh schema-2 bytes enter from an untrusted negotiation adapter | Fixed maker/taker roles and the signed direction select observations/effects; separate role databases and external claim keys prevent shared claim-recovery authority; refund observers cannot sign | Exact agreement validation, protected activation, both lock directions, LEZ reveal, observer preimage extraction, Zcash follow-up, and fixed LEZ-then-Zcash refund driving with observe-before-rebroadcast and versioned durable records | 16 KiB agreement and 2,000,000-byte submission caps; 131 SDK checks including one doctest plus 35 committed store tests pass. Claims replay through SQLite; refund SQLite rows remain pending. Chain evidence comes from deterministic port doubles, so this row is not an actual-node claim |
| SDK-facing LEZ bridge client | Main-workspace library boundary implemented; actor wiring pending under ADR 0022 | Literal-IP ephemeral owner-loopback HTTP for the M2 runner; owner-restricted Unix socket preferred for production; direct client follows no redirects or proxy settings | Sensitive high-entropy capability plus exact `RUN_ID` and fixed sidecar role; full run/role/runtime echoes and one-use request IDs are validated; canonicality still comes from signatures, exact identities, node facts, and SDK validation | All six typed `lez_bridge.v1.*` describe, prepare, observe, and submit methods; method names, identity headers, and body cap are owned once by the protocol crate | Nine contract tests cover identity, capabilities, replay, malformed/oversized bodies, exact maximum two-transaction envelopes, remote errors, and ambiguous transport outcomes. Each call is attempted once; the client never speaks or duplicates official LEZ JSON-RPC |
| Official-wire LEZ sidecar | Planner, node-RPC core, and authenticated server library implemented; executable runner and observation/revealing-claim cores pending under ADR 0022 | Official LEZ RPC client and actor-facing server both enforce literal loopback; actor listener binds `127.0.0.1:0`; one connection and 5.5 MB bodies maximum | Capability, exact `RUN_ID`, and fixed role are checked in middleware before JSON parsing. Planner binds runtime/role/signer/program and one consecutive nonce pair. The durable 0600 store validates identity, rejects symlinks, and fsyncs file plus directory | All six local methods register. Describe, official native prepare, and exact cached submit are complete. Escrow observation and revealing-claim prepare/observe return typed `Unavailable`. Generated upstream RPC covers nonce, submit, tip/block, and bounded range scans | Sixteen locked tests prove server/planner/node behavior, including exact randomized restoration and a durable pre-submit in-flight marker that prevents blind resubmit after crash. Strict format/Clippy/rustdoc/advisory/license/source gates pass; bounded misses never imply global absence |
| In-process typed Zebra adapter | First-lock plus owner and counterparty follow-up-claim library boundaries implemented; refund effects and actor-process wiring pending under ADR 0022 | Direct bounded Zebra JSON-RPC from each reference actor; explicit nonzero-port `127.0.0.1`/`::1` HTTP only; no bridge or sidecar | Regtest may be unauthenticated loopback; self-hosted Testnet consumes bounded cookie contents into a sensitive Basic header omitted from `Debug`; a role-local injected signer alone sees the claimant key; signed bytes and canonical node observations remain authoritative | Typed chain/block/raw/verbose/UTXO/broadcast DTOs, stable-tip funding classification, exact owner claim, observe-before-rebroadcast, and bounded canonical block/mempool counterparty discovery. Older/unresolved spends are `Unstable`, never absent. Pending: refund effects, removal/replacement ingestion, and actor wiring | Thirty-seven tests use SDK/SQLite-built authentic contexts and fail closed on local, identity, canonicality, policy, conflict, mutation, tip/block/byte, returned-hash, scan-exhaustion, and transport deviations; exact bytes are sent at most once on every post-send error branch; does not transit the LEZ bridge |
