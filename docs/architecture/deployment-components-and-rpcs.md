...
| SDK-facing LEZ bridge client and adapter | Main-workspace eight-method client plus signed-agreement native first-lock, escrow observation, and native-refund validation adapters implemented; revealing-claim adapter, context-owning SDK-port composition, and actor wiring remain under ADR 0022 | Literal-IP ephemeral owner-loopback HTTP for the M2 runner; owner-restricted Unix socket preferred for production; direct client follows no redirects or proxy settings | Sensitive high-entropy capability plus exact `RUN_ID` and fixed sidecar role; full run/role/runtime echoes and one-use caller-owned request IDs are validated. The adapters recheck signed environment/channel/genesis/program/role/signer before preparation or observation and independently invoke SDK validators; official wire decoding remains inside the sidecar | All eight typed `lez_bridge.v1.*` client calls; exact native initialize/fund conversion; exact-owner or bounded-discovery escrow observation; and explicit-ID/window refund state, prepare, exact/discovery observe, and submit conversion. Method names, identity headers, and body cap are owned once by the protocol crate | Refund tests cover both directions, roles, accounts, stable clocks, exact transaction/instruction fields, windows, deadline, depth, one-attempt failures, and unknown submit outcomes; prior suites cover identity, capabilities, replay, malformed/oversized bodies, signed-term mutations, 41 escrow mutations, response mismatch, and ambiguity. Neither layer speaks or duplicates official LEZ JSON-RPC |
| Official-wire LEZ sidecar | Native/revealing-claim planners, official native escrow and revealing-claim observation, node-RPC core, authenticated server, and executable role runner implemented; native-refund methods and actor composition remain under ADR 0022 | Official LEZ RPC client and actor-facing server enforce literal loopback; each actor listener binds `127.0.0.1:0`; one connection and 5.5 MB bodies maximum. The process test starts maker and taker concurrently with distinct private files and state | Capability, exact `RUN_ID`, and fixed role are checked in middleware before JSON parsing. Planners bind runtime/role/signer/program; native preparation reserves one consecutive nonce pair and revealing claim binds the exact preimage plus request/cache funding identity. The 0600 durable store validates identity, rejects symlinks, and fsyncs file plus directory | Six local methods register while the two refund handlers are RED. Describe, official native/revealing-claim prepare, exact cached submit, native escrow observe, and revealing-claim observe are complete. Observation decodes official transaction/signature/Risc0-instruction/account types, validates block/genesis links and identical tip brackets, rejects ambiguity, and reports only full-window absence. Generated upstream RPC covers nonce, submit, tip/block/account, and bounded range scans | Twenty-eight all-target tests prove server/planner/node/runner behavior, exact randomized restoration, native/claim cache coexistence, owner/discovery observations, ambiguity/digest/moving-tip rejection, role isolation, and the durable pre-submit marker. Strict format/Clippy/rustdoc/advisory/license/source gates pass; old, bounded, or partially covered misses never imply global absence |
