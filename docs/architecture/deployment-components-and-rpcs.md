...
| Bitcoin Core | Core 31.1 role infrastructure GREEN; aggregate P2TR transaction library GREEN; composed spend pending | Bitcoin Core 31.1 Regtest built from the official checksum/signature/Guix-verified archive; allocated loopback RPC port, no published P2P port | Full cookie and wallet/mining RPC belong only to the run provisioner. Maker and taker have distinct `rpcauth` credentials and tested method allowlists; signing keys remain client-side | Role smoke proves chain/tip/raw transaction/mempool/broadcast calls and deterministic funding. Exact-pinned `bitcoin` 0.32.101 constructs the one-leaf aggregate-key output and verified cooperative transaction, but it is not yet wired to actor RPC | Exact `RUN_ID` resources and cleanup are retained evidence. No public RPC, faucet, funds, or peers are used. Actual funding/broadcast under `Q`, MuSig2/adaptor signing, and the final dependency graph remain |
