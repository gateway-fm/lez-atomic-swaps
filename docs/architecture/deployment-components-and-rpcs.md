...
| M4 Monero output observation adapter | Exact receipt observation component-GREEN in the 25-test adapter suite; the 15-test sealed process suite proves prompt Pending for both missing and exact incoming `pool` transfers; pushed run `m7refundkill-f8bee63-d` proves that route after real daemon/actor SIGKILL | Typed `monero-rpc` 0.5.1 to distinct credential-configured literal-loopback daemon and wallet origins; fixed 30-second request timeout; public/DNS RPC rejected | Exact network/genesis, standard shared address, transaction, amount, wallet-reported availability, canonical decoded-block membership, at least ten confirmations, and stable tip. Pending is non-authorizing: after pinned genesis, a missing transfer returns promptly; an in-pool transfer must first match transaction, direction, destination, amount, and double-spend status | Pending uses height-zero hash plus exact account-0 wallet lookup. Confirmed candidates retain bracketed last headers, available outputs, daemon transaction, containing header/block, and stable-tip validation. The successful restart published revision one before mining and final revision two after exactly ten blocks without resending. Selected decoded collections are bounded | The public integration cross-binds finalized observations to the run-bound topology capability, consumes them once against Stage B, and journals them before publication. Prompt Pending removes unnecessary pre-mining deep queries but does not weaken finality. View-only spent status, upstream pre-decode bounds, discarded header trust flags, and malformed-block panic behavior remain explicit residuals. Peerless Regtest observation is supported; Stagenet/production hardening is pending |
...
| M7 joined Tag17 abandonment checkpoint | Actual-node GREEN on `m7abandon-a742c9f-a`; checked secret-free certificate retained | One isolated LEZ v0.2 Bedrock/sequencer/indexer stack plus official Monero 0.18.5.1 Regtest daemon and three wallet RPCs; all host publications are dynamic literal loopback | Independent owner-private role roots, sidecar capabilities, distinct Digest-authenticated wallet credentials, exact clean commit, and prepared transaction-ID-bound Tag17 release | Finalized deployment and Tag13; exact 1 XMR Stage-A funding with ten confirmations; pre-boundary Tag17 observation; one accepted post-boundary Tag17; Maker exact and Taker discovery finality; byte-identical post-Tag17 Monero re-observation | Runtime external resources, public RPCs, peers, faucets, public funds, and public deployment are absent. Exact cleanup passed and preserved the foreign sentinel. The result proves the disclosed penalty branch, not literal both-refund, independent key-image unspent authority, losing-branch races, or reorg immunity |
