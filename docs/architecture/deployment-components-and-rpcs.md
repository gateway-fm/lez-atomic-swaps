...
| In-process typed Zebra adapter | Agreement-bound both-direction funding discovery plus first-lock owner, owner/counterparty follow-up-claim, and refund library boundaries implemented. The production role-keyed claim/refund signer is GREEN; exact-outpoint funding planning and actor-process wiring remain | Direct bounded Zebra JSON-RPC from each reference actor; explicit nonzero-port `127.0.0.1`/`::1` HTTP only; no bridge or sidecar | Regtest may be unauthenticated loopback; self-hosted Testnet consumes bounded cookie contents into a sensitive Basic header omitted from `Debug`. The production signer fixes one role and public key, stores only zeroizing secret bytes, reconstructs and erases a short-lived secp256k1 key per canonical SDK-builder call, and can later be replaced behind the narrow traits by an HSM or OS keystore. Signed bytes and canonical node observations remain authoritative | Typed chain/block/raw/verbose/UTXO/broadcast DTOs, full-anchor stable-tip funding discovery through canonical blocks and mempool, exact owner claim/refund, observe-before-rebroadcast, CLTV maturity, and bounded counterparty spend discovery. Under-depth, moving-tip, short-horizon, unresolved, or older evidence is `Unstable`, never absent. Pending: agreement-committed exact-outpoint funding planning, durable maker-lock post-`BothLegsLocked` removal/replacement ingestion, and actor wiring | Fifty-five crate checks use authentic signed agreements and SDK/SQLite contexts and fail closed on role/key, identity, canonicality, output/outpoint/destination/fee/expiry/branch policy, ambiguity, mutation, tip/block/byte, returned-hash, scan-exhaustion, replacement, removal, and transport deviations. Three signer contracts prove byte equality with the canonical builders, foreign-key rejection, fixed role, and secret redaction; exact bytes are sent at most once on every post-send error branch |
