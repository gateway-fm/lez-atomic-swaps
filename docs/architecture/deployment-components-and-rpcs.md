...
| Official-wire LEZ sidecar | Native escrow, Maker-lock, claim, and refund paths plus finalized funding/claim/refund observers are actual-node GREEN. F7 official-token planning, all eleven authenticated v2 routes, durable replay, and finalized asset scanning are component GREEN | Actor-facing server is capability-authenticated literal loopback; outbound `local` is uncredentialed loopback HTTP and `official_public` remains exact pinned HTTPS | Capability, run, fixed role, signer, runtime, channel, genesis, program, destination, and M3 aggregate authority are checked independently of transport. F7 additionally rederives Token/ATA programs, definition, all owner/custody ATAs, and exact aggregate authority. Historical default-account absence remains distinct from RPC failure | Native Maker initialization/funding reservations restore exact bytes before bind. F7 produces ordered tag-11 initialize, permissionless tag-7 custody, tag-8 funding, tag-12 aggregate claim, and tag-10 permissionless refund bytes. The server exposes prepare/observe escrow, prepare/complete/observe claim, prepare/observe refund, and four finalized classifiers. Four v2 files restore exact escrow/claim/completion/refund results. Finalized scans check dual ID/hash blocks, ancestry, canonical bytes/hash, signers/accounts/instructions, historical metadata, fungible definition, holdings, and unchanged post-state tip | Actor-owned native Maker locks and claims completed in both run-D directions; actual-node native refunds completed in both directions in `m3refund-20260716h`. Five focused scanner regressions and all 122 sidecar tests pass; the same-height refund fork now forces `UnknownOrPending`. Actual-node F7 effects, actor composition, crash/reorg, and production trust remediation remain open |
