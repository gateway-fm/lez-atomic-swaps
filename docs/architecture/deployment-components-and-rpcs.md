...
actor or RPC. Its private release-plan and prepare/send APIs keep the internal
stable-resource encoder unwired until concrete Stage-B, finalized-LEZ,
topology, observation, and publication capabilities exist. Its 21 tests prove
only local journal invariants. They do not prove live replay prevention or a
claim PoC. The PoC deployment assumes one host, a dedicated UID, one
...
Production still needs a monotonic rollback anchor plus typed issuer,
publisher/outcome, and definitive-absence handling.
