...
| SDK-facing LEZ bridge client and adapter | Main-workspace eight-method client plus signed-agreement native first-lock preparation and observation conversion implemented; native refund and revealing-claim SDK-port composition plus actor wiring remain under ADR 0022 | Literal-IP ephemeral owner-loopback HTTP for the M2 runner; owner-restricted Unix socket preferred for production; direct client follows no redirects or proxy settings | Sensitive high-entropy capability plus exact `RUN_ID` and fixed sidecar role; full run/role/runtime echoes and one-use caller-owned request IDs are validated. The adapter rechecks signed environment/channel/genesis/program/role/signer before preparation or observation and independently invokes the SDK canonical validator; official wire decoding remains inside the sidecar | All eight typed `lez_bridge.v1.*` client calls, including native refund prepare/observe, plus exact native initialize/fund conversion and exact-owner or bounded-discovery escrow observation; method names, identity headers, and body cap are owned once by the protocol crate | Client and adapter suites cover identity, capabilities, replay, malformed/oversized bodies, signed-term mutations, 41 primitive observation mutations, remote errors, response mismatch, and ambiguous transport outcomes. Each call is attempted once; neither layer speaks or duplicates official LEZ JSON-RPC |
