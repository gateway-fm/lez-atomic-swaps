...
Status: Living executable inventory — 2026-07-14
...
| Bitcoin Core | Core 31.1 role infrastructure and a one-process public deterministic two-party MuSig2/adaptor P2TR composition are GREEN; complete swap pending | Bitcoin Core 31.1 Regtest built from the official checksum/signature/Guix-verified archive; allocated loopback RPC port, no published P2P port | Full cookie and wallet/mining RPC belong only to the run provisioner. Taker submits funding and maker submits the claim with distinct `rpcauth` credentials. The current helper owns public deterministic maker/taker key shares and adaptor scalar client-side; this is fixture authority, not production signing authority | Exact-pinned `bitcoin` 0.32.101 and `musig2` 0.4.1 aggregate the maker/taker shares, tweak to `Q`, compute role-tagged nonce commitments locally, produce both partials, verify a 65-byte adaptor presignature, adapt it with the public fixture scalar, and verify the 64-byte result under `Q`. Core mines funding at height 102 and policy/consensus accepts the one-item claim at height 103; extraction matches the adaptor point and the exact outpoint is spent once | Exact `RUN_ID` resources, critical-file hash manifest, cleanup proof, and bound attestation are retained per run. No public RPC, faucet, funds, peers, or live LEZ resource is used. Commitments are not exchanged before nonce reveal. Independent processes/stores, crash-safe nonce journals, LEZ effects, refunds, both complete directions, and atomicity remain pending; beta unaudited `musig2` is a PoC candidate |
