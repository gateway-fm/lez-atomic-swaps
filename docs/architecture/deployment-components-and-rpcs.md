...
| M3 one-shot BTC reference actor | Happy path GREEN through revision four in source, adapter tests, and 2 of 2 actual-node directions; actor-local store refund branch component GREEN | Private schema-2 configs bind dynamic loopback Core/sidecar routes, a finite 30-second LEZ read timeout, distinct sessions/journals, and full prepared claim. Taker alone names its adaptor-secret file; `status` performs no RPC | Each role has a separate database, Core Basic file, sidecar capability, runtime, and journals; contexts and signer/preparation authority are revalidated before effects and claims. The store admits maker-refund revision three only after both locks and taker-refund revision four only after durable maker-refund projection | Both roles reached happy revision 4 in both actual-node directions. Both-direction/role component tests replay the alternative branch to terminal `Refunded` and preserve old payloads through migration. Actor refund execution, public RPC/faucet, concurrency/chaos evidence, and production authority remain unclaimed |
