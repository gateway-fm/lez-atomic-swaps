...
| Bitcoin Core | M3 entry-audited; not active | Candidate Bitcoin Core 31.1 Regtest built from the official checksum/signature-verified archive; allocated loopback RPC port, no published P2P port | Full cookie and wallet/mining RPC belong only to the run provisioner. Maker and taker use separate adapter instances, distinct `rpcauth` credentials and tested method allowlists; signing keys stay client-side and no actor can access the other's wallet or mining methods | Candidate typed read/broadcast adapter: chain identity/tip, raw transaction, mempool acceptance, broadcast, funding outpoint, confirmation/removal; mining and deterministic funding only through the separate provisioner | ADR 0029 requires unique `RUN_ID`/Compose/network/volumes/root and exact-label cleanup. No public RPC or faucet in the PoC; final image digest, RPC allowlists, and dependency graph remain unaccepted until executable gates pass |
