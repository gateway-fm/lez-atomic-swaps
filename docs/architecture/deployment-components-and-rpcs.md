...
| M3 witnessed LEZ v0.2 stack | GREEN in both composed happy directions; typed finalized observer GREEN | Certified run used Bedrock `127.0.0.1:32854`, sequencer `127.0.0.1:32855`, indexer `127.0.0.1:32856`, maker sidecar `127.0.0.1:32857`, and taker sidecar `127.0.0.1:32858` | Separate capabilities, signer keys, stores, and journals. Claimant is the destination actor while the distinct aggregate x-only account is authority. Sequencer admission is never accepted as finality; either participant can query through its own sidecar, and the indexer must return the exact transaction in a finalized block by both ID and hash | Guest ELF `a199c5be...e293`, ProgramId `39b6a4db...4dec`, deployment tx `94a49583...76d3` finalized in block 405. Claims `ef77099e...cde3` and `834c67e9...3033` finalized exactly once in blocks 570 and 644; both escrows ended `claimed` with custody zero. The typed observer adds same-containing-block terminal reads and independent BIP-340 verification |
