...
flowchart TB
    ...
    subgraph DeterministicCorridor["Deterministic SDK claim corridor; no node or RPC"]
        ...
        MakerClaimState[("Maker schema-v13 state")]
        TakerClaimState[("Taker schema-v13 state")]
...
that socket registers only `zec_chat_propose_v1`, while the owner socket never
...
and signs the canonical unsigned ZEC draft, and commits the one-winner schema-v13
...
lanes. It uses two role-fixed SDK actors, two different temporary schema-v13
...
flowchart LR
    ...
    subgraph MakerProcess["Role-fixed maker reference actor process"]
        ...
        MakerState[("Maker-only schema-v13 state")]
    ...
    subgraph TakerProcess["Role-fixed taker reference actor process"]
        ...
        TakerState[("Taker-only schema-v13 state")]
...
| `lez-maker-daemon` | Running M5 application shell | Bounded HTTP/1 JSON-RPC over disjoint absolute owner-control and taker-facing Chat Unix sockets; no TCP listener | Effective-UID-owned mode-0700 runtime, mode-0600 sockets, no-clobber paths and exact-inode cleanup | Owner methods plus isolated `zec_chat_propose_v1`; daemon-owned Delivery publication/reconciliation | Operator/service-manager-owned process; caller-selected owner-private SQLite and Delivery paths; Ctrl-C shutdown; taker completion, chain watcher, and health remain |
| `lez-taker` | Running discovery-only M5 CLI | Owner-private run-local Delivery directory; no RPC or port | Taker-supplied expected compressed maker key plus trusted local time and optional exact route | Key-pinned signed offer discovery with schema-v1 JSON and retained envelope commitment | Initiation, durable agreement, status, claim, refund, and actual corridor composition remain |
...
| Chat | Maker proposal process stage GREEN; atomic final acceptance store GREEN | Separate caller-selected absolute mode-0600 Unix socket; `zec_chat_propose_v1` only | Exact Delivery-envelope authentication, canonical draft validation, pinned low-S maker signature, commit-before-response, exact replay | `ZecAgreementDraftV1`; `ZecMakerAgreementProposalV1`; schema-v13 negotiation | Taker countersign and process final acceptance pending; removable after first lock |
...
The deterministic SDK/schema-v13 corridor uses only local temporary files and
