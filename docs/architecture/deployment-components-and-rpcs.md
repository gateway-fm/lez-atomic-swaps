...
| SDK-facing LEZ bridge client | Planned under ADR 0022 | Ephemeral owner-loopback HTTP for the M2 runner; owner-restricted Unix socket preferred for production | High-entropy runner capability plus exact `RUN_ID`; canonicality still comes from actor signatures, exact identities, node facts, and SDK validation | Bounded typed requests for prepare/observe/submit; primitive exact bytes, identifiers, account/inclusion facts, and structured errors return | Serde-only protocol; never speaks or duplicates official LEZ JSON-RPC and never accepts a sidecar assertion as chain evidence |
| Official-wire LEZ sidecar | Official transaction-planner core implemented; server and node adapters pending under ADR 0022 | Planned run-scoped local listener toward the actor; official LEZ JSON-RPC client toward the selected sequencer | Implemented planner binds the full runtime, role, signer, escrow and one mutex-protected consecutive nonce pair; capability and `RUN_ID` middleware remain server work | Implemented: official native initialize/fund construction, serialization, signing, exact cached byte/ID validation, and byte-identical retry. Pending: `getAccountsNonces`, `sendTransaction`, exact-identity observation, and raw snapshot collection | Separately built exact LEZ lockfile because its stable `crypto-common ^0.2` graph cannot share the Zcash graph pinned to `=0.2.0-rc.1`; 7 locked planner tests and strict format/Clippy/rustdoc/advisory/license/source gates pass; no pin weakening or hand-copied LEZ wire types permitted |
| In-process typed Zebra adapter | First-lock library boundary implemented; actor-process wiring planned under ADR 0022 | Direct bounded Zebra JSON-RPC from each reference actor; explicit nonzero-port `127.0.0.1`/`::1` HTTP only; no bridge or sidecar | Regtest may be unauthenticated loopback; self-hosted Testnet consumes bounded cookie contents into a sensitive Basic header omitted from `Debug`; signed transactions and canonical node observations remain authoritative | Implemented: typed chain/block/raw/verbose/broadcast DTOs, stable-tip snapshots, exact V5 bytes and vout-zero binding, observe-before-rebroadcast, submit, and unknown-outcome detection. Pending: spend/removal/replacement adapters and actor corridor wiring | Lives in the Zcash actor graph and uses canonical Zcash crates; 14 committed tests use SDK-built signed V5 transactions and fail closed on local, identity, canonicality, byte, and transport deviations; does not transit the LEZ bridge |
