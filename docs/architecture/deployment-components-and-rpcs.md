...
| SDK-facing LEZ bridge client and adapter | Ten-method client, signed-agreement adapters, crash-safe context-owning SDK ports, witnessed prepare/complete, and historical live actor wiring for both M2 happy directions | Literal-IP run-owned loopback HTTP; fresh client per attempt; no redirects or proxy settings | Capability plus exact run/role/runtime/canonical ProgramId and caller-owned request IDs; role-local journal retains IDs, windows, ambiguous context, and the witnessed preparation/completion identity | Historical canonical forward and reverse runs crossed prepare, observe, and submit through both actors without duplicate effects. M3 client tests independently recompute the official witnessed message hash before external signing and before completion | Existing adapter and all 16 client gates remain GREEN. M3 witnessed live-node submission, actual-node refund, and recovery composition remain open |
| Official-wire LEZ sidecar | Native/revealing-claim/native-refund planners and observations, M3 aggregate-witness preparation/completion, node-RPC core, authenticated ten-method server, executable role runner, historical M2 local happy directions, and dormant exact-public client construction are implemented | Actor-facing server remains nonzero literal loopback with capability authentication. Outbound `local` is explicit uncredentialed loopback HTTP; `official_public` is only exact `https://testnet.lez.logos.co/` HTTPS for sequencer and indexer. One connection and bounded bodies; no automatic retry | Capability, exact `RUN_ID`, fixed role, signer, runtime, channel, genesis, and program are checked independently of transport. M3 additionally derives the aggregate authority from the x-only key and keeps it distinct from the claimant. Route validation rejects credentials, alternate paths/ports, query, fragment, generic domain, remote HTTP, and mixed local/public endpoints | All ten local bridge methods register. Witnessed preparation fixes the official message/hash and nonce; completion verifies the external aggregate signature and persists exact bytes before the separate submit method. Generated official RPC covers nonce, submit, health/channel, tip/block/account, bounded scans, and the required finalized-tip startup call. Local observations retain same-tip and canonical validation | Protocol 17/17 and pinned sidecar 47/47 are GREEN. Public construction was tested without I/O. Live endpoint method availability, especially indexer `getLastFinalizedBlockId`, is unknown upstream. M3 witnessed live inclusion, actual-node refund/finality reconciliation, and fault recovery remain open |
...
| Bitcoin Core and BTC signing boundary | Core 31.1 role infrastructure, a public deterministic two-party MuSig2/adaptor P2TR composition, and a separate fresh-nonce dual-domain in-memory session are GREEN; complete swap pending | Bitcoin Core 31.1 Regtest is built from the official checksum/signature/Guix-verified archive with an allocated loopback RPC port and no published P2P port. The dual-domain fixture needs no node or network | Full cookie and wallet/mining RPC belong only to the run provisioner. Taker submits funding and maker submits the claim with distinct `rpcauth` credentials. The Core helper owns public fixture secrets. The newer boundary uses separate maker/taker state objects, OS entropy, and canonical byte exchange; neither is an independent process or production authority | Exact-pinned `bitcoin` 0.32.101 and `musig2` 0.4.1 aggregate/tweak to `Q`, verify partials and a 65-byte presignature, adapt/extract, and verify the 64-byte result. Core mines funding at height 102 and the one-item claim at 103. Pushed `0177151` additionally exchanges transcript-bound commitments before nonce reveal for distinct BTC/LEZ sessions and proves both reveal orders with focused phase/message/point negatives | Exact `RUN_ID` resources, critical-file hashes, cleanup proof, and attestation bind the actual-Core run. No public RPC, faucet, funds, peers, or live LEZ resource is used. The actual-Core fixture itself does not exchange commitments; the newer in-memory fixture does. Independent processes/stores, crash-safe nonce journals, actual LEZ effects, refunds, both complete directions, and atomicity remain pending; beta unaudited `musig2` is a PoC candidate |
...
|---|---|---|---|---|
| Bitcoin Core 31.1 service mode | GREEN provisioner; composed corridor pending | One run-owned daemon, dynamic literal-loopback JSON-RPC, no published P2P port, zero peers; deterministic Regtest genesis and mature 50 BTC output | Provisioner retains cookie, wallet, mining, and funding-key authority. Maker and taker receive distinct mode-`0600` `rpcauth` curl configs with tested method matrices | `BITCOIN_CORE_E2E_MODE=service` emits the private endpoint/credential/funding manifest for external actors. It explicitly sets P2TR fixture, adaptor, LEZ composition, and atomicity claims false until the composed runner uses them |
| M3 durable MuSig2 SDK/journal | GREEN component boundary | No RPC or public resource; separate owner-only SQLite/WAL files per actor and per-session canonical byte exchange | Each role reserves a fresh BIP-327 nonce before exposing its commitment. The SDK revalidates the complete context, own and peer role-bound commitments, and secret/public nonce relation; SQLite atomically replaces the nonce with one exact replayable partial | Six focused journal tests, all 60 store tests, and all 12 BTC SDK tests pass. Plaintext nonce at rest until consumption and non-zeroizing upstream scalar internals are production nonclaims |
| M3 witnessed LEZ v0.2 sidecar | GREEN prepare/complete; live inclusion pending | Existing authenticated role-local loopback server to explicit local sequencer/indexer loopbacks. Versioned surface is now ten methods, adding `prepare_witnessed_claim` and `complete_witnessed_claim`; `submit_transaction` remains separate | Claimant/destination is the actor account; authority is the official account derived from the distinct aggregate x-only key. Preparation fixes the official Borsh message/hash and authority nonce; completion accepts and verifies only one exact external aggregate signature | Protocol 17/17, client 16/16, pinned sidecar 47/47. Restart completes without a nonce reread and exact replay is stable. New ProgramId `39b6a4db...4dec` is not yet deployed or submitted through the local sequencer |
| Composed LEZ/BTC runner | IN PROGRESS | Will retain one isolated Core service plus the run-owned Bedrock, sequencer, and indexer services; no faucet, public RPC, public funds, or public peers | Independent maker/taker processes use separate configs, keys, stores, sidecar capabilities, and node credentials. Both presignatures and refunds must be durable before the taker creates the first lock | User-level acceptance remains 0 of 2 directions. No scalar may be revealed before both locks; after first reveal, the opposite claim must be unilateral from local state and the selected nodes |
