...
flowchart LR
    ...
    Lez -->|"Reverse canonical validator pending"| MakerObserver
...
| SQLite | Running | Local file; no RPC or port | Daemon/runtime process filesystem authority; SDK adapter fixes one local role per handle | Aggregate, revision, ZEC journal, immutable binding, operator-alert list/ack APIs; schema-v5 role-local SDK agreement/open-or-closed taker intent plus taker-submission or maker-observation transition recovery | WAL, `FULL` synchronous, foreign keys, immediate transition transaction; seven SDK-adapter tests prove role isolation, rollback, torn-state rejection, exact maker replay, no maker-side taker intent, and close/reopen recovery; one process mutex remains |
| Concrete LEZ/ZEC agreement and lifecycle boundary | Running library boundary | No socket or RPC; bounded Borsh v1 bytes enter from an untrusted negotiation adapter; typed first-lock action and maker-observation ports have no selected production endpoint | Maker and taker transparent keys provide dual low-S signatures; each SDK and SQLite adapter fixes its local role; signed direction alone selects the maker's LEZ or Zcash observation port | Exact decode, cross-binding, persistence-before-activation, adversarial resume, durable exact taker first-lock intent, primitive-record revalidation, observe-before-rebroadcast, complete canonical forward-Zcash observation persistence, ordered LEZ initialize/fund, atomic projection/unknown-commit probe/replay | 16 KiB agreement and 2,000,000-byte per-submission caps; 81 SDK tests and 20 store tests pass. Maker observation remains non-authorizing; production node ports, canonical LEZ evidence, the SDK reorg journal, and later effects are remaining M2 work |
