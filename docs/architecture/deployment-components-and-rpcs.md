...
| In-process typed Zebra adapter | Agreement-bound both-direction funding discovery plus first-lock owner, owner/counterparty follow-up-claim, refund, and agreement-committed exact-outpoint funding planning implemented. The production role-keyed funding/claim/refund signer is GREEN; the thin all-trait SDK composite and actor-process wiring remain | Direct bounded Zebra JSON-RPC from each reference actor; explicit nonzero-port `127.0.0.1`/`::1` HTTP only; no bridge or sidecar | Regtest may be unauthenticated loopback; self-hosted Testnet consumes bounded cookie contents into a sensitive Basic header omitted from `Debug`. The production signer fixes one role and public key, stores only zeroizing secret bytes, reconstructs and erases a short-lived secp256k1 key per canonical SDK-builder call, and can later be replaced behind the narrow traits by an HSM or OS keystore. The funding signer is a trusted consensus-authorization capability; the planner independently revalidates complete transaction policy and shape without reimplementing Zcash script verification. Signed bytes, Zebra broadcast validation, and canonical node observations remain authoritative | Typed chain/block/raw/verbose/UTXO/broadcast DTOs, exact signed candidate-set planning from stable-tip confirmed `gettxout` results, full-anchor funding discovery through canonical blocks and mempool, exact owner claim/refund, observe-before-rebroadcast, CLTV maturity, and bounded counterparty spend discovery. Under-depth, moving-tip, short-horizon, unresolved, or older evidence is `Unstable`, never absent. Pending: durable maker-lock post-`BothLegsLocked` removal/replacement ingestion, one delegating SDK composite, and actor wiring | Sixty-two crate checks use authentic signed agreements and SDK/SQLite contexts and fail closed on role/key, candidate count/commitment, network/branch/genesis, UTXO ownership/confirmation/tip, identity, canonicality, transparent-only shape, output/outpoint/destination/change/fee/expiry policy, ambiguity, mutation, returned hash, scan exhaustion, replacement, removal, and transport deviations. Three signer contracts plus seven planner API/contract checks prove byte equality, both directions, foreign-role/key rejection, stable-tip planning, mutation rejection, and secret redaction; exact bytes are sent at most once on every post-send error branch |
