...
| SDK-facing LEZ bridge client and adapter | Fourteen-method client, signed-agreement adapters, crash-safe context-owning SDK ports, witnessed prepare/complete, distinct finalized-funding and finalized-claim observation, and historical live actor wiring for both M2 happy directions | Literal-IP run-owned loopback HTTP; fresh client per attempt; no redirects or proxy settings | Capability plus exact run/role/runtime/canonical ProgramId and caller-owned request IDs; role-local journal retains IDs, windows, ambiguous context, and the witnessed preparation/completion identity. Either correctly bound participant may observe by exact ID or discover the unique terms match without a peer ID | Historical canonical forward and reverse runs crossed prepare, observe, and submit through both actors without duplicate effects. The finalized-funding result validates the inclusive window, canonical funding facts, historical `Funded` metadata, exact custody, and role mapping. The finalized-claim result also rechecks the transcript and BIP-340 signature | Protocol 22/22 and client 2 unit plus 23 integration and 3 CLI tests are GREEN. Official transaction decoding and PDA validation stay in the pinned graph-isolated sidecar; cohesive actor agreement binding, Core-to-terminal wiring, and actual-node refund remain open |
| Official-wire LEZ sidecar | Native/revealing-claim/native-refund planners and observations, M3 aggregate-witness preparation/completion plus finalized-funding/finalized-claim observation, node-RPC core, authenticated fourteen-method server, executable role runner, historical M2 local happy directions, and dormant exact-public client construction are implemented | Actor-facing server remains nonzero literal loopback with capability authentication. Outbound `local` is explicit uncredentialed loopback HTTP; `official_public` is only exact `https://testnet.lez.logos.co/` HTTPS for sequencer and indexer. One connection and bounded bodies; no automatic retry | Capability, exact `RUN_ID`, fixed role, signer, runtime, channel, genesis, and program are checked independently of transport. M3 derives the aggregate authority from the x-only key and keeps it distinct from claimant and either observer role. Route validation rejects credentials, alternate paths/ports, query, fragment, generic domain, remote HTTP, and mixed local/public endpoints | Both finalized observers scan a fully covered bounded window, require equal `Finalized` blocks by ID/hash plus parent-hash continuity through the stable tip, support exact ID or unique derived-account discovery, canonicalize official instruction bytes, and read metadata/custody at the containing numeric `BlockId`; neither submits. Funding requires historical `Funded` state and exact custody; the pending cohesive actor must persist it before claim. Claim requires the exact transcript/signature and zero terminal custody | All 78 pinned-sidecar targets are GREEN. Both local M3 witnessed claims finalized exactly once. Historical account DTOs lack an upstream proof or atomic snapshot token. Public construction was tested without I/O; actual-node refund, cohesive recovery, and production trust remediation remain open |
