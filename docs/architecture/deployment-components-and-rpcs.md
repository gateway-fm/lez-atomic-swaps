...
| SDK-facing LEZ bridge client and adapter | Eleven-method client, signed-agreement adapters, crash-safe context-owning SDK ports, witnessed prepare/complete/finalized observation, and historical live actor wiring for both M2 happy directions | Literal-IP run-owned loopback HTTP; fresh client per attempt; no redirects or proxy settings | Capability plus exact run/role/runtime/canonical ProgramId and caller-owned request IDs; role-local journal retains IDs, windows, ambiguous context, and the witnessed preparation/completion identity. Either correctly bound participant may observe by exact ID or discover the unique terms/transcript match without a peer ID | Historical canonical forward and reverse runs crossed prepare, observe, and submit through both actors without duplicate effects. The finalized observer checks the inclusive window, terminal terms/accounts, and BIP-340 signature independently of the sidecar | Protocol 21/21 and client 2 unit plus 20 integration tests are GREEN. M3 witnessed live-node submission is GREEN in both operator-composed happy directions; Core-to-terminal actor wiring, actual-node refund, and cohesive recovery remain open |
| Official-wire LEZ sidecar | Native/revealing-claim/native-refund planners and observations, M3 aggregate-witness preparation/completion/finalized observation, node-RPC core, authenticated eleven-method server, executable role runner, historical M2 local happy directions, and dormant exact-public client construction are implemented | Actor-facing server remains nonzero literal loopback with capability authentication. Outbound `local` is explicit uncredentialed loopback HTTP; `official_public` is only exact `https://testnet.lez.logos.co/` HTTPS for sequencer and indexer. One connection and bounded bodies; no automatic retry | Capability, exact `RUN_ID`, fixed role, signer, runtime, channel, genesis, and program are checked independently of transport. M3 derives the aggregate authority from the x-only key and keeps it distinct from claimant and either observer role. Route validation rejects credentials, alternate paths/ports, query, fragment, generic domain, remote HTTP, and mixed local/public endpoints | Witnessed preparation fixes the official message/hash and nonce; completion verifies the external aggregate signature and persists exact bytes before submit. Finalized observation scans a fully covered bounded window, requires equal `Finalized` blocks by ID/hash, supports exact ID or unique pinned-program/derived-account discovery, verifies the exact transcript/signature, and reads metadata/custody at its containing numeric `BlockId`; it never submits | All 66 pinned-sidecar targets are GREEN. Both local M3 witnessed claims finalized exactly once. Historical account DTOs lack an upstream proof or atomic snapshot token. Public construction was tested without I/O; actual-node refund, cohesive recovery, and production trust remediation remain open |
...
| M3 durable MuSig2 SDK/journal | GREEN component boundary | No RPC or public resource; separate owner-only SQLite/WAL files per actor and per-session canonical byte exchange | Each role reserves a fresh BIP-327 nonce before exposing its commitment. The SDK revalidates the complete context, own and peer role-bound commitments, and secret/public nonce relation; SQLite atomically replaces the nonce with one exact replayable partial | Six focused journal tests, all 68 store tests, and all 12 BTC SDK tests pass. Plaintext nonce at rest until consumption and non-zeroizing upstream scalar internals are production nonclaims |
| M3 actor-local BTC recovery store | GREEN component; actor wiring pending | No RPC, node, Docker service, faucet, or public resource; separate owner-private SQLite/WAL per actor | Caller validates the canonical agreement and chain DTO before construction. The store binds exact acceptance and role, then uses `BEGIN IMMEDIATE`, predecessor CAS, and a versioned SHA-256 evidence chain for four exact lock/claim revisions | Eight focused and all 68 store tests cover both directions, close/reopen `Completed`, replay/conflict/rollback, typed and raw corruption, and scalar absence. Chain plus database atomicity, actual-node process-kill/refund/reorg, and authentication against a database owner rewriting all state are not claimed |
| M3 typed Bitcoin Core adapter | GREEN component; actual-node actor wiring pending | Exact literal-loopback HTTP root; one bounded concurrent request by default; finite 5-minute/16-request maxima; separate role-local Basic files. Current chain support is Core 31.1 Regtest only | Requires exact version/subversion, agreement genesis, selected isolated or network-enabled Regtest policy, unpruned synchronized tip, and synchronized `txindex` plus `txospenderindex`. Credential files must remain exact `0600`, regular, single-link, and unchanged across bounded read. The provisioner cookie/wallet/mining authority is never accepted | Eighteen tests cover typed/consensus cross-checks, stable-tip exact funding/claim, exact public witness, canonical 64 KiB scalar-free evidence, bounded HTTP, credential races, and durable one-attempt submission. Attempt identity binds txid, wtxid, and raw-byte digest; already-known/ambiguous outcomes become `Unknown`, and conflicting witness bytes cannot cause a second send. The service runner emits distinct Basic files and least-privilege `getindexinfo`; an actual actor-to-Core call and Testnet4 admission remain open |
| M3 witnessed LEZ v0.2 stack | GREEN in both composed happy directions; exact-or-peerless typed finalized observer GREEN | Certified run used Bedrock `127.0.0.1:32854`, sequencer `127.0.0.1:32855`, indexer `127.0.0.1:32856`, maker sidecar `127.0.0.1:32857`, and taker sidecar `127.0.0.1:32858` | Separate capabilities, signer keys, stores, and journals. Claimant is the destination actor while the distinct aggregate x-only account is authority. Sequencer admission is never accepted as finality; either participant can query through its own sidecar, and peerless mode accepts no counterparty transaction ID | Guest ELF `a199c5be...e293`, ProgramId `39b6a4db...4dec`, deployment tx `94a49583...76d3` finalized in block 405. Claims `ef77099e...cde3` and `834c67e9...3033` finalized exactly once in blocks 570 and 644; both escrows ended `claimed` with custody zero. The observer adds bounded unique discovery, by-ID/by-hash finality, same-containing-block terminal reads, and independent BIP-340 verification |
| Operator-composed LEZ and Bitcoin flow | GREEN private local PoC, 2 of 2 happy directions | Uses all six loopback endpoints above. Core and LEZ have different derived run IDs so each service runner's cross-project collision guard remains effective | Both chain presignatures exist before the first effect. The taker effect is observed before the maker effect. Scalar release waits for the confirmed Bitcoin lock and finalized LEZ funding; after release the opposite role completes from its store and chain nodes with no Delivery or Chat | Proves both direction-specific claim paths and point-checked scalar recovery. The executable CSV refund plan is now bound by a canonical dual-signed agreement, but that agreement plus the typed Core/LEZ evidence and recovery store are not yet one cohesive actor workflow. Refund, concurrent swap, chaos, production custody, public endpoints, and production finality policy are not proven |
