...
flowchart LR
    ...
    Client --> MakerSidecar
    Client --> TakerSidecar
    StageB["Validated Stage B"] --> ClaimAdapter["Taker claim-authorization adapter<br/>93 package tests green"]
    ClaimAdapter -->|one authenticated success| Client
    Client --> AuthMock["Authenticated literal-loopback mock<br/>component E2E only"]
    TakerSidecar --> ClaimBuilder["Official claim-authorization builder<br/>tag 14 component green"]
    XmrReservation --> ClaimBuilder
    ClaimBuilder --> AuthReservation[("Owner-only exact authorization bytes<br/>Fund nonce plus one; no submit authority")]
...
an actor or production RPC. Its public typed issuer consumes opaque finalized
Fund, prepared authorization, Monero-output, and topology capabilities before
the private release plan feeds an internal transaction-scoped publisher. The
issuer derives the exact `[finalized Fund time, signed refund time)` interval;
the publisher authenticates the expected transaction ID, elects one sender,
samples finalized LEZ time after the CAS, and records admitted, ambiguous, or
suppressed terminal outcomes. One authenticated-loopback integration test
proves public preparation and restart reload within the 32-test suite. The
finalized-clock and submission seams remain in-process, so this proves local
journal behavior rather than live replay prevention or a claim PoC. The generic
sidecar submit route remains closed. The PoC deployment assumes one host, a
dedicated UID, one
owner-private mode-`0700` directory, one mode-`0600` canonical SQLite
journal, no backup/restore or clone, and no hostile same-UID WAL/SHM race.
Production still needs a monotonic rollback anchor. The happy PoC still needs
concrete authenticated node transport and receipt checking, a genesis-bound
finalized clock, finality, and definitive-absence handling. Production also
requires a dedicated release-service UID so actors never receive signed
authorization bytes.

The LEZ sidecars register all eight authenticated additive v3 routes. The
Taker `prepare_native_xmr_escrow_v3` route checks the generated v0.2 ABI, exact
...
The same official sidecar now recomputes the NUL-terminated committed partial,
requires and revalidates the durable Fund, derives nonce `Fund + 1` without a
nonce RPC, signs generated tag 14 over metadata/depositor with the sole
depositor signer, and owner-only persists the exact authorization before return.
...
and overflow fail closed. Generic submit rejects the authorization and the
sequencer send count remains zero. The other five builders and
...
138 of 138 with strict Clippy and warning-free Rustdoc. The concrete main-process adapter separately
...
fails closed. All 93 adapter tests, 3 authenticated cases, 2 doctests, and strict
gates pass. ADR 0063 supplies the separate official ABI-validating builder, but
neither component is journal or submission authority and no node effect exists.
Actual-local-indexer execution, publication
...
| M4 sealed XMR release journal | Public opaque-evidence issuer plus internal transaction-scoped publisher component-GREEN in 32 of 32 tests; no live node authority | No production RPC or listener. The integration uses authenticated literal-loopback capability factories and one local mode-`0600` SQLite file under a dedicated-UID mode-`0700` directory; finalized clock and node submission remain in-process seams | Raw release plan and publisher stay private. The public issuer consumes Fund, authorization, Monero-output, and topology capabilities, derives the exact ID, bytes, commitments, and `[finalized Fund time, signed refund time)` interval. XChaCha20-Poly1305, domain-separated HMACs, exact binary IDs, authenticated expected publication ID, and schema-v3 constraints protect one local journal | Stable resource and later-tip observation; semantic restart preserves ciphertext; one prepared-to-started CAS; post-CAS half-open-window check; admitted only for a matching returned ID; errors or mismatches become ambiguous and a known no-send becomes suppressed; all started outcomes remain observe-only | Assumes one trusted host/process boundary, one canonical journal, no clone/backup/restore/rollback, and no hostile same-UID WAL/SHM race. Tests prove neither live replay prevention nor a claim PoC. Concrete authenticated node transport, genesis-bound finalized clock, finalized classification, definitive absence, dedicated release-service isolation, cancellation-after-CAS hardening, and an external rollback anchor remain |
