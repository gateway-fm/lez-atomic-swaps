...
The `lez-xmr-release-authority` component is deliberately not connected to an
actor or RPC. Its private release plan now feeds an internal transaction-scoped
publisher that authenticates an expected transaction ID, elects one sender,
samples finalized LEZ time after the CAS, and records admitted, ambiguous, or
suppressed terminal outcomes. Its 29 tests use only in-process clock and
submission seams, so they prove local journal behavior rather than live replay
prevention or a claim PoC. The generic sidecar submit route remains closed. The
PoC deployment assumes one host, a dedicated UID, one
...
typed issuers, concrete authenticated node transport and receipt checking,
checked-guest deadline equality, finality, and definitive-absence handling.
...
| M4 typed Stage-B claim authorization | Private-field non-`Clone` `PreparedXmrClaimAuthorizationEvidenceV3` adapter component-GREEN; package 93 of 93, authenticated matrix 3 of 3, doctests 2 of 2, and strict gates pass | One authenticated literal-loopback mock request after Taker-only preflight; no node, chain RPC, or external resource | Only `LezBridgeAdapter<BridgeClient>` can mint evidence. It re-derives exact Stage B, verifies the committed partial before wire, and binds signed channel/genesis/runtime plus client run/role/runtime | Prepare native-XMR claim authorization through the strict v3 client with exact response context and terms | ADR 0063 separately supplies the official builder. This adapter row still does not claim journal, submission, node effect, or a claim PoC |
...
| M4 sealed XMR release journal | Internal transaction-scoped publisher component-GREEN in 29 of 29 tests; externally usable release authority remains zero | No RPC, listener, chain call, or sidecar call. One local mode-`0600` SQLite file under one dedicated-UID mode-`0700` directory; finalized clock and node submission are in-process seams | Private release plan and publisher; public callers cannot construct authority. XChaCha20-Poly1305, domain-separated HMACs, exact binary IDs, authenticated expected publication ID, and schema-v3 constraints protect one local journal | Stable resource and later-tip observation; semantic restart preserves ciphertext; one prepared-to-started CAS; post-CAS half-open-window check; admitted only for a matching returned ID; errors or mismatches become ambiguous and a known no-send becomes suppressed; all started outcomes remain observe-only | Assumes one trusted host, one canonical journal, no clone/backup/restore/rollback, and no hostile same-UID WAL/SHM race. Tests prove neither live replay prevention nor a claim PoC. Typed issuer, concrete authenticated node transport, checked-guest deadline equality, finalized classification, definitive absence, cancellation-after-CAS hardening, and an external rollback anchor remain |
