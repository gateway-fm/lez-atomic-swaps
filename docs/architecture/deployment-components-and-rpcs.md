...
flowchart LR
    ...
    TakerSidecar -.-> ClaimBuilder["Official claim-authorization builder<br/>typed Unavailable"]
...
137 of 137 with strict Clippy. The concrete main-process adapter separately
...
gates pass. The official sidecar builder remains `Unavailable`, valid transaction
semantics are not locally ABI-decoded, and no journal or node effect exists.
...
| M4 typed Stage-B claim authorization | Private-field non-`Clone` `PreparedXmrClaimAuthorizationEvidenceV3` component-GREEN; adapter package 93 of 93, authenticated matrix 3 of 3, doctests 2 of 2, and strict Clippy/Rustdoc/format/diff pass | One authenticated literal-loopback mock request after Taker-only preflight; no node, chain RPC, or external resource | Only `LezBridgeAdapter<BridgeClient>` can mint evidence. It re-derives and compares exact Stage B, verifies the committed partial before wire, validates signed channel/genesis/runtime, and relies on client run/role/runtime binding. Wrong partial, Stage B, binding, run, role, or runtime makes zero calls | Prepare native-XMR claim authorization through the strict v3 client. Exact response context and terms are required; empty bytes fail strict response decoding after one call | The official sidecar builder remains typed `Unavailable`. Valid transaction semantics are trusted to the authenticated sidecar and are not independently ABI-decoded here. No release-journal wiring, actual-node effect, submission authority, or claim PoC is claimed |
