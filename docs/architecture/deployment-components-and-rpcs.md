...
| SDK-facing LEZ bridge client and adapter | Fourteen-method client, signed-agreement adapters, crash-safe context-owning SDK ports, witnessed prepare/complete, distinct finalized observers, public prepared-message validation, historical M2 actor wiring, and M3 actor funding observation through revision two | Literal-IP run-owned loopback HTTP; fresh client per attempt; no redirects or proxy settings | Capability plus exact run/role/runtime/ProgramId and caller-owned request IDs. The BTC actor derives terms from the signed agreement and uses each role own sidecar | Prepare, complete, submit, progress observation, finalized funding, and finalized claim. The public pure validator checks exact prepared bytes against the official domain-separated message hash without RPC | Funding validates the covered window, canonical transaction, historical `Funded` metadata, custody, accounts, and finalized tip before CAS. The prepared validator is GREEN in the complete 28-test client graph. Pre-funding errors are retryable unavailability, not false absence. Claim actor revisions and actual-node E2E remain open |
| Official-wire LEZ sidecar | Native/claim/refund planners and observations plus M3 witnessed preparation/completion and finalized observers are implemented | Actor-facing server is capability-authenticated literal loopback; outbound `local` is uncredentialed loopback HTTP and `official_public` remains exact pinned HTTPS | Capability, run, fixed role, signer, runtime, channel, genesis, program, and M3 aggregate authority are checked independently of transport | Finalized funding requires canonical `FundNative`, historical `Funded` state, exact custody, stable finalized ancestry, and signed-account agreement binding in either funding transition. Claim requires the exact transcript/signature and zero terminal custody | All pinned-sidecar targets are GREEN. The actor now persists both finalized funding observations through revision two, but the sidecar does not retain this prerequisite across claim methods. Historical account proof/snapshot, claim revisions three and four, refund, and production trust remediation remain open |
...
| M3 durable MuSig2 SDK/journal | GREEN component boundary | No RPC or public resource; separate owner-only SQLite/WAL files per actor and per-session canonical byte exchange | Each role reserves a fresh BIP-327 nonce before exposing its commitment. The SDK revalidates the complete context, own and peer role-bound commitments, and secret/public nonce relation; SQLite atomically replaces the nonce with one exact replayable partial. Existing-only open cannot create an empty signer store | Seven focused journal tests, all 84 store tests, and all 26 BTC SDK all-target tests pass. The focused BTC-recovery slice is 11/11. The SDK point-checks a private adaptor scalar without creating a final signature. Plaintext nonce at rest until consumption and non-zeroizing upstream scalar internals are production nonclaims |
| M3 actor-local BTC recovery store | GREEN through all four component revisions; public actor integration GREEN for predecessors zero and one | No RPC, node, Docker service, faucet, or public resource; separate owner-private SQLite/WAL per actor | Only schema-2 `activate` inserts exact agreement acceptance after full prepared-result, both role-local signer-journal validation, and taker-only adaptor-secret point checking. Empty/no-acceptance state remains not activated, conflicting/corrupt state fails closed, and drive projects typed taker- or maker-lock evidence with `BEGIN IMMEDIATE` plus predecessor CAS | Eleven focused BTC-recovery tests cover both directions, terminal replay, and concurrent predecessor handling. Twenty-four actor tests cover activation authority/offline status, strict secret-file safety/non-disclosure, both lock projections, observe-before-project, concurrent convergence, and request-identity changes. Chain plus database atomicity, claim revisions three and four, actual-node actor E2E, and malicious database-owner authentication are not claimed |
| M3 public-effect journal | GREEN component boundary; actor integration pending | No RPC or public resource. Additive owner-private SQLite table stores complete public Bitcoin or LEZ transaction bytes, their SHA-256, expected chain ID, agreement commitment, and composite role/revision authority | Only a committed `Prepared` to `Started` CAS grants one fresh RPC call. `Started` and `Unknown` are observe-only after restart. Accepted responses require the exact expected effect ID; exact observed bytes may reconcile an ambiguous state. Scalars, nonces, keys, seeds, and secret-bearing Zcash material are forbidden | Twelve focused tests cover concurrency, rollback, immutable replay, exact-ID isolation, exact-byte reconciliation, and corrupt schema/rows. Actual Core/LEZ calls and actor revisions three/four remain pending; no cross-system atomic commit is claimed |
| M3 one-shot BTC reference actor | GREEN through revision two; strict signing-material activation GREEN; claim revisions three and four and actual-node actor E2E pending | Public one-shot `activate`, `drive`, and `status` commands. The owner-private schema-2 role config binds literal-loopback Core and LEZ sidecar routes, distinct session IDs, existing role-local journals, and the full prepared claim result. Taker also names an owner-only adaptor-secret file; maker must omit it rather than null it. `status` constructs no client, opens no signer material, and performs no RPC | Before revision zero, `activate` rederives both contexts from the countersigned agreement, verifies the prepared run/claimant/request/hash plus exact local-role identities, completed phases and presignatures, and point-checks the taker scalar without producing a signature. Missing, cross-domain, incomplete, unsafe, forbidden, or changed authority fails before state creation. Each role has a separate database, Core Basic file, sidecar capability, runtime, and journals | Actor tests are 24/24, including secret-free stdout and SQLite/WAL assertions. Drives observe exact agreement-derived taker then maker funding and return before the applicable predecessor CAS. Valid concurrent revision-one or revision-two winners are reconstructed as `converged_on_existing_projection` without overwrite; other failures fail closed. The material gate must run again immediately before claim use. No cross-system atomicity, public RPC, faucet, owner submission, or live actor-node E2E is claimed |
| M3 typed Bitcoin Core adapter | GREEN component; both funding actor call paths GREEN; actual-node actor run pending | Exact literal-loopback HTTP root, bounded concurrency/timeouts, and separate role-local Basic files. Current chain support is Core 31.1 Regtest only | Exact version/genesis/readiness/indexes and stable credentials are required; the provisioner cookie, wallet, funding, and mining authority are never accepted | Eighteen adapter tests cover typed/consensus exact funding/claim and one-attempt submission. The actor selects exact funding whenever Bitcoin is the agreement-derived taker or maker chain, returns from observation, then projects. Actual Core service-mode actor evidence and Testnet4 remain open |
...
| Operator-composed LEZ and Bitcoin flow | GREEN private local PoC, 2 of 2 happy directions; public actor GREEN through both lock projections | Uses all six loopback endpoints. Core and LEZ have separate service-qualified run IDs; deterministic local Regtest and genesis/Vault funds require no faucet or public resource | Both presignatures precede effects and reveal waits for both finalized locks. The actor now owns exact taker- and maker-lock observation and the first two CAS projections, with separate role configs/stores | The existing operator composition proves both complete claim directions. The public actor does not yet submit effects or reproduce claim revisions three and four, refunds, concurrent swaps, production custody, public routes, or production finality. No cross-system atomic commit is claimed |
