...
| [0131](0131-isolate-taker-facade-on-owner-service.md) | Run the Taker facade as a separate owner-only Unix service and register only methods with real backing logic | Read RPC, private config, and process binary GREEN through `8826836`; standalone registry wiring, mutation workers, QtRO host, and QML remain |
| [0132](0132-persist-taker-initiation-admission-separately.md) | Persist one Taker initiation's public facts, private authority, and exact replay result atomically outside the Maker database | Standalone ZEC `TakerSellsLez` schema-v1 foundation GREEN at `ca10c13`; service wiring, initiate RPC, worker, actions, and chain effects remain absent |
