...
| [0168](0168-activate-maker-refund-from-finalized-evidence.md) | Select the Maker Refund branch only after exact funding and finalized role-local Tag-16 evidence | Accepted application checkpoint; fail-closed schema-3 validation, evidence import, no-branch CLI and replay-safe Refund preparation are GREEN, while the joined actual-node supervisor replay remains open |
