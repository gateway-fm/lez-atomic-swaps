...
| [0055](0055-preserve-xmr-atomicity-with-dual-reveal-branches.md) | Require two DLEQ-bound shares, delay the Maker-claim partial until canonical XMR funding, and reveal the opposite share through distinct signed LEZ claim/refund branches | Accepted; the successful Maker-claim reveal and Taker reconstructed-wallet sweep are working-tree GREEN. Signed tag-16 refund/tag-17 punishment, literal both-refund conformance, and clean replay remain |
...
| [0128](0128-enter-m6-through-current-basecamp-qml.md) | Enter M6 through signed-off clickable role prototypes, then current Basecamp QML over M5 authority | Clickable prototypes and nonvisual Maker atomic-route prerequisite implemented; owner sign-off and QML remain pending |
| [0129](0129-save-maker-local-route-atomically.md) | Commit one Maker local pair policy, exact price, and global replay result as one owner operation | Accepted and implemented at `8c6a7db`; Basecamp host wiring remains gated on prototype sign-off |
| [0130](0130-expose-a-strict-role-fixed-taker-facade.md) | Expose only seven typed role-fixed Taker methods and keep private receipts, keys, paths, and effect authority behind the service | All seven prepared-ZEC methods are service-backed; certified actual-local Claim replay is GREEN, while equivalent Refund, QML, and actor-real UI E2E remain |
| [0131](0131-isolate-taker-facade-on-owner-service.md) | Run the Taker facade as a separate owner-only Unix service and register only methods with real backing logic | Service custody, prepared acceptance, receipt-bound reads, and actual-local Claim are GREEN through `0c32200`; QtRO/QML and equivalent Refund remain |
...
| [0136](0136-project-admitted-zec-swaps-under-actor-lock.md) | Project admitted ZEC swaps from service-owned receipt authority under the actor per-swap lock | Locked reads are GREEN and extended by ADR 0137 actual-local Claim; durable rollback identity, equivalent Refund, and UI work remain |
| [0137](0137-authorize-one-taker-terminal-action-before-effects.md) | Commit one generation-fenced Taker terminal authorization before invoking the role actor, then reconcile exact replay through actor journals | Service-driven actual-local `TakerSellsLez` Claim and exact replay are GREEN through `0c32200`; equivalent Refund, UI composition, owner signoff, final gates, and the M6 tag remain pending |
| [0138](0138-pin-refund-snapshots-across-forward-finality.md) | Give local ZEC bridge calls measured headroom and accept only bounded fully verified forward-finalized descendants while returning the pinned refund clock | Timeout and finalized-refund component suites are GREEN; a fresh actual-node service Refund certificate remains pending |
| [0139](0139-bound-service-actions-above-actor-bridges.md) | Keep terminal service requests bounded but longer than the actor bridge work they invoke | Focused runner contract is GREEN at 15-second query and 40-second action budgets; fresh Refund, post-terminal replay, and Claim regression evidence remain pending |
| [0140](0140-prefer-durable-terminal-conflicts.md) | Resolve exact replay and any durable terminal winner before consulting transient actor availability | Process/no-mutation, atomic-race mapping, and object-envelope runner regressions are GREEN; a fresh actual-node Refund certificate remains pending |
| [0141](0141-certify-terminal-refund-replay.md) | Bind canonical Zcash inclusion and post-terminal exact Refund replay to unchanged LEZ and Zebra effects | Evidence wiring is contract-GREEN; ADR 0143 extends the measured outer fail-safe to 300 seconds; fresh actual-node certificate pending |
| [0142](0142-handoff-refund-control-state-to-parent.md) | Carry admitted Refund control state explicitly from the Taker-driver child shell to the effect-owning parent runner | Pending/finalized/replay/regression executable contract GREEN; fresh actual-node certificate pending |
| [0143](0143-retry-admitted-refund-reconciliation.md) | Treat only the documented dependency-unavailable envelope as a retryable reconciliation of an already admitted Refund | Exact-envelope executable contract GREEN; fresh actual-node certificate pending |
| [0144](0144-reconcile-confirmed-maker-lock-before-refund.md) | Project an already-confirmed Maker Zcash lock while normal Maker authority is suppressed, proving the observation adds no chain effect | Bounded authority, exact-call, mutation rejection, and result-binding contract GREEN; fresh actual-node Refund certificate pending |
