...
| [0177](0177-reconcile-killed-monero-refund-actors.md) | Pause only after one Monero refund send, then require a killed actor to resume through observation rather than resend | Accepted; exact replays RED/GREEN fixed the pre-stdout projection trigger and bounded actor-group quiescence, while a fully passing pushed-commit replay remains pending |
