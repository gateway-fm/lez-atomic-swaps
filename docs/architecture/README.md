...
| [0100](0100-fence-maker-actor-process-leases.md) | Fence scheduling by swap, random owner, monotonic generation, kernel lock, and sealed artifacts rather than TTL/path trust | Accepted; schema-v16 foundation, atomic ZEC acceptance, daemon-owned provisioning, expiry-independent replay, bounded per-swap authority registry, inherited held-lock recovery, sealed config consumers, physical artifact binding, exact pair semantics, and the opt-in persistent daemon supervisor GREEN; actual-node composition, concurrent disjoint live-process overlap, and systemd actor crash/restart pending |
