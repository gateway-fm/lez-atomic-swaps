...
| [0130](0130-expose-a-strict-role-fixed-taker-facade.md) | Expose only seven typed role-fixed Taker methods and keep private receipts, keys, paths, and effect authority behind the service | Contract/backend GREEN through receipt-bound list and monitor in ADR 0136; claim, refund, QML, and actor-real E2E remain |
| [0131](0131-isolate-taker-facade-on-owner-service.md) | Run the Taker facade as a separate owner-only Unix service and register only methods with real backing logic | Service custody, conditional pre-effect ZEC acceptance, and receipt-bound reads are GREEN through `e9393cf`; QtRO/QML and effect lifecycle remain |
...
| [0136](0136-project-admitted-zec-swaps-under-actor-lock.md) | Project admitted ZEC swaps from service-owned receipt authority under the actor per-swap lock | Accepted at `e9393cf` and hardened at `3307dca`; offline list/monitor and process-incarnation receipt/lock fencing are GREEN, while durable rollback identity hardening, actor driving, terminal effects, and UI work remain |
