...
flowchart TB
    ...
    XmrClaimAuthorization -.-> XmrJournal
```
...
| [0062](0062-mint-stage-b-claim-authorization-through-authenticated-client.md) | Mint a private-field non-`Clone` Stage-B claim-authorization capability only through the concrete authenticated bridge client | Accepted as a component checkpoint: the Taker-only adapter re-derives exact Stage B, verifies the committed partial and signed runtime binding before wire, makes exactly one authenticated call on success, and fails pre-wire drift with zero calls. All 93 adapter tests, 3 authenticated cases, 2 doctests, and strict gates pass. The test transport is an authenticated literal-loopback mock; the official sidecar builder, journal wiring, actual-node effect, and claim PoC remain absent |
