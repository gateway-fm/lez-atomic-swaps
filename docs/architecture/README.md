...
| [0022](0022-isolate-lez-official-wire-sidecar.md) | Keep incompatible pinned LEZ official-wire and Zcash graphs in separate processes behind a bounded local protocol | Process boundary accepted; the separately locked v0.2 official-wire bridges, generated RPC/query path, direct Zebra adapters, and independent actor wiring completed both private local claim directions. Refund/restart/reorg composition and public transports remain open |
| [0023](0023-private-local-m2-certification.md) | Certify M2 through a private fully functional actual-node corridor and defer public deployment/testnet publication | Accepted; finalized Vault Claims, checked deployment, official-wire effects, and independent maker/taker happy claims are GREEN in both directions. The PoC gate is met, but configuration-only public portability, repository completion, and later recovery hardening remain; public execution stays deferred |
| [0024](0024-source-audited-lez-v0-2-local-stack.md) | Build the source-audited LEZ v0.2 services into an isolated Bedrock-settled local stack | Architecture accepted; exact inputs/topology, isolated service startup, signed channel onboarding, finalized Vault Claims, checked deployment, and both private local corridor directions are evidenced. Cold independent rebuild reproducibility and composed recovery remain not measured/open |
