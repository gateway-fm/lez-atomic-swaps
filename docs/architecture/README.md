...
| [0130](0130-expose-a-strict-role-fixed-taker-facade.md) | Expose only seven typed role-fixed Taker methods and keep private receipts, keys, paths, and effect authority behind the service | All seven prepared-ZEC methods are service-backed; fresh actual-local Claim and Refund replay are GREEN, while QML and actor-real UI E2E remain |
| [0131](0131-isolate-taker-facade-on-owner-service.md) | Run the Taker facade as a separate owner-only Unix service and register only methods with real backing logic | Service custody, prepared acceptance, receipt-bound reads, and fresh actual-local Claim/Refund are GREEN; QtRO/QML remain |
...
| [0146](0146-pin-basecamp-builds-behind-consumer-locks.md) | Pin Basecamp QML, LGX, and loader builds behind repository-owned consumer locks and a digest-pinned Nix image | Official tutorial package, core/UI LGXs, and exact `lgpm` install GREEN; full Basecamp runtime load, upstream all-output integration evaluation, and production closure review remain disclosed, while Maker/Taker source still waits for owner prototype sign-off |
