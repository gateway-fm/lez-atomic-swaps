...
fork-safe finalized asset scans are component GREEN. Actor/journal composition
and role-owned actual-node custom-token execution remain open.
...
| [0042](0042-bind-witnessed-token-claims-to-exact-atas.md) | Bind aggregate-witness custom-token claims to one fungible definition and exact depositor, custody, and claimant ATAs in one recursive LEZ transition | Accepted through checked guest, adapters, and actor composition. Forward actual-node custom-token execution is terminal GREEN twice; reverse initialization/custody/funding and both first-lock projections are GREEN. Historical reads remain authoritative-indexer consistency, not cryptographic proof or an atomic snapshot. The terminal reverse direction and repeatable F7 packet remain open |
...
| [0047](0047-pin-finalized-windows-and-separate-test-lanes.md) | Read only a pinned requested finalized interval, tolerate monotonic descendants, and separate fast development from fresh certification lanes | Accepted and actual-node GREEN for two complete forward executions plus the reverse three-effect first lock at one-second cadence. Run W measures the cold wallet build at 2 minutes 7 seconds and the safe parallel-start ceiling near 36 seconds; terminal reverse evidence and the immutable official-wallet cache remain open |
