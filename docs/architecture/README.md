...
| [0130](0130-expose-a-strict-role-fixed-taker-facade.md) | Expose only seven typed role-fixed Taker methods and keep private receipts, keys, paths, and effect authority behind the service | Contract and real authenticated read backend GREEN; only health and offer list are registered, while the other five target methods, private resolution, QML host wiring, and actor-real E2E remain |
| [0131](0131-isolate-taker-facade-on-owner-service.md) | Run the Taker facade as a separate owner-only Unix service and register only methods with real backing logic | Read RPC and process binary GREEN through `ad088f8`; prepared context library GREEN through `28006dc`; service registry wiring, mutation workers, QtRO host, and QML remain |
| [0132](0132-persist-taker-initiation-admission-separately.md) | Persist one Taker initiation's public facts, private authority, and exact replay result atomically outside the Maker database | Standalone ZEC `TakerSellsLez` schema-v1 foundation and two-connection concurrency GREEN through `9820400`; service wiring, initiate RPC, worker, actions, and chain effects remain absent |
| [0133](0133-bind-prepared-zec-service-authority.md) | Bind a bounded static ZEC authority catalog to authenticated Delivery facts and stable private-file identities before service admission | Component-GREEN at `28006dc`; executable wiring, initiate RPC, worker revalidation, actors, and chain effects remain absent |
