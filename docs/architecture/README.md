...
| [0177](0177-reconcile-killed-monero-refund-actors.md) | Pause only after one Monero refund send, then require a killed actor to resume through observation rather than resend | Accepted; first exact replay exposed and RED/GREEN fixed a pre-stdout projection race, while the corrected pushed-commit actual-node replay remains pending |
