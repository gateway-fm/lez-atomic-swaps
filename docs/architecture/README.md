...
| [0058](0058-activate-xmr-swap-in-two-stages.md) | Derive XMR adaptor sessions from a countersigned base agreement, then countersign the exact nonce/partial activation transcript before the first LEZ lock | Accepted and source-executed: canonical Stage A/Stage B plus structural LEZ-lock/cutoff validation pass six SDK tests; trusted evidence, checked artifact/bridge, and actors remain in progress |
