...
| [0103](0103-persist-replay-safe-manual-actor-actions.md) | Bind explicit claim/refund requests to the global mutation ledger and the existing owner/generation/kernel-lock execution authority | Accepted; schema-v17 replay, conflict, fencing, atomic resolution, reopen, abandoned transfer, literal ZEC claim/recover routing, and schema-v18 progress projection GREEN. Maker/Taker RPC/CLI commands remain |
| [0104](0104-commit-actor-progress-with-fenced-resolution.md) | Commit validated secret-free actor progress in the same fenced transaction as process and manual-action resolution | Accepted; strict actual-schema BTC/ZEC supervisor projection, revision-zero activation, terminal coherence, atomic completion, failure preservation, reopen, actor-kind binding, and stale rollback GREEN. Maker RPC/CLI remain |
