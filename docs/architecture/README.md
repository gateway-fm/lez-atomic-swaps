...
| [0072](0072-derive-xmr-actor-inputs-from-validated-stage-material.md) | Derive actor protocol inputs and adaptor sessions only from canonical validated Stage-A/Stage-B material | Accepted as an M4 SDK and actor component checkpoint: validated agreements mint private-field checked claim/refund descriptors, independent provisioners bind private bundles to public packets, and the completed Taker claim journal is rebound directly to Stage B before typed tag-14 preparation without a plaintext side store. The role-fixed tag-13 actor revalidates canonical Stage A/B, stable finalized nonces, dedicated-account assumptions, and the signed funding cutoff. Stage-A/B process composition, resumable ambiguous-send reconciliation, actual-node execution, and the claim PoC remain |
| [0073](0073-compose-xmr-stage-material-through-role-journals.md) | Compose countersigned Stage-A/Stage-B material through separate role processes and durable adaptor journals without directly revealing the Taker claim partial | Accepted and in progress. Provisioning, actual-local read-only composition against official LEZ/Monero RPCs, independent Stage-A signing, role-indexed assembly, atomic complete session roots, unsigned codecs, reusable finalized facts, and the journal-to-tag-14 handoff are GREEN. Stage-B journal rounds/countersigning and actual chain execution remain; there is no external Logos blocker |
