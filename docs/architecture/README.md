...
| [0038](0038-durable-permissionless-lez-refund.md) | Durably prepare exact permissionless LEZ refund bytes before finalized actor eligibility can authorize one send | Accepted at the planner boundary. Exact unsigned ABI, strict legacy/witnessed bindings, mutation rejection, owner-only restart replay, owned-submission admission, and zero nonce calls are GREEN; finalized observer, server, actor, and actual-node execution remain |
