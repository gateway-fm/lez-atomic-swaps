...
| [0123](0123-drive-the-local-finalized-clock-with-one-sealed-effect.md) | Advance an idle local LEZ finalized clock with one terms-sealed Taker-to-Maker unit transfer while separating current finalized-tip observation from fixed effect discovery | Accepted and focused runtime GREEN after `m5xmrrefund842610ca` proved one effect plus normal Bedrock finality and exposed the fixed-window observation bug. Strict protocol/client/runtime/driver/runner contracts and complete test suites pass; fresh corrected two-devnet replay, retained evidence, and M5 certification remain open |
