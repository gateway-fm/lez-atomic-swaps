...
| [0053](0053-enter-m4-through-isolated-monero-regtest.md) | Enter M4 through one real LEZ-first Monero Regtest/LEZ v0.2 actor flow with an exact adaptor/DLEQ transcript | Accepted as the M4 local-PoC entry decision; implementation and evidence pending |
| [0054](0054-bound-xmr-share-to-lez-adaptor-point.md) | Bind one canonical Monero spend-key share to the exact LEZ secp256k1 adaptor point through a versioned cross-curve DLEQ envelope | Accepted for the M4 PoC boundary; deterministic proof, mapping, strict lint/docs, and dependency-policy gates are green, while full adaptor lifecycle and production review remain pending |
