...
| [0125](0125-daemon-driven-zec-deadline-and-cutover.md) | Bound daemon-driven ZEC effects to one inherited boot-clock cutoff across full-daemon and supervisor-only incarnations while distinguishing pre-cutoff submission from later finalization | Proposed and fresh-chain proof in progress; diagnostic `appf` is rejected because its scheduler failed despite both chain legs completing, so literal M5 remains `3/7` |
