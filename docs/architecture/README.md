...
| [0029](0029-m3-bitcoin-local-poc-entry.md) | Enter M3 through isolated Bitcoin Core Regtest and LEZ v0.2 actors with aggregate witnessed claim authorities | Accepted and active; two-direction actual-node happy paths and deterministic one-attempt timeout composition are GREEN. Pushed `8870910`, `3336b6e`, `11111dd`, and `923586b` add the typed schema-4 Maker seam, exact-idempotent journal admission, typed actor mapping/restart no-rearm, and generic current-funded proof. Live composition and actual-node admission evidence remain open. Concurrency, Testnet4 portability, and hardening remain |
...
| [0031](0031-one-shot-btc-actor-observe-before-project.md) | Use a public one-shot role-fixed actor that returns from exact chain observation before predecessor-CAS projection | Accepted and actual-node GREEN for both historical happy directions. Schema 3 remains observation-only compatibility. Schema 4 has a GREEN typed Maker seam, exact-idempotent journal, actor mapping/restart no-rearm, and current-funded proof components, while its live CLI intentionally fails closed. Fresh schema-4 actual-node admission, crash, reorg, and concurrency remain |
...
| [0039](0039-admit-first-lock-recovery-only-after-cross-chain-cutoff.md) | Admit a revision-one refund only after a signed cutoff, two fresh exact maker-lock classifications, and a fresh first-lock unspent/eligibility check | Accepted for the M3 BTC PoC; refund-side actual-node admission, exact unspent/submission ports, role-fixed plans, durable Maker authority, typed schema-4 actor seam, exact-idempotent journal admission, typed actor mapping/restart no-rearm, and generic current-funded proof are GREEN. Live adapter joins and actual-node maker-lock evidence remain |
