...
| [0029](0029-m3-bitcoin-local-poc-entry.md) | Enter M3 through isolated Bitcoin Core Regtest and LEZ v0.2 actors with aggregate witnessed claim authorities | Accepted and active; exact `f5a9caa` proves the actual-Core claim, while pushed `0177151` proves fresh-nonce commitment exchange, separate role-state objects, dual BTC/LEZ sessions, focused negatives, and both reveal orders in memory. Durable journals, independent processes, actual LEZ submission, both complete directions, and atomicity remain |
