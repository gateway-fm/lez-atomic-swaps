...
| [0042](0042-bind-witnessed-token-claims-to-exact-atas.md) | Bind aggregate-witness custom-token claims to one fungible definition and exact depositor, custody, and claimant ATAs in one recursive LEZ transition | Accepted at checked-guest/protocol/client/adapter/sidecar component boundaries. Finalized `Found` facts are anchored to their containing block and revalidated by official indexer ID/hash after bounded same-block historical reads; bounded absence retains its requested-end anchor. Historical reads remain authoritative-indexer consistency, not cryptographic proof or an atomic snapshot. Journal/actor composition and fresh actual-node F7 certification remain open |
