...
| [0126](0126-separate-xmr-effect-workflow-authority.md) | Separate role-local XMR branch and process orchestration from immutable adaptor transcripts and tag-specific one-attempt sidecar journals | Accepted through workflow v2, schema-v3 execution loading, sealed FD 197..210 custody, and receipt-v2 `lez-taker claim` invoking and reaping one Tag14 marker child. Replay is observe-only and ambiguity is sticky Unknown. No RPC, node, semantic Tag14 transaction, classifier, or reconciliation is composed; literal M5 stays `4/7` |
