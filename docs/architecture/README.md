...
| [0038](0038-durable-permissionless-lez-refund.md) | Durably prepare exact permissionless LEZ refund bytes before finalized actor eligibility can authorize one send | Accepted through authenticated prepare/restart replay. Exact unsigned ABI, strict legacy/witnessed bindings, mutation rejection, owner-only and bridge restart replay, owned-submission admission, and zero nonce calls are GREEN; finalized observer, actor, and actual-node execution remain |
