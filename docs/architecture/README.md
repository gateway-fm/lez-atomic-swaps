...
| [0130](0130-expose-a-strict-role-fixed-taker-facade.md) | Expose only seven typed role-fixed Taker methods and keep private receipts, keys, paths, and effect authority behind the service | Contract and authenticated backend GREEN; ADR 0134 registers initiation only with prepared authority, while swap-list, monitor, claim, refund, QML host wiring, and actor-real E2E remain |
| [0131](0131-isolate-taker-facade-on-owner-service.md) | Run the Taker facade as a separate owner-only Unix service and register only methods with real backing logic | Read RPC, process binary, and conditional admission are GREEN through `1664c41`; effect workers, QtRO host, and QML remain |
| [0132](0132-persist-taker-initiation-admission-separately.md) | Persist one Taker initiation's public facts, private authority, and exact replay result atomically outside the Maker database | Standalone schema-v1 and concurrency GREEN through `9820400`; service admission wired by ADR 0134, while worker, actions, and chain effects remain absent |
| [0133](0133-bind-prepared-zec-service-authority.md) | Bind a bounded static ZEC authority catalog to authenticated Delivery facts and stable private-file identities before service admission | Component-GREEN at `28006dc` and service-wired by ADR 0134; worker use-time revalidation, actors, and chain effects remain absent |
| [0134](0134-admit-taker-initiation-before-effects.md) | Resolve durable replay first, then require prepared and live-authenticated agreement before atomic local admission | Service and process GREEN at `1664c41`; returns `Initiating` generation zero only, with no ZEC execution worker or chain effect |
