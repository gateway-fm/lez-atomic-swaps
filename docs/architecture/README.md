...
| [0038](0038-durable-permissionless-lez-refund.md) | Durably prepare exact permissionless LEZ refund bytes before finalized actor eligibility can authorize one send | Accepted through authenticated prepare/restart replay and finalized witnessed observation. Exact unsigned ABI, strict bindings, stable state-only clocks, fully covered finalized exact/discovery scans, deadline enforcement, mutation rejection, owner/bridge replay, owned-submission admission, and no-submit observation are GREEN; actor and actual-node execution remain |
