...
| [0130](0130-expose-a-strict-role-fixed-taker-facade.md) | Expose only seven typed role-fixed Taker methods and keep private receipts, keys, paths, and effect authority behind the service | Contract/read backend GREEN; initiate now performs optional pre-effect ZEC acceptance through ADR 0135, while swap-list, monitor, claim, refund, QML, and actor-real E2E remain |
| [0131](0131-isolate-taker-facade-on-owner-service.md) | Run the Taker facade as a separate owner-only Unix service and register only methods with real backing logic | Service custody and conditional pre-effect ZEC acceptance are GREEN through `5536dd0`; QtRO/QML and actor lifecycle remain |
