...
| [0177](0177-reconcile-killed-monero-refund-actors.md) | Pause only after one Monero refund send, then require a killed actor to resume through observation rather than resend | Accepted; exact replays RED/GREEN fixed the pre-stdout trigger, actor-group quiescence, and pre-chain replay monitor race, while a fully passing pushed-commit replay remains pending |
