...
| [0177](0177-reconcile-killed-monero-refund-actors.md) | Pause only after one Monero refund send, then require a killed actor to resume through observation rather than resend | Accepted; four exact diagnostics drove fixes for the pre-stdout trigger, actor-group quiescence, pre-chain replay race, prompt Pending observation, and a wall-clock/hash-free recovery watchdog. Focused gates are GREEN; a fully passing pushed-commit replay remains pending |
| [0178](0178-route-custom-token-refunds-through-asset-v2.md) | Route witnessed custom-token refunds through the asset-v2 bridge while retaining native schema-4 behavior | Accepted; actor route, submit-once journal, focused tests, and two-direction runner contract are GREEN; exact actual-node certification remains active |
...
| [0180](0180-refresh-refund-observation-window.md) | Refresh refund reconciliation to the exact post-deadline finalized baseline before one actor-owned submission | Accepted; RED/GREEN runner contract passes and fresh actual-node replay is active |
| [0181](0181-preserve-exact-refund-miss-uncertainty.md) | Preserve uncertainty for exact-ID refund misses and authorize the owner only from its fixed finalized baseline | Accepted; actor and sidecar RED/GREEN suites plus strict Clippy pass; fresh replay is pending |
| [0182](0182-preserve-asset-aware-refund-effect-counts.md) | Preserve the native/custom-token durable-effect invariant after finalized refund projection | Accepted and actual-node GREEN through the post-projection gate; full two-direction replay remains pending |
| [0183](0183-use-canonical-refund-transition-evidence-names.md) | Validate terminal refund evidence with the actor's canonical MakerLeg and TakerLeg wire names | Accepted; RED/GREEN orchestration contract passes, while the fresh actual-node replay remains pending |
| [0184](0184-index-terminal-effects-by-asset-shape.md) | Select the terminal LEZ effect from the native or witnessed-token lifecycle shape in every aggregate journey validator | Accepted; exact bounded RED and focused RED/GREEN contract pass, while the fresh actual-node replay remains pending |
