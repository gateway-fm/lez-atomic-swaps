...
| [0009](0009-bitcoin-refund-path.md) | Taproot key-path cooperative claim with script-path CSV refund | Accepted; separate role actors complete both actual-node cooperative directions, and canonical BIP-342 construction plus typed Core anchor/maturity/observation/one-send/evidence are GREEN. Actor key custody, actual-node refund, restart/reorg/fee-bump, and LEZ composition remain |
...
| [0012](0012-lez-escrow-custody.md) | Split metadata PDA from authenticated-transfer custody or required custom-token ATA | Native/ATA custody and both local v0.2 happy directions are GREEN; strict refund wire and durable exact v0.2 refund preparation are GREEN, while finalized refund observation/execution and token-corridor hardening remain |
...
| [0029](0029-m3-bitcoin-local-poc-entry.md) | Enter M3 through isolated Bitcoin Core Regtest and LEZ v0.2 actors with aggregate witnessed claim authorities | Accepted and active; actual-Core consensus, durable journals, fresh independent role processes, external adaptation/extraction, the checked witnessed guest, canonical agreement, typed observers, recovery store, and fresh two-direction repository-owned revision-four actual-node E2E are GREEN. Refunds, concurrency, Testnet4 portability, and hardening remain |
...
| [0031](0031-one-shot-btc-actor-observe-before-project.md) | Use a public one-shot role-fixed actor that returns from each exact funding observation before predecessor-CAS projection | Accepted and actual-node GREEN through revision four in both roles and directions. `status` remains offline and replay adds no effects; refund, crash, reorg, and concurrency remain |
...
| [0036](0036-prove-bounded-lez-claim-absence-before-first-send.md) | Distinguish exact finalized LEZ presence from stable complete bounded absence before first-send reconciliation | Accepted and actual-node GREEN for both claim paths. Only complete stable finalized absence authorizes first send; equivalent refund evidence remains |
...
| [0038](0038-durable-permissionless-lez-refund.md) | Durably prepare exact permissionless LEZ refund bytes before finalized actor eligibility can authorize one send | Accepted through authenticated prepare/restart replay, finalized witnessed observation, and ordered durable lifecycle replay. Exact unsigned ABI, strict bindings, stable finalized observation, deadline enforcement, old-row-safe maker/taker refund evidence, and terminal `Refunded` reconstruction are GREEN; actor one-attempt and actual-node execution remain |
