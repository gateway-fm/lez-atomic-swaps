...
| [0099](0099-pause-actors-after-submission-only-in-fault-tests.md) | Pause a feature-gated real actor after an exact submitted effect and before stdout | Accepted; marker helper unit-GREEN, real submitted subprocess/composed evidence pending |
| [0100](0100-fence-maker-actor-process-leases.md) | Fence scheduling by swap, random owner, monotonic generation, kernel lock, and sealed artifacts rather than TTL/path trust | Accepted; schema-v16 foundation, atomic ZEC acceptance, daemon-owned provisioning, expiry-independent replay, bounded per-swap authority registry, authority-bearing systemd restart, inherited held-lock recovery, sealed config consumers, physical artifact binding, exact pair semantics, and one bounded supervisor cycle GREEN; long-running daemon/systemd execution, disjoint overlap, and actual-node composition pending |
