...
| [0072](0072-derive-xmr-actor-inputs-from-validated-stage-material.md) | Derive actor protocol inputs and adaptor sessions only from canonical validated Stage-A/Stage-B material | Accepted as an M4 SDK and actor component checkpoint: validated agreements mint private-field checked claim/refund descriptors, and the role-fixed Taker tag-13 actor revalidates canonical Stage A/B, stable finalized nonces, dedicated-account assumptions, and the signed funding cutoff. Its 12 actor tests plus five reusable finalized-facts tests prove stale and cutoff-crossing attempts make zero unsafe sends. Independent material composition, resumable ambiguous-send reconciliation, actual-node execution, and the claim PoC remain |
| [0073](0073-compose-xmr-stage-material-through-role-journals.md) | Compose countersigned Stage-A/Stage-B material through separate role processes and durable adaptor journals without directly revealing the Taker claim partial | Accepted as the M4 happy-PoC material route; implementation pending. The decision reuses the existing journal and runner packet protocol, defines the role-separated packet sequence and atomicity boundary, and lists the shortest missing public APIs. There is no external Logos blocker; canonical material CLI/process evidence and its handoff to actual tag-13/tag-14 execution remain |
