...
| [0123](0123-drive-the-local-finalized-clock-with-one-sealed-effect.md) | Advance an idle local LEZ finalized clock with one terms-sealed Taker-to-Maker unit transfer while preserving finalized classification as refund authority | Accepted as a contract checkpoint after diagnostic RED `m5xmrrefund8c10cd7a`; 324 Rust tests, strict lint/docs, compatibility contracts, and complete repository quality gates pass; focused live-runtime behavior remains open. Fresh two-devnet replay, retained evidence, and M5 certification remain open |
