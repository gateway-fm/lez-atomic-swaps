...
| [0009](0009-bitcoin-refund-path.md) | Taproot key-path cooperative claim with script-path CSV refund | Accepted; a one-process public deterministic two-party MuSig2/adaptor/extraction Core claim is GREEN, while independent signing sessions and refund evidence remain |
...
| [0029](0029-m3-bitcoin-local-poc-entry.md) | Enter M3 through isolated Bitcoin Core Regtest and LEZ v0.2 actors with aggregate witnessed claim authorities | Accepted and active; exact `f5a9caa` proves a one-process public deterministic BIP-327 MuSig2/adaptor claim, Core policy/consensus, and scalar extraction. Commitment exchange, nonce journals, independent actors/stores, the LEZ BTC guest, both complete directions, and atomicity remain |
