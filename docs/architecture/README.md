...
| [0076](0076-lease-exclusive-xmr-sidecar-state.md) | Hold a nonblocking exclusive descriptor-safe lease for each XMR bridge sidecar state directory before any request store or server use | Accepted as an M4 state-ownership component checkpoint. The source, focused secure-directory tests, actual bridge-binary acquisition test, strict Clippy, formatting, and warning-fatal Rustdoc are GREEN. Exact tag-13 directory adoption by the launcher, typed evidence export, actual-local tag-13-to-tag-14 replay, and production process isolation remain pending |
