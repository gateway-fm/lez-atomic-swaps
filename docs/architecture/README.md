...
| [0100](0100-fence-maker-actor-process-leases.md) | Fence scheduling by swap, random owner, monotonic generation, kernel lock, and sealed artifacts rather than TTL/path trust | Accepted; schema-v16 foundation, inherited held-lock recovery, and physical artifact binding components GREEN; acceptance handoff, pair-specific semantic config binding, supervisor, and actual-node composition pending |
