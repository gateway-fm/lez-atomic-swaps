...
| [0100](0100-fence-maker-actor-process-leases.md) | Fence scheduling by swap, random owner, monotonic generation, kernel lock, and sealed artifacts rather than TTL/path trust | Accepted; schema-v16 foundation, atomic ZEC acceptance, daemon-owned provisioning, expiry-independent exact committed replay, inherited held-lock recovery, sealed config consumers, and physical artifact binding GREEN; pair-specific semantic config binding, supervisor, actor-bearing systemd, and actual-node composition pending |
