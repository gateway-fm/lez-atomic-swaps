...
|---|---|---|
| ZEC SDK agreement/activation/locks/claims | Canonical bounded dual-signed terms, separate role stores, exact lock recovery, and direction-fixed effects reach `BothLegsLocked`; LEZ reveal then Zcash follow-up takes both independent actors to `Completed` in both directions and replays through schema-v9 SQLite with protected claim material | These commands still use deterministic contract doubles and no RPC, node, Docker, faucet, or external resource. The production LEZ bridge client and Zebra owner-claim port pass separate loopback contract suites, but are not yet wired into these independent actor roles; refunds and the composed real-node corridor remain pending |
| LEZ bridge and Zebra owner-claim contracts | One-attempt authenticated loopback client covers all six bounded bridge methods; the typed Zebra port derives and validates the exact owner follow-up claim, observes stable chain state before byte-identical rebroadcast, and preserves unknown outcomes | These are adapter contract tests against ephemeral loopback RPC mocks, not a manual end-user flow or consensus proof. The sidecar server, LEZ claim planner, counterparty spend discovery, actor processes, and composed corridor remain pending; no Docker, faucet, or public endpoint is used |
