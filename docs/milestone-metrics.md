...
| M6 | Active progressive local-functional PoC | Clickable Maker/Taker prototypes and isolated actor browser are GREEN 6/6. Atomic Maker route save, strict role-fixed Taker DTOs, owner-only service custody, replay-first registry, prepared authority, and real pre-effect ZEC acceptance are GREEN. Pushed `e9393cf` adds receipt-bound swap list and monitor: service-owned authority must match the private admission; actor status is read under the shared per-swap lock; and restart/offline proof returns the accepted swap as `NotActivated` generation zero after Delivery and Chat disappear without artifact mutation. Health reports exactly five registered methods and fixed errors remain redacted. Hardening `3307dca` rejects same-byte receipt inode replacement and live actor-lock contention, then proves exact recovery without role or chain effects. Pushed `9cf1a34` adds vanished receipt, coherent receipt/config cross-tamper, corrupt role-state, whole-list failure, and no-receipt restart coverage. Pushed `c90b21d` adds real activated-SQLite `AwaitingFirstLock` projection plus future/malformed durable-row rejection and recovery with unit chain ports. This adds no chain evidence: neither actor starts and no Zebra, LEZ, wallet, Delivery, or Chat call occurs during reads. Actor driving, claim/refund, durable receipt/state rollback-incarnation hardening, owner-signoff-gated QML/QtRO, actor-real composition, Basecamp packages, final gates, and tag remain | None | Owner directed M6 entry on 2026-08-03 |
