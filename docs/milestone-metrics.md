...
| Durable signer, public-effect, and BTC recovery-state boundaries | 7 signing-journal tests, 12 public-effect tests, 11 BTC-recovery tests, all 84 store tests, 18 actor tests, 4 fresh-process signing journeys, and 2 operator-composed live directions GREEN; 0 full-lifecycle actor integrations | Missing signer stores open existing-only. Exact complete public Bitcoin/LEZ bytes and agreement authority are durable before the one `Prepared` to `Started` send CAS; `Started` or `Unknown` never re-arm and exact observation can reconcile them. Only `activate` inserts agreement acceptance. `drive` still composes only revisions one and two, so chain plus database atomicity is not claimed. Claim revisions three and four, full process-kill actor E2E, and malicious database-owner authentication remain pending |
