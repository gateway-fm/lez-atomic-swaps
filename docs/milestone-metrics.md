...
| M6 | Active progressive local-functional PoC | Clickable Maker/Taker prototypes, static no-effect/CSP checks, and the digest-pinned isolated actor browser are GREEN 6/6. Atomic Maker route save, bounded owner RPC, reusable secure-file custody, and the strict typed Taker facade contract are GREEN through `6161e35`. Explicit owner sign-off still gates production QML; the facade service, Basecamp packages/hosts, actor-real composition, final gates, and tag remain | None | Owner directed M6 entry on 2026-08-03 |
