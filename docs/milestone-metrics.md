...
| Typed Bitcoin Core adapter | 29 of 29 all-target test executions plus 2 of 2 happy actual-node actor integrations GREEN | Exact Core 31.1 readiness, funding, claim, signed-anchor refund maturity, canonical claim/refund evidence, one-attempt submission, and exact post-send witness readback are covered. Actual-node timeout paths and Testnet4 remain |
...
| Durable signer, public-effect, and BTC recovery-state boundaries | Schema-3 actor: 49 library cases and 8 CLI integrations GREEN; happy actual-node actor: 2 of 2 directions | Deterministic `recover` covers both rev2 to 3 to 4 direction mappings, role authority, pre-deadline no-send, exact bytes before one CAS/attempt, ambiguous restart suppression, owner/nonowner finality projection, and terminal `Refunded`. Happy run-n retained unchanged effect counts after replay. Process-kill timing and malicious database-owner authentication remain pending; no chain/database atomic commit is claimed |
| Actual local timeout/refund compositions | 0 of 2 directions | The same public command and adapters are deterministic GREEN, but no fresh Core/LEZ timeout run is retained yet. Record exact transaction/block IDs, both revision-4 `Refunded` stores, replay counts, resources, flakes, and cleanup before increasing this number |
...
| Runnable manual BTC flows | Happy path actual-node evidenced; timeout/refund procedure maintained, fresh evidence pending | The guide mirrors successful bootstrap, pre-lock funding/admission/finalization, role journals, happy revisions zero through four, terminal/replay checks, and local-only inventory. Its schema-3 refund section gives both direction orders, maturity/finality steps, atomicity rationale, and exact resource/flakiness caveats without presenting run-n as refund evidence |
