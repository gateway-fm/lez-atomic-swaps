...
| Executable BTC-specific crates/components | 1 BTC crate, 1 actual-node runner, 4 fixture examples/CLIs, 1 role-local journal, and 1 independent role-runner crate | The SDK and Core runner prove exact P2TR/CSV construction and consensus. `f827dad` emits canonical BTC role sessions and verifies external signatures before final transaction bytes. `ca524ff` and `96f2a31` move commitment through aggregation, adaptation, and extraction into fresh maker/taker processes with separate journals. Seven agreement tests now cover both directions, dual signatures, bounded canonical wire and early in-memory rejection, Bitcoin genesis/confirmation policy, derived P2TR/claim drift, role ownership, and recovery scheduling. A production Core adapter and complete reference actor remain |
...
| Runnable manual BTC flows | 5 component flows plus 1 complete two-direction operator recipe | Component gates remain useful diagnostics. The complete guide now names the exact build, node, onboarding, role-sidecar, signing, confirmation/finality, witness-binding, cleanup, and no-public-resource checks used by the retained run; a single full-lifecycle SDK/reference-actor command remains production work |
