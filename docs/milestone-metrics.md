...
| XMR-specific executable crates/components | 1 crypto crate, 1 actual-node runner, 1 release verifier, 0 chain adapters/actors | `lez-xmr-swap-sdk` generates and verifies the bounded cross-curve proof and independently matches its secp point through MuSig2. The runner controls official `monerod` plus three official wallet RPC processes; protocol RPC and role processes are next |
...
| M4 dependency groups accepted | 2 PoC groups: crypto slice and official node binaries; Rust RPC/type graph pending | Pinned `sigma_fun` 0.9.0 plus postcard 1.1.3 pass advisories, bans, licenses, and sources. Official Monero 0.18.5.1 is bound to its clearsigned archive manifest, signer fingerprint, exact tag object/source commit, archive size/hash, binary hashes and version output. The first bincode design failed policy and was removed |
...
| Actual Monero topology bootstrap | 4 successful fresh runs after 1 assertion-defect run | Runs `m4-monero-poc-20260719b` and `19c` passed default mode; manual keep-mode runs `19d` and `19e` repeated the same real funding path. Timing run 19c reached fakechain height 111 with one daemon, three wallets, distinct loopback RPC credentials/stores, real two-destination funding, 10 confirmations, unlocked 10 XMR per role, height agreement and exact cleanup. Run 19a failed closed only because the runner omitted the provisioner wallet from the final height barrier; cleanup still passed |
...
| Monero cleanup evidence | 2 default successes and the failed assertion run auto-cleaned; 2 keep-mode successes cleaned manually | Run-owned containers, four tmpfs volumes, bridge and image were absent after cleanup; default runs also proved foreign-sentinel survival. Run 19e directly verified the documented sourced-manifest cleanup. No global Docker prune or unrelated-resource selection was used |
...
| Public RPCs, peers, faucets, or funds used | 0 in 4 successful Monero runtime runs | Runtime evidence records `[]`. Cold setup can require the official archive URL, pinned distroless registry object, and live Monero source-tag lookup; those are availability/flakiness inputs, not chain RPCs. Stagenet setup and self-hosted CI remain literal closure outputs, not PoC evidence |
