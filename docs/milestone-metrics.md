...
| F7 custom-token actual-node composition | Bounded RED through forward finalized funding; full packet open | Run `m3f7compose20260718s` at clean pushed `ba17e3b` repeated deployment, two Vault claims, the official eight-transaction Token/ATA fixture, Bitcoin lock, and finalized token initialize/custody/fund. Maker reached revision 2; Taker's intended v2 terms scan exposed a scanner bug where a valid earlier same-swap lifecycle step was treated as a funding conflict. No claim/refund followed and exact non-foreign cleanup passed. Focused RED-GREEN plus a terms-drift fail-closed assertion and all 127 sidecar tests now pass locally. Certification still requires a fresh exact-commit two-direction run with four terminal actors, balances, effects, replay, and cleanup; Run S is not an F7 PoC pass |
