...
| Executable BTC-specific crates/components | 3 BTC crates, 1 actual-node runner, 4 fixture examples/CLIs, 1 role-local journal, and 1 independent role-runner crate | The SDK and Core runner prove exact P2TR/CSV construction and consensus. Nine agreement tests bind both directions and centrally derive the fresh coordinator. The public `btc-reference-actor` exposes one-shot `activate`, `drive`, and `status`; its 12 focused tests cover both role activations, offline/repeatable status, private config, explicit activation, finalized-tip retention, deterministic request/window identity, observe-then-project, concurrent convergence, and fail-closed contradictions. Full lifecycle revisions one through four remain |
| Typed Bitcoin Core adapter | 18 of 18 adapter tests GREEN; revision-zero actor call path GREEN; 0 actual-node actor integrations | Exact Core 31.1/subversion, agreement Regtest genesis, readiness/index policy, stable-tip exact funding/claim, canonical evidence, and one-attempt submission are covered. The actor selects this observer only when the agreement-derived taker-funded chain is Bitcoin and projects only `FundingObservation::Ready`. Actual Core service-mode actor composition and Testnet4 admission remain open |
| LEZ BTC witnessed path | 1 checked artifact, 1 finalized deployment, 2 finalized witnessed claims, 2 typed finalized observers, and 1 revision-zero actor binding | The finalized funding observer proves canonical `FundNative`, historical `Funded` state, exact custody, and parent-linked finalized ancestry. The actor builds terms from the signed agreement, checks metadata/custody accounts, retains the complete finalized tip, and then projects predecessor zero. Normal pre-funding observer errors remain retryable/unavailable, not false absence. Claim gating remains pending in revisions one through four; upstream historical-account proof/snapshot limits remain |
| Durable signer and BTC recovery-state boundaries | 6 signing-journal tests, 11 BTC-recovery tests, all 71 store tests, 12 revision-zero actor tests, 4 fresh-process signing journeys, and 2 operator-composed live directions GREEN; 0 full-lifecycle actor integrations | Only `activate` inserts agreement acceptance. Absent or empty/no-acceptance state is not activated; corrupt/conflicting state fails closed. `status` may migrate an existing schema but performs no RPC and creates no acceptance. `drive` observes before predecessor CAS, so chain plus database atomicity is not claimed. A valid concurrent revision-one winner is reconstructed without overwrite; other projection failures fail closed. Revisions one through four, full process-kill actor E2E, and malicious database-owner authentication remain pending |
...
| Runnable manual BTC flows | 6 component flows, 1 revision-zero reference-actor recipe, and 1 complete two-direction operator recipe | The guide now builds `btc-reference-actor`, creates separate private role configs, repeats `status`, `activate`, and one bounded `drive`, and explains expected revision-zero/one output and local-only resources. The complete operator recipe remains the evidence for revisions one through four; one full-lifecycle actor workflow remains PoC work |
