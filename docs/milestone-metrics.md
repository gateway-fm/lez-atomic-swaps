...
| Structured M3 phase evidence | Outer baseline: Run AE 17m03.10s measured with 280 ms unattributed; child semantic timing and complete pinned CI GREEN, clean measurement pending | ADR 0049 fixes monotonic outer and direction phases, exact arithmetic/order, owner-private no-clobber publication, secret-safe schemas, effect-manifest binding, parent-duration containment, and main-packet path/SHA binding with before/after rehash of five files. `m3f7compose20260718ae` at `a82876d` remains the outer baseline: forward/reverse were 363.66/405.81 s, 75.2 percent combined; LEZ bootstrap 103.82 s, F7 fixture 75.40 s, and node startup 60.11 s. RED-GREEN now splits future sequential actor flows into eight semantic phases and overlap into eleven per child without changing execution. The complete pinned quality suite passes; one clean actual-node measurement remains before selecting another optimization |
