...
Semantic health remained Bitcoin-unavailable and Zcash-available across Maker
restart; the Bitcoin quote failed closed. The corridor completed both roles in
36.920 seconds, used 69 bounded drive rounds, zero same-run retries and three
deterministic Zebra blocks, and preserved confirmed Zcash funding before the
revealing LEZ claim before the confirmed Zcash claim. Checked certificate:
...
| Concurrent application swaps | 2 accepted BTC applications actual-node GREEN under 1 daemon/database; earlier two-worker process isolation remains 10 of 10 GREEN | Checked run `m7btcconc-272788c-a` restarts once before activation, proves distinct agreements, outpoints, four role states, eight journals, escrows and deadlines, holds both swaps at revision two before settlement, completes both directions and replays all four roles with zero resubmission. Arbitrary-N, same-direction and process-kill schedules remain |
| Missing-chain degradation | 1 explicit route-control process journey; 0 retained unavailable-node application compositions | Automatic pair-scoped withdrawal, rejected unhealthy quote/publication, unrelated-route continuity and restart durability are GREEN. A reproducible Bitcoin-Core-stopped/Zcash-surviving actual-node runner is contract-GREEN; execute and retain its certificate before incrementing the application-composition count or closing F1/R3 |
...
| Actual local application swaps | 11 functional happy/refund completions; 9 checked records, now including one two-application BTC concurrency certificate | Run `m7btcconc-272788c-a` adds two simultaneous accepted BTC application completions under one daemon/database in 596,940 ms with exact cleanup. Remaining all-pair lifecycle and adverse hardening stay separate |
...
## M7 joined refund progressive replay (2026-08-05)

| Measurement | Result | Interpretation |
|---|---|---|
| Exact pushed replay | `m7refund-7cd3a9c-a` at `7cd3a9c` | Certified joined Maker Refund with retained finality and exact cleanup |
| Exact pushed crash replay | `m7refundkill-f8bee63-d` at `f8bee63` | Certified daemon-then-actor kill after one durable submission, ObserveOnly Pending restart, ten-confirmation terminal refund, and exact cleanup |
| Real refund sends | 1 | Durable one-attempt authority and create-new submission receipt; observer sent no transaction |
| External confirmation blocks | 10 | Run-owned Regtest driver, separate from sender and observer |
| Wallet and daemon observation | Exact incoming unlocked output at 10 confirmations | Node behavior and wallet synchronization were correct; that earlier non-crash run did not claim daemon restart after submission |
...
| Evidence retention | GREEN | ADR 0172 published the allowlisted byte-identical receipt mode `0600`, one link, before cleanup |
| Checked packet | `docs/evidence/m7-actual-maker-refund-7cd3a9c-20260805.json` | Binds deployment, Tag13/Tag16, funding/refund, supervisor, retained receipt, phase ledger and cleanup hashes |
| Repository-controlled candidate ETA | 1 to 2 focused days | Semantic Claim, adverse/concurrency/reorg/fee matrices, demos and candidate security dossier remain; independent S12/S13 and public deployment excluded |
