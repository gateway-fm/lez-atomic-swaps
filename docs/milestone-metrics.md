...
| M7 | Repository-controlled review readiness active | Live RFP/issue/template authority is pinned; ADR 0148 separates self-owned readiness from independent attestation; the first RED-GREEN contract validates the S7 write-up, S12/S13 scope, finding policy and five complete local doc packets. Hard-requirement reconciliation, gap remediation, QA/chaos/security/production gates and immutable handoff remain in progress | None; formal review is deliberately deferred until all self-owned work is exhausted | Initial repository-controlled ETA 5-10 focused workdays, recalculated after executable hard-requirement audit; external review calendar excluded |
