...
| F7 custom-token actual-node composition | Bounded RED after both forward actors reached revision 2; full packet open | Run `m3f7compose20260718t` at clean pushed `50db397` repeated deployment, both Vault Claims, the official eight-transaction fixture, Bitcoin lock, and finalized token initialize/custody/fund at blocks 120/148/170. Maker exact observation and Taker lifecycle-aware peer discovery both reached `both_legs_locked` revision 2, proving the Run S scanner fix on actual nodes. The next evidence-only jq serializer failed before claims or reverse direction; exact non-foreign cleanup passed. The serializer is now a directly tested, validate-before-publish filter and the full pre-Docker contract is GREEN. Certification requires a fresh exact-commit two-direction terminal run followed by at least three clean custom-token executions per direction with exact balances, effects, replay, and cleanup; Run T is not an F7 PoC pass |
