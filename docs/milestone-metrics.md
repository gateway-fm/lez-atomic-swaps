...
| M3 | Progressive local PoC | Core 31.1 infrastructure, the typed P2TR/CSV library, the actual-Core MuSig2/adaptor claim, and a fresh-nonce commitment-exchanging dual-domain in-memory session boundary are GREEN; independent processes, durable nonce state, and the complete LEZ/BTC corridor remain in progress | None | Owner entered M3 on 2026-07-14 |
...
| Executable BTC-specific crates/components | 1 crate plus 1 actual-node runner and 3 fixture examples | `lez-btc-swap-sdk` constructs the exact P2TR/CSV funding and one-item claim; the standalone example and isolated runner prove ordered two-party MuSig2 aggregation, Taproot tweak parity, adaptor presignature/adaptation/extraction, Core policy/consensus, actor RPC ordering, and spent-once observation. The dual-domain example adds fresh OS nonces, exchanged transcript-bound commitments, separate role-state objects, partial verification, one-use in-memory phases, and both BTC-first/LEZ-first scalar reveal orders. Production Core adapter, independent signer processes/stores, crash-safe nonce persistence, and actual LEZ submission remain |
| Dependency groups accepted | 2 of 5 entry candidates | Core 31.1 and exact-pinned `bitcoin` 0.32.101 graphs passed their acceptance gates. The exact `musig2` 0.4.1 graph is locked, policy-gated, and exercised through Core plus focused commitment/phase/message/point negatives, but remains an unaccepted beta/unaudited candidate pending nonce durability, stronger secret handling, independent-process verification, and review; `miniscript` and `corepc` also remain unaccepted |
