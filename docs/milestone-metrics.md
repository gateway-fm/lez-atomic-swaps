...
| M3 | Progressive local PoC | PoC gate met by `m3actor-20260716n` at commit `6ded2f9`. Run `m3refund-20260716h` adds fresh actual-node two-lock timeout/refund evidence in both directions at commit `ef5f306`: all role stores reached revision 4 `Refunded`, replay added no submission, and exact cleanup passed. No cross-system atomic commit or production-hardening completion is claimed | None | Owner entered M3 on 2026-07-14; local happy and ordered two-lock refund gates are GREEN, while absent-maker/survivor, cutoff/race, concurrency, process-kill/reorg, later phases, and any milestone tag remain pending |
...
| Completed native happy E2E executions | 4 per direction actually executed; 3 per direction retained in committed secret-safe packets | The fourth pair is `m3actor-20260716n`, whose detailed evidence remains in the owner-private `.e2e` tree and is referenced by architecture docs. Public packets cover the initial composition, schema-4 actor-owned locks, and overlapping pair. Before the M3 tag, the requested repeatability target is 5 native executions per direction, including another recorded pair and a clean overlap repeat |
| F7 custom-token actual-node composition | Three complete pairs GREEN on clean pushed commits; 3 of 3 requested repetitions per direction | Runs X (`422c72e`, 20m52s), Z (`1555749`, 19m10.95s), and AA (`df7ed86`, 18m13.61s) each completed both actual-node directions at one-second cadence. Every direction ended revision 4 with exactly 2 Bitcoin and 4 LEZ effects, one Maker second lock, zero replay resubmission, zero custody, conserved total 250, and exact balances `175/75/0` or `75/175/0`; exact cleanup removed only captured resources. Run Y failed its pre-deployment guest pin and counts as no repetition. The F7 repeatability gate is closed; this is not the M3 tag |
| M3 official-wallet repeat preparation | Hardened cold 202.42 s; exact hit 10.35 s; certified Z/AA hits 10.32/7.81 s | Same host and production inputs under policy 2, input key `6607d474...ded208`, wallet `28245d5f...f96e6`, 118,659,320 bytes. Cold/hit peak RSS was 856,824/33,844 KiB, about 804 MiB lower on hit. Contract tests cover miss/hit, expected-output pin, effective legacy Cargo config, production test-var rejection, dirty source, fingerprint failure, tamper/mode/missing object, no-overwrite, and concurrent one-build publication. Runs Z and AA retained production-mode hits on exact pushed code without changing finality, effects, balances, replay, or cleanup |
...
| Concurrent actual-node startup | Behavioral GREEN; successful actual-node benchmark pending; AC diagnostic saving 16 s | Run AA's pre-change service logs measured 39 seconds Core, 58 seconds LEZ, and a 98-second sequential window. Run AB on pushed `74c58d1` proved concurrent startup but failed closed at the unparsed actor handoff. Run AC on pushed `b5bf322` completed both effect-bearing directions in 17m10.41s; its concurrent logs measured 45 seconds Core, 82 seconds LEZ, and an 82-second overlapped window, provisionally 16 seconds faster despite contention. AC then failed terminal cleanup proof because valid false/omitted ownership booleans were interpreted through `jq -e`; all exact resources were absent. RED-GREEN fixes both defects, but AB and AC are spent and neither is a certifiable benchmark. A fresh pushed run must pass cleanup before replacing the provisional measurement |
