...
| M3 | Progressive local PoC | Core 31.1 infrastructure, typed P2TR/CSV, crash-safe independent role signing, external adaptation/extraction, witnessed LEZ guest, prepare/observe/complete boundaries, and typed operator helpers are GREEN; the complete LEZ/BTC corridor remains in progress | None | Owner entered M3 on 2026-07-14 |
...
| LEZ BTC witnessed-claim path | 1 checked artifact, 1 recursive two-party lifecycle, witnessed initialize/fund/observe and claim prepare/complete paths GREEN; 0 live-node claims | ELF `a199c5be...e293` / ProgramId `39b6a4db...4dec` binds the x-only aggregate key to the official LEZ account. `79735dd` reserves/completes the externally signed claim; `3862dde` prepares exact generated initialize/fund messages; `3d7386b` validates their canonical chain facts and effects without finality overclaim. `a3da09e`/`bf5bdbd` provide typed operation and official account-mapping helpers. Exact deployment, submission, and corridor use remain |
| Durable signer-state boundary | 6 focused journal tests plus all 60 swap-store tests and 4 fresh-process role journeys GREEN; 1 PoC process integration, 0 complete reference-actor integrations | Exact session bytes and partial outboxes survive restart and concurrent retries; separate maker/taker processes reconstruct, aggregate, adapt, and extract in both chain domains. `nonce_encrypted_at_rest=false`: owner-only plaintext SQLite/WAL until consumption is PoC crash safety, not production key custody |
...
| Runnable manual BTC flows | 5 partial flows; 0 complete swaps | Focused P2TR vectors, deterministic MuSig2/adaptor, fresh-nonce dual-domain sessions, fresh-process durable role journeys, and the actual-Core BTC-leg fixture are runnable. The complete role-fixed LEZ/BTC swap remains unavailable |
