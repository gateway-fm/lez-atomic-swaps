...
| F7 custom-token actual-node composition | One direction terminal GREEN at one-second cadence; full two-direction packet open | Run `m3f7compose20260718v` on clean pushed `4b55dda` completed `taker_sells_foreign` through Bitcoin funding height 103, four finalized custom-token LEZ effects, both actors at revision 2, revealing claim, Bitcoin follow-up height 104, terminal revision 4, exact `175/75/0` balances, zero replay submission, and exact cleanup. This is the first tangible actual-node speed proof after ADR 0047 replaced the ten-second quiet-tip workaround. Reverse failed before stage-two agreement or public effect because overlap-era allocation had preassigned its anchor to the now-consumed height 104. Behavioral RED-GREEN reserves sequential anchors from the fresh stable empty-mempool tip (`[103,105]` in the claim model and arbitrary post-refund `247 -> 248`) while overlap remains atomically `[103,104]`; signed mined-height equality is unchanged. Certification still requires a fresh exact-commit two-direction terminal run followed by at least three clean custom-token executions per direction with exact balances, effects, replay, and cleanup; Runs T, U, and V are not full F7 PoC passes |
