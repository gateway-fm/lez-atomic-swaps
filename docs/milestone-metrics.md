...
| XMR-specific executable crates/components | 1 crypto/key crate, 1 pair-neutral signing leaf, 1 actual-node runner, 1 release verifier, 0 chain adapters/actors | `lez-xmr-swap-sdk` generates two bounded canonical DLEQ envelopes, independently matches both secp points through MuSig2, derives the shared Monero address, and reconstructs the same spend key in claim/refund reveal order. `lez-adaptor-signature` now owns the reusable signing lifecycle without a BTC dependency. The runner controls official `monerod` plus three official wallet RPC processes; agreement, protocol RPC and role processes are next |
...
| XMR atomicity prerequisites | 2 of 5 executable checkpoints | Both DLEQ-bound shares and symmetric Monero key reconstruction/official-wallet spend are green. Agreement-bound claim/refund adaptor sessions, post-XMR-confirmation claim-partial release, XMR-specific signed LEZ refund, punishment disposition, and composed role actors remain. The unsigned generic LEZ refund is explicitly rejected as share-recovery evidence by ADR 0055 |
