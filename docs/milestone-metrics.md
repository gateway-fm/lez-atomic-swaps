...
| Concurrent actual-node startup | Behavioral GREEN; successful actual-node benchmark pending | Run AA's pre-change service-log timestamps measured about 39 seconds Core, 58 seconds LEZ, and 98 seconds including sequential handoff. Run AB on pushed `74c58d1` proved concurrent actual-node startup and reconciled inventory but failed closed at 4m24.69s before a certified direction because actor handoff exported `ID<TAB>component` instead of the ID. RED-GREEN now enforces the byte-exact owner-private record, canonical 12/64-hex ID, ID-only handoff, and live run/scope/component checks in both processes. AB certifies no speedup and is spent; the expected 25 to 40 second saving remains a hypothesis until a fresh pushed custom-token run passes |
