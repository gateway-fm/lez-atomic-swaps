...
| F7 custom-token actual-node composition | Forward terminal GREEN twice at one-second cadence; reverse three-effect first lock GREEN; full pair open | Runs V and W on clean pushed commits each completed `taker_sells_foreign` through two Bitcoin and four finalized custom-token LEZ effects, terminal revision 4, exact `175/75/0` balances, zero replay submission, and exact cleanup. Run W proved sequential JIT anchors `[103,105]` around forward settlement height 104, then finalized reverse initialization/custody/funding at blocks 57/61/64 and projected both roles to revision 2. A native-only retry invariant stopped before the reverse Bitcoin lock; RED-GREEN now requires exactly two LEZ effects for native or three for custom token and rejects drift in either mode. Run W measured a 2 minute 7 second cold wallet build, about 36/58 seconds serialized Core/LEZ readiness, 5 minutes 32 seconds from forward stage two to terminal, and 2 minutes 39 seconds from reverse stage two to the bounded RED. Certification still requires a fresh exact-commit terminal pair followed by at least three clean custom-token executions per direction with exact balances, effects, replay, and cleanup; Runs T through W are not full F7 PoC passes |
