...
| M3 | Progressive local PoC closure | All six issue-#112 outputs are repository-complete at the private functional boundary: actual-node happy/refund/concurrent actors and F7, public durable SDK, official/independent vectors, Testnet4 route/setup contract, three private recordings and sealed bundle, and inline security/atomicity mapping. No cross-system atomic commit, public live deployment, or production-hardening completion is claimed | None | Owner entered M3 on 2026-07-14 and directed completion. Synchronized docs plus final full gates/push remain before the exact tag; cutoff-race/process-kill/reorg/fee/chaos/formal review are later owner-selected phases |
...
| D1 BTC recordings | 3 of 3 required M3 scenarios plus one sealed bundle GREEN | Happy, refund, and concurrent owner-private actual-node recordings bind evidence commit `a6eb1ad`; verifier commit `946208a` sealed mode-`0600` bundle SHA-256 `3d7d7adc...a86c7cc`. It records exact Core/LEZ versions and run identities, no public RPC/faucet/funds, and no external-network certification dependency |
...
| M3 completion tag | None | Created only after synchronized docs, the exact full repository gates, final push, and tag-commit audit are GREEN |
