...
| M6 | Active progressive local-functional PoC | Clickable Maker/Taker prototypes and the isolated actor browser are GREEN 6/6. Atomic Maker route save, strict Taker DTOs, authenticated reads, owner-only service custody, replay-first registry 12/12, and prepared-ZEC loading are GREEN. Commit `1664c41` service-wires admission: configured health reports exactly three methods; first admission requires prepared/public/live-authenticated agreement at one trusted time and commits facts, private authority, and replay atomically; restart replay precedes Delivery and survives offer removal. Focused service tests are GREEN 16/16, including service-level concurrent replay and one-winner conflict. The process-proven real ZEC acceptance and actor-provisioning path is reusable at `0afb6da` but is not service-connected. This is `Initiating` generation-zero admission only: no ZEC worker, Chat acceptance, actor, Zebra/LEZ effect, swap-list, monitor, claim, or refund exists. Explicit owner sign-off still gates production QML/QtRO; real ZEC execution, Basecamp packages, actor-real composition, final gates, and tag remain | None | Owner directed M6 entry on 2026-08-03 |
