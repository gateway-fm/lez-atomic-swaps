...
| Executable BTC-specific crates/components | 1 BTC crate, 1 actual-node runner, 4 fixture examples/CLIs, 1 role-local journal, and 1 independent role-runner crate | The SDK and Core runner prove exact P2TR/CSV construction and consensus. `f827dad` emits canonical BTC role sessions and verifies external signatures before final transaction bytes. `ca524ff` and `96f2a31` move commitment through aggregation, adaptation, and extraction into fresh maker/taker processes with separate journals. Six agreement tests now cover both directions, dual signatures, bounded canonical wire, Bitcoin genesis/confirmation policy, derived P2TR/claim drift, role ownership, and recovery scheduling. A production Core adapter and complete reference actor remain |
| LEZ BTC witnessed-claim path | 1 checked artifact, 1 finalized deployment, 2 finalized witnessed claims | ELF `a199c5be...e293` / ProgramId `39b6a4db...4dec` deployed in finalized block 405. `TakerSellsForeign` finalized init/fund/claim in blocks 540/544/570; `TakerSellsLez` finalized them in 617/620/644. Every indexer audit requires exact-once bounded membership, `Finalized`, equal by-ID/by-hash blocks, exact finalized witness bytes, and zero terminal custody |
