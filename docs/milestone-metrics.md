...
| F7 custom-token actual-node composition | Two complete pairs GREEN on clean pushed commits; 2 of 3 requested repetitions per direction | Runs X (`422c72e`, 20m52s) and Z (`1555749`, 19m10.95s) each completed both actual-node directions at one-second cadence. Every direction ended revision 4 with exactly 2 Bitcoin and 4 LEZ effects, one Maker second lock, zero replay resubmission, zero custody, conserved total 250, and exact balances `175/75/0` or `75/175/0`; exact cleanup removed only captured resources. Run Y failed its pre-deployment guest pin and counts as no repetition. One more clean pair remains before the M3 tag |
| M3 official-wallet repeat preparation | Hardened cold 202.42 s; exact hit 10.35 s; Run-Z certified hit 10.32 s | Same host and production inputs under policy 2, input key `6607d474...ded208`, wallet `28245d5f...f96e6`, 118,659,320 bytes. Cold/hit peak RSS was 856,824/33,844 KiB, about 804 MiB lower on hit. Contract tests cover miss/hit, expected-output pin, effective legacy Cargo config, production test-var rejection, dirty source, fingerprint failure, tamper/mode/missing object, no-overwrite, and concurrent one-build publication. Run Z retained the production-mode hit on exact pushed code without changing finality, effects, balances, replay, or cleanup |
