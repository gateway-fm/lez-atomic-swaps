...
| M4 | Progressive local-functional PoC | In progress: authority and entry architecture complete; deterministic DLEQ slice plus official Monero 0.18.5.1 actual-node topology, real local funding, evidence, and exact cleanup are green; adaptor lifecycle and composed swap remain | None | Repository-wide continuation entered M4 on 2026-07-19 after remote `m3-complete`; no QA/chaos/infosec/production phase implied |
...
| XMR-specific executable crates/components | 1 crypto/key crate, 1 actual-node runner, 1 release verifier, 0 chain adapters/actors | `lez-xmr-swap-sdk` generates two bounded canonical DLEQ envelopes, independently matches both secp points through MuSig2, derives the shared Monero address, and reconstructs the same spend key in claim/refund reveal order. The runner controls official `monerod` plus three official wallet RPC processes; agreement, protocol RPC and role processes are next |
