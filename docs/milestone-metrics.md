...
| M6 | Active progressive local-functional PoC | Clickable Maker/Taker prototypes and isolated actor browser are GREEN 6/6. The Maker atomic route, strict Taker service, prepared acceptance, receipt-bound reads, and terminal authorization are component-GREEN. Certification run `m6cert20260803164006` proves the service-driven local `TakerSellsLez` Claim happy path through isolated actual LEZ v0.2 and Zebra Regtest: first admission returned replay false, the empty Zebra mempool gained exactly transaction `6b65cdff...11b70`, exact durable replay returned replay true with the identical one-transaction mempool, and both actors completed in 35.100 seconds. It reused isolated LEZ run `m6lez20260803155817` and used fresh Zebra run `m6zec20260803164006`; deterministic local funds only, with no public RPC, faucet, public funds, or public deployment. Fresh pushed-commit run `m6refund8f76d87a` additionally certifies service-driven Refund on wholly fresh LEZ and Zebra stacks: LEZ refund `c43df1bb...dcf5ad` finalized exactly once at block 129 before Maker Zcash refund `db066a94...5ab470` appeared exactly once in canonical block 110; Maker, Taker, and service reached `refunded`; opposite Claim conflicted; terminal replay added no effect; completion took 211.530 seconds with no public RPC, faucet, public funds, or public deployment. Maker/Taker Basecamp `ui_qml`, QtRO hosts, actor-real UI E2E, explicit owner prototype signoff, Claim regression after the shared timeout change, final gates, and tag remain. Cross-restart rollback anchoring is deferred production hardening, not an issue-#112 PoC gate | None | Owner directed M6 entry on 2026-08-03; full M6 remains incomplete |
