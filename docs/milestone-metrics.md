...
| M4 | Progressive local-functional PoC | In progress: authority/architecture, DLEQ, pair-neutral adaptor leaf, canonical two-stage activation, focused guest source, strict v3 bridge protocol, and official Monero 0.18.5.1 topology/funding are green; checked artifact/runtime, trusted evidence, actors, and composed swap remain | None | Repository-wide continuation entered M4 on 2026-07-19 after remote `m3-complete`; no QA/chaos/infosec/production phase implied |
...
| XMR-specific executable crates/components | 1 crypto/lifecycle SDK, 1 pair-neutral signing leaf, 1 strict bridge protocol, 1 actual-node runner, 1 focused LEZ guest source/IDL, 0 chain adapters/actors | `lez-xmr-swap-sdk` validates canonical Stage A/B, DLEQ, shared address, recovery presignatures, exact v3 init, and structural LEZ-lock/cutoff candidates without treating caller data as authority. The additive v3 protocol binds eight methods and six finalized effects with 52 tests while preserving 44 legacy cases. Guest tags 13–17 are source-green; fresh checked artifact and bridge runtime remain next |
