...
| M6 | Active progressive local-functional PoC | Clickable Maker/Taker prototypes, static no-effect/CSP checks, and the digest-pinned isolated actor browser are GREEN 6/6. Atomic Maker route save, bounded/reusable owner RPC custody, strict Taker DTOs, and real authenticated read backend are GREEN through `1584b76`. Explicit owner sign-off still gates production QML; typed RPC/process, durable mutations, Basecamp packages/hosts, actor-real composition, final gates, and tag remain | None | Owner directed M6 entry on 2026-08-03 |
