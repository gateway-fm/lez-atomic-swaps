...
| XMR atomicity prerequisites | 7 of 9 executable checkpoints; first-lock preparation substep green | Both DLEQ-bound shares, symmetric Monero reconstruction/spend, focused LEZ branches, two-stage activation, fresh checked guest, strict bridge protocol/client, exact Monero receipt observation, and the zero-submit durable Taker escrow preparation substep are green. Trusted finalized LEZ sidecar evidence plus Stage-B-bound at-most-once release, then composed role actors remain. The non-cloneable observation alone cannot publish the hidden partial, and recursive checked execution is not an actual swap |
