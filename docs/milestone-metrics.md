...
| F7 custom-token actual-node composition | First complete pair GREEN on clean pushed commit; 1 of 3 requested repetitions per direction | Run `m3f7compose20260718x` on exact `422c72e` completed both actual-node directions at one-second cadence in 20 minutes 52 seconds. Each ended revision 4 with exactly 2 Bitcoin and 4 LEZ effects, one Maker second lock, zero replay resubmission, zero custody, conserved total 250, and exact balances `175/75/0` or `75/175/0`. The packet records no public RPC/faucet/funds/private disclosure; exact cleanup removed only captured resources. Runs V/W remain supporting forward/JIT-anchor RED-GREEN evidence, not full pairs. Two more clean custom-token repetitions per direction remain before the M3 tag |
| M3 official-wallet repeat preparation | Hardened cold 202.42 s; exact hit 10.35 s; 192.07 s / 94.9% saved | Same host and production inputs under policy 2, input key `6607d474...ded208`, wallet `28245d5f...f96e6`, 118,659,320 bytes. Cold/hit peak RSS was 856,824/33,844 KiB, about 804 MiB lower on hit. Contract tests cover miss/hit, expected-output pin, effective legacy Cargo config, production test-var rejection, dirty source, fingerprint failure, tamper/mode/missing object, no-overwrite, and concurrent one-build publication. Measurement is dirty-tree development performance evidence; the next clean pushed actor run must certify an integrated hit |
