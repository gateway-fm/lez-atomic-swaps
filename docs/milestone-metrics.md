...
| Concurrent actual-node startup | Behavioral GREEN; actual-node benchmark pending | Run AA's pre-change service-log timestamps measured about 39 seconds Core, 58 seconds LEZ, and 98 seconds including sequential handoff. ADR 0048 now starts both fixed launchers concurrently and proves exact registration, wait/reap, statuses, signals, component-labelled inventory, cleanup, and foreign survival. The expected 25 to 40 second saving remains a hypothesis until a fresh clean pushed custom-token run measures it |
