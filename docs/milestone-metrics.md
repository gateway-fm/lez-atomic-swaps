...
| M3 | Progressive local PoC | The operator-composed local-devnet happy path remains 2 of 2. The public role-fixed actor now has GREEN `activate`, offline `status`, and agreement-derived taker- then maker-lock `drive` composition through revisions one and two using typed Core or finalized LEZ evidence plus predecessor CAS. Claim revisions three and four and live two-direction actor E2E remain pending; no cross-system atomic commit is claimed | None | Owner entered M3 on 2026-07-14; no QA transition or completion tag requested |
...
| Durable signer and BTC recovery-state boundaries | 6 signing-journal tests, 11 BTC-recovery tests, all 71 store tests, 18 actor tests, 4 fresh-process signing journeys, and 2 operator-composed live directions GREEN; 0 full-lifecycle actor integrations | Only `activate` inserts agreement acceptance. Absent or empty/no-acceptance state is not activated; corrupt/conflicting state fails closed. `status` may migrate an existing schema but performs no RPC and creates no acceptance. `drive` observes before predecessor CAS, so chain plus database atomicity is not claimed. Valid concurrent revision-one and revision-two winners are reconstructed without overwrite; other projection failures fail closed. Claim revisions three and four, full process-kill actor E2E, and malicious database-owner authentication remain pending |
