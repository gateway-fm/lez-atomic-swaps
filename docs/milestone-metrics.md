...
| M4 | Progressive local-functional PoC | In progress: authority and entry architecture, deterministic DLEQ, pair-neutral adaptor leaf, canonical two-stage XMR activation, focused guest source, and official Monero 0.18.5.1 actual-node topology/funding are green; checked bridge, trusted evidence, actors, and composed swap remain | None | Repository-wide continuation entered M4 on 2026-07-19 after remote `m3-complete`; no QA/chaos/infosec/production phase implied |
...
| Literal M4 outputs complete | 0 of 6 | The XMR LEZ claim update and SDK are source-executed checkpoints, but neither output is complete before fresh checked integration, actual-node swap evidence, and the literal conformance/documentation requirements. U9, D1 XMR videos, and self-hosted stagenet CI remain |
| XMR-specific executable crates/components | 1 crypto/lifecycle SDK, 1 pair-neutral signing leaf, 1 actual-node runner, 1 focused LEZ guest source/IDL, 0 chain adapters/actors | `lez-xmr-swap-sdk` now validates canonical Stage A and Stage B, two bounded DLEQ envelopes, shared address, both recovery presignatures, exact v3 LEZ initialization, and structural LEZ-lock/cutoff candidates without treating caller data as authority. `lez-adaptor-signature` owns the signing lifecycle. The official-node runner controls `monerod` plus three wallet RPCs. Guest tags 13–17 are source-green; fresh checked artifact/bridge evidence remains next |
...
| M4 dependency groups accepted | 2 PoC groups: crypto/key slice and official node binaries; Rust RPC graph pending | Pinned `sigma_fun` 0.9.0, `monero` 0.22.0 and postcard 1.1.3 pass advisories, bans, licenses, sources, strict Clippy and Rustdoc. `tiny-keccak` 2.0.2 has an exact CC0-only exception rather than a broad allowance. Official Monero 0.18.5.1 is bound to its clearsigned archive manifest, signer fingerprint, exact tag object/source commit, archive size/hash, binary hashes and version output. The first bincode design failed policy and was removed |
...
| XMR atomicity prerequisites | 4 of 6 executable checkpoints | Both DLEQ-bound shares, symmetric Monero key reconstruction/official-wallet spend, focused LEZ source branches, and canonical two-stage activation are green. The guest refuses claim before exact Taker publication and refuses the generic unsigned refund. Trusted RPC-backed release, checked guest/bridge execution, and composed role actors remain; source-only branch success is not counted as an actual swap |
