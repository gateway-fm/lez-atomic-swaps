...
| M6 | Active progressive local-functional PoC | Clickable Maker/Taker prototypes and isolated actor browser are GREEN 6/6. Atomic Maker route save, strict role-fixed Taker DTOs, owner-only service custody, replay-first registry, and prepared authority are GREEN. Pushed `5536dd0` service-connects the real pre-effect ZEC happy path: admission commits first; bounded Maker Chat completes; the countersigned agreement, role-fixed Taker actor, Maker queued actor, and private receipt are durable; response is `NotActivated` generation zero. Exact restart replay revalidates full durable private authority, rejects same-byte inode drift, and succeeds from a valid receipt with Delivery and Chat offline without rewriting artifacts. Affected suites are GREEN 14/14 with strict Clippy and Rustdoc. This adds no chain evidence: neither actor starts and no Zebra or LEZ call, swap-list, monitor, claim, or refund exists. Owner signoff still gates QML/QtRO; lifecycle, actor-real composition, Basecamp packages, final gates, and tag remain | None | Owner directed M6 entry on 2026-08-03 |
