...
| M3 | Progressive local PoC | Core 31.1 infrastructure, typed P2TR/CSV and dual-domain adaptor signing, a crash-safe SDK/journal boundary, and the checked LEZ aggregate-witness guest are GREEN; independent processes and the complete LEZ/BTC corridor remain in progress | None | Owner entered M3 on 2026-07-14 |
...
| Executable BTC-specific crates/components | 1 BTC crate, 1 actual-node runner, 3 fixture examples, and 1 role-local journal | `lez-btc-swap-sdk` constructs the exact P2TR/CSV funding and one-item claim; the standalone example and isolated runner prove ordered two-party MuSig2 aggregation, Taproot tweak parity, adaptor presignature/adaptation/extraction, Core policy/consensus, actor RPC ordering, and spent-once observation. The dual-domain example adds fresh OS nonces, exchanged transcript-bound commitments, separate role-state objects, partial verification, one-use phases, and both reveal orders. `SqliteAdaptorSessionJournal` adds durable reserve-before-commit, commitment-gated reveal, atomic nonce-consume plus partial outbox, and restart/concurrent replay. The SDK bridge binds and revalidates exact persisted MuSig2 material and verifies both partials and the aggregate presignature. Production Core adapter and independent signer processes remain |
| LEZ BTC witnessed-claim path | 1 checked artifact, 1 recursive two-party lifecycle, and 1 durable official-wire sidecar path GREEN; 0 live-node submissions | ELF `a199c5be...e293` / ProgramId `39b6a4db...4dec` binds the x-only aggregate key to the official LEZ account, separates authority from claimant, transfers custody only to the claimant, and rejects one-share, wrong-message, mismatched-authority, and preimage-bypass attempts. `79735dd` adds exact message/hash reservation, external aggregate-signature completion, official verification, fresh-process replay, and separate submission. Deployment and corridor use remain |
| Durable signer-state boundary | 6 focused journal tests plus all 60 swap-store tests GREEN; 0 independent actor integrations | Exact session bytes and partial outboxes survive restart and concurrent retries without a second callback. `nonce_encrypted_at_rest=false`: owner-only plaintext SQLite/WAL until consumption is PoC crash safety, not production key custody |
| Dependency groups accepted | 2 of 5 entry candidates | Core 31.1 and exact-pinned `bitcoin` 0.32.101 graphs passed their acceptance gates. The exact `musig2` 0.4.1 graph is locked, policy-gated, exercised through Core and the real LEZ guest, and is bridged to the durable journal boundary, but remains an unaccepted beta/unaudited candidate pending stronger secret handling, independent-process verification, and review; `miniscript` and `corepc` also remain unaccepted |
| Actual local M3 node compositions | 1 actual Core BTC-leg composition; 0 complete swap compositions | Strict clean pushed-commit run `m3-musig-exact-f5a9caa` on `f5a9caa` confirmed taker funding `7393db97...54ae3f` at height 102 and maker two-party MuSig2/adaptor claim `46ba3858...4300ac` at height 103, with verified extraction, one 64-byte witness, spent-once proof, zero peers/public runtime resources, exact cleanup, and a hash-bound attestation. Both signers remain one public deterministic helper process; pinned LEZ v0.2 is not yet composed |
| Supported happy directions completed | 0 of 2 | `TakerSellsForeign` then `TakerSellsLez` against actual local nodes |
| Runnable manual BTC flows | 4 partial flows; 0 complete swaps | Focused P2TR vectors, the standalone deterministic MuSig2/adaptor example, the fresh-nonce dual-domain session example, and the combined actual-Core `TakerSellsForeign` BTC-leg fixture are runnable. The complete role-fixed LEZ/BTC swap remains unavailable |
