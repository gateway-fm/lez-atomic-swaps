...
| M4 | Progressive local-functional PoC | In progress: authority and entry architecture complete; deterministic DLEQ slice, pair-neutral adaptor leaf, and official Monero 0.18.5.1 actual-node topology, real local funding, evidence, and exact cleanup are green; M4-bound adaptor lifecycle and composed swap remain | None | Repository-wide continuation entered M4 on 2026-07-19 after remote `m3-complete`; no QA/chaos/infosec/production phase implied |
...
| XMR-specific executable crates/components | 1 crypto/key crate, 1 pair-neutral signing leaf, 1 actual-node runner, 1 focused LEZ guest source/IDL, 0 chain adapters/actors | `lez-xmr-swap-sdk` generates two bounded canonical DLEQ envelopes, derives the shared Monero address, and reconstructs the same spend key in both reveal orders. `lez-adaptor-signature` owns the signing lifecycle. The official-node runner controls `monerod` plus three wallet RPCs. The guest source now has exact on-chain claim-partial publication, claim, signed-refund, and punishment branches; checked artifact/bridge evidence and the corrected two-stage agreement remain next |
...
| XMR atomicity prerequisites | 3 of 6 executable checkpoints | Both DLEQ-bound shares, symmetric Monero key reconstruction/official-wallet spend, and focused LEZ source branches are green. The guest refuses claim before exact Taker publication and refuses the generic unsigned refund. Two-stage activation, non-bypassable RPC-backed release, checked guest/bridge execution, and composed role actors remain; source-only branch success is not counted as an actual swap |
