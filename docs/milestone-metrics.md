...
| LEZ BTC witnessed-claim path | 1 checked artifact, 1 finalized deployment, 2 finalized witnessed claims, 1 typed finalized observer | ELF `a199c5be...e293` / ProgramId `39b6a4db...4dec` deployed in finalized block 405. `TakerSellsForeign` finalized init/fund/claim in blocks 540/544/570; `TakerSellsLez` finalized them in 617/620/644. The eleven-method sidecar now performs bounded fully finalized scans, equal by-ID/by-hash checks, same-containing-BlockId metadata/custody reads, dual-participant observation, and client-side BIP-340 verification. Protocol 20/20, client 20/20, and sidecar 62/62 are GREEN. Historical account DTOs have no upstream proof/snapshot token |
| Durable signer-state boundary | 6 focused journal tests plus all 60 swap-store tests, 4 fresh-process role journeys, and 2 operator-composed live directions GREEN; 0 full-lifecycle public reference-actor integrations | Both live directions complete distinct BTC and LEZ sessions in separate maker/taker journals before the first effect. The nonrevealing role extracts from exact chain bytes and completes the opposite claim from persisted state. `nonce_encrypted_at_rest=false`: owner-only plaintext SQLite/WAL until consumption is PoC crash safety, not production key custody |
