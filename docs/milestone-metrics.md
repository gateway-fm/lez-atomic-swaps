...
| Actual local M3 node compositions | 1 actual Core BTC-leg composition; 0 complete swap compositions | Clean pushed-commit run `m3-p2tr-exact-4f7b6b3` reached heights 102/103 with taker funding, maker known-key claim, one 64-byte witness, spent-once proof, zero peers/public resources, exact cleanup, and a hash-bound attestation. Pinned LEZ v0.2 is not yet composed |
