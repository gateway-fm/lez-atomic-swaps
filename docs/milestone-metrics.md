...
| M3 | Progressive local PoC | Core 31.1 infrastructure, the typed P2TR/CSV library, and a one-process public deterministic two-party MuSig2/adaptor/extraction funding/claim fixture are GREEN; independent actors, durable nonce state, and the complete LEZ/BTC corridor remain in progress | None | Owner entered M3 on 2026-07-14 |
...
| Runnable manual BTC flows | 3 partial flows; 0 complete swaps | Focused P2TR vectors, the standalone one-process MuSig2/adaptor example, and the combined actual-Core `TakerSellsForeign` BTC-leg fixture are runnable. The complete role-fixed LEZ/BTC swap remains unavailable |
