...
| M3 | Progressive local PoC | The operator-composed local-devnet happy path is 2 of 2: exact Core 31.1 and LEZ v0.2 effects, distinct role processes/stores, dual pre-lock presignatures, both reveal orders, exact chain-signature extraction, spent-once BTC outputs, and zero terminal LEZ custody are GREEN. Secret-safe packaging and the reproducible operator guide are in progress; accepted-proposal hardening remains later work | None | Owner entered M3 on 2026-07-14; no QA transition or completion tag requested |
