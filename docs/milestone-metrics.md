...
workflow locks, prepares the exact Taker Tag14 plan, wins one durable CAS, and
invokes/reaps one hash-pinned child with FDs 197..210. The first output is
schema 3 `invoked_unreconciled` with a nonzero plan digest and false chain
finality; restart is `observe_only` with the same digest and no second child.
Spawn, wait, 30-second timeout, and nonzero ambiguity are sticky Unknown and
never rearm; the losing refund branch fails closed. The exact real
Maker-daemon/Delivery/Chat black-box case is GREEN 1/1 in 124.23 seconds with
transports withdrawn/stopped before action. The child is a marker only: no RPC,
node, semantic Tag14 transaction, evidence classifier, reconciliation, or funds
participated. Literal M5 remains 4/7.
