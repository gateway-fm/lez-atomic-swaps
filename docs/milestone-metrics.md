...
| Structured M3 phase evidence | Clean pushed Run AE: 17m03.10s measured, 280 ms unattributed; 17m09.57s through cleanup | ADR 0049 fixes monotonic outer phases, exact arithmetic/order, owner-private no-clobber publication, secret-safe schema, and path/SHA binding into the main packet with before/after rehash. `m3f7compose20260718ae` at `a82876d` passed both custom-token directions and cleanup. Forward/reverse were 363.66/405.81 s, 75.2 percent combined; LEZ bootstrap 103.82 s, F7 fixture 75.40 s, node startup 60.11 s, and all other measured phases 13.80 s. Both directions retained exact effects, balances, conservation, zero replay/custody, and no foreign cleanup. RED-GREEN now splits each future sequential direction into funding reservation, stage two, child actor flow, terminal/replay, and optional token balances; child semantic decomposition precedes the next run |
