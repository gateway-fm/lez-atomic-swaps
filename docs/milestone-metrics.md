...
| M3 | Progressive local PoC | The operator-composed local-devnet happy path is 2 of 2: exact Core 31.1 and LEZ v0.2 effects, distinct role processes/stores, dual pre-lock presignatures, both reveal orders, exact chain-signature extraction, spent-once BTC outputs, and zero terminal LEZ custody are GREEN. Secret-safe evidence, the operator guide, exact cleanup, architecture/flow diagrams, and traceability are pushed at `a58ef96`; the canonical countersigned BTC/LEZ agreement is GREEN; typed chain adapters and durable terminal reference-actor states remain | None | Owner entered M3 on 2026-07-14; no QA transition or completion tag requested |
...
| Executable BTC-specific crates/components | 2 BTC crates, 1 actual-node runner, 4 fixture examples/CLIs, 1 role-local journal, and 1 independent role-runner crate | The SDK and Core runner prove exact P2TR/CSV construction and consensus. `f827dad` emits canonical BTC role sessions and verifies external signatures before final transaction bytes. The typed Core adapter reconstructs and cross-checks funding/claim consensus bytes, enforces exact Core/Regtest/index/connectivity readiness, emits bounded canonical scalar-free evidence, and performs one durable CAS-gated submission attempt. Nine agreement tests bind both directions and the executable recovery plan; validation now centrally derives the fresh role-neutral coordinator and exact signed chain policies. A complete reference actor remains |
| Typed Bitcoin Core adapter | 18 of 18 tests GREEN; 0 actual-node actor integrations | Exact Core 31.1/subversion, agreement Regtest genesis, unpruned synced tip, isolated or networked-Regtest policy, synced `txindex` and `txospenderindex`, stable-tip observations, typed-versus-consensus transaction metrics, exact funding/claim, 64-byte public witness, canonical 64 KiB evidence, and single-attempt submission are covered. Attempt identity binds txid, wtxid, and exact raw-byte digest; already-known results become `Unknown`, conflicting witness payloads cannot cause another send, and contradictory preflight fields fail closed. Loopback Basic credentials and finite transport overrides are bounded. Actual Core service-mode composition and Testnet4 admission remain open |
| LEZ BTC witnessed path | 1 checked artifact, 1 finalized deployment, 2 finalized witnessed claims, 2 exact-or-peerless typed finalized observers | ELF `a199c5be...e293` / ProgramId `39b6a4db...4dec` deployed in finalized block 405. `TakerSellsForeign` finalized init/fund/claim in blocks 540/544/570; `TakerSellsLez` finalized them in 617/620/644. The fourteen-method sidecar separately proves canonical finalized funding with historical `Funded` state and the terminal revealing claim. Both bounded observers require equal by-ID/by-hash blocks, parent-hash continuity through the stable finalized tip, same-containing-BlockId accounts, dual-participant role binding, unique exact-or-peerless discovery, and an authenticated server-to-runtime zero-submit path. Claim observation additionally verifies BIP-340 in the client. Protocol 22/22, client 2 unit plus 23 integration and 3 CLI tests, and sidecar 78/78 are GREEN. Historical account DTOs have no upstream proof/snapshot token; the still-pending cohesive actor must bind the returned accounts to the signed agreement and persist funding before permitting claim |
| Durable signer and BTC recovery-state boundaries | 6 focused signing-journal tests, 8 focused BTC-recovery tests, all 68 swap-store tests, 4 fresh-process signing journeys, and 2 operator-composed live directions GREEN; 0 full-lifecycle public reference-actor integrations | Both live directions complete distinct BTC and LEZ signing sessions in separate maker/taker journals before the first effect. The recovery component separately binds each actor's accepted agreement and role, projects four exact public-evidence revisions with immediate transactions/CAS plus a versioned SHA-256 chain, reconstructs `Completed` offline, retains the 64-byte public witness, and stores no recovered scalar. Actor/adapter wiring is pending; chain+DB atomic commit and authentication against a malicious database owner are not claimed. `nonce_encrypted_at_rest=false`: owner-only plaintext SQLite/WAL until signing consumption is PoC crash safety, not production key custody |
...
| Actual local M3 node compositions | 1 isolated Core/LEZ service tuple; 2 complete swap compositions | Run `m3poc-live2-20260715a` uses Core 31.1 Regtest plus exact LEZ v0.2 Bedrock/sequencer/indexer, a finalized checked guest, separate role sidecars, restricted Core `rpcauth`, and separate signing journals. No public runtime resource is present |
| Supported happy directions completed | 2 of 2 | `TakerSellsForeign`: BTC lock, LEZ lock/claim reveal, recovered-scalar BTC claim. `TakerSellsLez`: LEZ lock, BTC lock/claim reveal, recovered-scalar LEZ claim. Both terminal BTC outpoints are spent once and both LEZ custody accounts are zero |
| Runnable manual BTC flows | 6 component flows plus 1 complete two-direction operator recipe | Component gates include the typed Core adapter/codec suite and remain useful diagnostics. The complete guide names the exact build, node, onboarding, role-sidecar, signing, confirmation/finality, witness-binding, cleanup, and no-public-resource checks used by the retained run; a single full-lifecycle SDK/reference-actor command remains PoC work |
...
| Architecture and repository-policy closure | GREEN | All 69 tracked Mermaid diagrams render with the repository harness; traceability, CI hardening, formatting, strict Clippy, tests, and docs gates pass. Remote-hosted CI status is not inferred from this checked-in local evidence |
