...
| LEZ BTC witnessed-claim path | 1 checked artifact, 1 finalized deployment, 2 finalized witnessed claims, 1 exact-or-peerless typed finalized observer | ELF `a199c5be...e293` / ProgramId `39b6a4db...4dec` deployed in finalized block 405. `TakerSellsForeign` finalized init/fund/claim in blocks 540/544/570; `TakerSellsLez` finalized them in 617/620/644. The eleven-method sidecar now performs bounded fully finalized scans, equal by-ID/by-hash checks, same-containing-BlockId metadata/custody reads, dual-participant observation, client-side BIP-340 verification, and an authenticated server-to-runtime zero-submit integration. Peerless mode derives the exact program/accounts from signed terms and distinguishes absence, ambiguity, and conflicting transcript without accepting a peer transaction ID. Protocol 21/21, client 2 unit plus 20 integration, and sidecar 66/66 are GREEN. Historical account DTOs have no upstream proof/snapshot token |
