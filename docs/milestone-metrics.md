...
| M4 | Progressive local-functional PoC | In progress: authority/architecture, DLEQ, pair-neutral adaptor leaf, canonical two-stage activation, focused guest source, fresh checked local artifact, strict v3 protocol/client, official Monero 0.18.5.1 topology/funding, exact Monero output observation, and a sealed 21-test release-journal storage foundation are green. The journal is private and unwired; functional LEZ sidecar/runtime, typed Stage-B issuer/publisher/outcome, live release authority, actors, and composed swap remain | None | Repository-wide continuation entered M4 on 2026-07-19 after remote `m3-complete`; no QA/chaos/infosec/production phase implied |
...
| XMR-specific executable crates/components | 1 crypto/lifecycle SDK, 1 pair-neutral signing leaf, 1 strict bridge protocol, 1 eight-call bridge client, 1 Monero observation adapter, 1 sealed release-storage foundation, 1 actual-node runner, 1 focused LEZ guest source/IDL plus checked artifact, 0 actors | `lez-xmr-swap-sdk` validates canonical Stage A/B without treating caller data as authority. The protocol preserves all 44 legacy cases; the client routes all eight v3 methods once with exact roles/echoes/coverage; the adapter observes exact canonical Monero receipt but cannot authorize release. Guest tags 13–17 execute in the fresh checked artifact; the Taker escrow route durably prepares and restart-replays exact initialize/fund bytes with zero submit. The release journal has private/unwired prepare and publication operations, so it adds no externally functional authority; six sidecar builders, classification, typed release integration, and actors remain |
| M4 v3 bridge-client contract | 8 of 8 methods; 4 of 4 new contracts; 51 of 51 package targets GREEN | Exact Maker/Taker role, run, runtime, terms, context, target, effect, and finalized-window checks pass. Invalid local inputs make zero calls; timeout and oversized responses are not retried; strict Clippy/Rustdoc/formatting pass. This is client enforcement, not sidecar or actor execution |
| Native-XMR escrow preparation | 1 of 7 builders functional; 20 of 20 scoped regressions plus final 3 of 3 XMR tests GREEN | Taker-only checked `InitializeNativeXmr` plus `FundNative`, exact PDAs/accounts/terms/signers, consecutive nonces, and atomic owner-only persistence are green. Same request after fresh planner/server is byte-identical with zero nonce reads; sequencer sends remain zero. The other six builders are `Unavailable`, classifier is `HistoryUnavailable`, and no submission, chain mutation, actual-chain `Found`, or claim PoC is claimed |
| Monero output observation boundary | 7 of 7 focused tests GREEN; non-cloneable result; release authority 0 | Typed `monero-rpc` 0.5.1 calls bind network/genesis, transaction, standard address, amount, wallet-reported availability, canonical decoded block membership, at least ten confirmations, and a stable tip. Distinct origins are required. The actor must additionally bind Stage B, consume once durably, and retain the run-bound wrong-credential 401. View-only spent status, discarded upstream header trust flags, upstream block-decode panic, and pre-decode response bounds remain explicit residuals |
| Sealed XMR release-journal foundation | 21 of 21 storage tests GREEN; externally usable prepare/publish authority 0 | Schema v2, stable immutable resource identity, authenticated later-tip observations, exact binary swap/run IDs, deterministic activation, randomized first-insert encryption, semantic restart, local CAS/ambiguity, tamper, and private-path checks pass. The stable-resource encoder and release plan are internal/unwired. One host, dedicated UID, mode-`0700` parent, mode-`0600` single journal, no clone/backup/restore, and no hostile same-UID WAL/SHM race are mandatory PoC assumptions. The tests prove neither live replay prevention nor a claim PoC; typed issuer, consuming publisher/outcome, definitive absence, and rollback anchor remain |
...
| Actual local LEZ/XMR happy swaps | 0; preparation-only effects 1 of 7 builders | The Taker escrow planner prepares durable exact bytes with zero submission or chain mutation. First actual target is one `TakerSellsLez` run through independent Maker/Taker processes, actual LEZ v0.2 and Monero 0.18.5.1 Regtest effects, exact adaptor extraction, DLEQ verification, and reconstructed Monero spend |
...
| XMR atomicity prerequisites | 7 of 9 executable checkpoints; first-lock preparation and sealed storage substeps green | Both DLEQ-bound shares, symmetric Monero reconstruction/spend, focused LEZ branches, two-stage activation, fresh checked guest, strict bridge protocol/client, exact Monero receipt observation, the zero-submit durable Taker escrow preparation substep, and isolated release-journal storage invariants are green. Trusted finalized LEZ sidecar evidence plus typed Stage-B issuer/publisher/outcome integration, then composed role actors remain. The sealed journal is unwired and does not close the live release checkpoint; neither it nor the non-cloneable observation can publish the hidden partial, and recursive checked execution is not an actual swap |
