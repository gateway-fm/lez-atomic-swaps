...
| M3 | Progressive local PoC | The operator-composed local-devnet happy path remains 2 of 2. The public role-fixed actor now has strict schema-2 activation gated by the full agreement-bound prepared claim plus both completed signer journals, offline `status`, and agreement-derived taker- then maker-lock `drive` composition through revisions one and two using typed Core or finalized LEZ evidence plus predecessor CAS. Claim revisions three and four and live two-direction actor E2E remain pending; no cross-system atomic commit is claimed | None | Owner entered M3 on 2026-07-14; no QA transition or completion tag requested |
...
| Executable BTC-specific crates/components | 3 BTC crates, 1 actual-node runner, 4 fixture examples/CLIs, 1 role-local journal, and 1 independent role-runner crate | The SDK and Core runner prove exact P2TR/CSV construction and consensus. Ten agreement tests bind both directions, centrally derive the fresh coordinator, and reconstruct both domain-separated adaptor contexts from signed terms plus fresh session IDs. The public `btc-reference-actor` exposes one-shot `activate`, `drive`, and `status`; its 24 focused tests additionally reject schema-1 migration, absent/cross-domain signer authority, run/claimant/request drift, invalid prepared hashes, internally valid messages not signed by the agreement, and missing, unsafe, or point-mismatched taker secrets before state creation. Maker configs cannot name or explicitly null the taker secret. Fresh-process output and SQLite/WAL scans reject scalar disclosure. Both role activations, offline/repeatable status, both directions and roles through revision two, finalized-tip retention, deterministic request/window identity, observe-then-project, exact replay, concurrent convergence, and fail-closed contradictions remain GREEN. Claim revisions three and four remain |
| Typed Bitcoin Core adapter | 18 of 18 adapter tests GREEN; both actor funding-transition call paths GREEN; 0 actual-node actor integrations | Exact Core 31.1/subversion, agreement Regtest genesis, readiness/index policy, stable-tip exact funding/claim, canonical evidence, and one-attempt submission are covered. The actor selects this observer whenever the agreement-derived taker- or maker-funded chain is Bitcoin and projects only `FundingObservation::Ready`. Actual Core service-mode actor composition and Testnet4 admission remain open |
| LEZ BTC witnessed path | 1 checked artifact, 1 finalized deployment, 2 finalized witnessed claims, 2 typed finalized observers, and 2 actor funding-transition bindings | The finalized funding observer proves canonical `FundNative`, historical `Funded` state, exact custody, and parent-linked finalized ancestry. The actor builds terms from the signed agreement, checks metadata/custody accounts, retains the complete finalized tip, and projects the applicable taker or maker funding predecessor. Normal pre-funding observer errors remain retryable/unavailable, not false absence. Claim gating remains pending in revisions three and four; upstream historical-account proof/snapshot limits remain |
| Durable signer, public-effect, and BTC recovery-state boundaries | 7 signing-journal tests, 12 public-effect tests, 11 BTC-recovery tests, all 84 store tests, 24 actor tests, 4 fresh-process signing journeys, and 2 operator-composed live directions GREEN; 0 full-lifecycle actor integrations | Missing signer stores open existing-only. Schema-2 activation rederives both contexts from the agreement, requires exact local-role completed journals, the full bound prepared result, and the point-matching taker-only secret, and refuses state creation on drift. Exact complete public Bitcoin/LEZ bytes and agreement authority are durable before the one `Prepared` to `Started` send CAS; `Started` or `Unknown` never re-arm and exact observation can reconcile them. `drive` still composes only revisions one and two, so chain plus database atomicity is not claimed. Claim revisions three and four, full process-kill actor E2E, and malicious database-owner authentication remain pending |
...
| Runnable manual BTC flows | 6 component flows, 1 two-lock reference-actor recipe, and 1 complete two-direction operator recipe | The guide now builds `btc-reference-actor`, creates strict schema-2 private role configs only after both presignatures and the full prepared claim exist, repeats `status`, `activate`, and bounded `drive` for the taker and maker locks, and explains expected revisions zero through two and local-only resources. The complete operator recipe remains the evidence for claim revisions three and four; one full-lifecycle actor workflow remains PoC work |
