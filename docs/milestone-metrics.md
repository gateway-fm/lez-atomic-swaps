...
| F7 custom-token actual-node composition | Bounded RED after both forward actors reached revision 2; fixed-window speed gate component GREEN; full packet open | Run `m3f7compose20260718t` at clean pushed `50db397` repeated deployment, both Vault Claims, the official eight-transaction fixture, Bitcoin lock, and finalized token initialize/custody/fund at blocks 120/148/170. Maker exact observation and Taker lifecycle-aware peer discovery both reached `both_legs_locked` revision 2, proving the Run S scanner fix on actual nodes. The next evidence-only jq serializer failed before claims or reverse direction; exact non-foreign cleanup passed. Run U on pushed `65f55c5` was deliberately stopped after thirteen minutes because the artificial ten-second slot had reached only bootstrap and no new F7 boundary; exact cleanup left no owned resources. ADR 0047 now pins the requested finalized interval, accepts monotonic descendants, retains rewind/fork/identity-drift rejection, and restores the one-second local cadence. The 128-test sidecar suite, five binary/example tests, strict Clippy, and pre-Docker contract are GREEN. Certification still requires a fresh exact-commit two-direction terminal run followed by at least three clean custom-token executions per direction with exact balances, effects, replay, and cleanup; Runs T and U are not F7 PoC passes |
