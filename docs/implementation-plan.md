...
|---|---|---|---|
| Reference-actor composition prerequisites | The first process-composition audit found no production fresh-client factory, actor-owned request-context allocator, store-derived LEZ claim funding authority, or role-keyed Zcash signer; SDK ports were non-cloneable and the reusable local node exposed an all-zero channel that signed agreements reject | The private fresh-client factory, OS-random request/window source, and shared role-local crash journal are GREEN. Reopened SQLite now derives canonical LEZ funding for both directions without accepting primitive transaction IDs and, in reverse, refuses until the durable Zcash second lock exists. A zeroizing role-keyed signer delegates exact claim/refund construction to the existing secp256k1/librustzcash builders. The complete 61-test bridge-adapter and 55-test Zebra-adapter suites plus strict Clippy pass. A focused locked-graph RED/GREEN changes node/config/readiness to one nonempty deterministic channel and all three readiness tests pass. The last exact full runner predates that channel correction. | Re-run the exact full standalone verifier, then implement the agreement-committed exact-outpoint Zebra funding planner and one-shot maker/taker actor process |
