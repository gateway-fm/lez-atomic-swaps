...
- [x] Harden the unreleased reference-actor configuration as schema v2 before
  ...
  chain port. The current `activate`, `drive`, and `status` executable still
  stops after command-scoped configuration validation; descriptor-bound opening
  of mutable SQLite plus SDK/chain-port composition is the next
  repository-controlled task.
