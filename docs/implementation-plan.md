...
| Crash-safe ZEC lock intents and projection | Initial active chain capabilities were inert; a naive RPC-success transition would lose unknown outcomes and combined LEZ initialize/fund into one unsafe effect | The maker consumes fresh exact-head eligibility internally, persists the opposite-chain plan before submission, and atomically projects confirmed funding. Noneligible polls create no intent and invoke no submission in either direction. A canonical depth/finality update can commit after staging without another maker step; the retained intent then closes against the newer predecessor and replays at `BothLegsLocked`. Stale instances, projection failure, unknown store success, and SQLite rollback are covered. SDK has 90 ordinary tests plus one doctest; store has 25 tests. | Harden unknown-submission, reorg, and corruption boundaries; then connect production ports and claims/refunds |
| SQLite SDK-recovery schema | Schema v4 had no role-local accepted agreement, open/closed effect intent, separate active revision, or exact transition slot for the concrete SDK contract | Schema-v7 strict tables retain taker and maker intents separately. Both directions now prove intent staged at revision 1, canonical update committed at 2 with no new maker submission, and maker transition predecessor 2/close 3 before reopen at `BothLegsLocked`. Union replay rejects holes/duplicates, and trigger injection proves all maker writes roll back together. | Add maker submission/reorg/corruption matrices, then extend the pattern to claims/refunds; process-kill coverage and secret encryption remain M5 |
