...
| Crash-safe ZEC first-lock intent | Initial active chain capabilities were inert; a naive RPC-success transition would lose unknown outcomes and combined LEZ initialize/fund into one unsafe effect | Three RED–GREEN cases stage exact role/direction-bound bytes before a node call, conflict on changed replay, wait on unstable observation, observe before exact rebroadcast after restart, and require confirmed LEZ initialization before funding; full package has 75 passing tests and core remains `Offered` | Add production SQLite intent storage and atomic validated-observation plus aggregate projection; then extend the same contract to maker lock, claims, and refunds |
...
- [ ] Complete typed LEZ and Zcash actions plus atomic active-swap transition
  ...
  rebroadcast, and separates LEZ initialize/fund; production SQLite intent
  storage and atomic evidence projection remain, so this item is still open.
