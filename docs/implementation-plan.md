...
| Restart recovery and user isolation | 2026-07-11 unresolved `SqliteSwapStore` and later missing `claim_evidence`; the schema-v9 RED then exposed plaintext legacy evidence and missing claim journals | Schema v9 now closes/reopens two independent role stores through both claim directions to `Completed`. Core aggregate state retains only a SHA-256 evidence marker; preimages and exact submissions use context-bound XChaCha20-Poly1305 envelopes. Commit `5ed04ec` transactionally migrates and scrubs legacy plaintext from SQLite/WAL; `340bf10` proves wrong-key/corruption/orphan rejection, coupled rollback, and observe-before-rebroadcast after unknown outcomes or stale replay. | Replace deterministic claim ports with isolated actual-node adapters; retain broader process-kill coverage for the M5 coordinator and add rotation-aware production key provisioning |
...
| ZEC full-lifecycle SDK boundary | 2026-07-12 `sdk_lifecycle` could not import discovery, negotiation, concrete agreement, active-swap, recovery-store, or secret types | Independent role-fixed SDKs receive bounded untrusted wire, validate the same dual-signed agreement, persist before activation, reject adversarial resume, expose no raw transport/store handles, drive both lock and claim directions, and replay schema-v9 SQLite to `Completed`. The package has 120 passing checks including one doctest plus the intentional ignored Zebra gate; the committed swap-store surface has 35 passing tests. | Add the typed refund lifecycle, then replace deterministic claim ports with actual-node adapters and independent reference-actor processes |
| Crash-safe ZEC lock intents and projection | Initial active chain capabilities were inert; a naive RPC-success transition would lose unknown outcomes and combined LEZ initialize/fund into one unsafe effect | Fresh gating, noneligible zero-effect, intervening canonical revisions, stale instances, projection failure/unknown success, and SQLite rollback pass. Two independent role-fixed SDK instances with separate stores complete both lock directions and restart at `BothLegsLocked`. Commit `add5d98` extends the deterministic actor path through the agreement-fixed LEZ reveal and Zcash follow-up to `Completed` in both directions, with separate owner/observer transition slots and protected extracted material. Commit `340bf10` closes the claim-specific unknown-outcome, stale-replay, rollback, key/ciphertext/nonce/future-version, orphan, duplicate, and head-drift matrix. SDK has 120 passing checks including one doctest plus one existing ignored Zebra gate; store has 35 passing tests. | Replace contract-double claim ports with canonical actual-node adapters, then add typed refunds and independent process actors. |
...
| Composed actual-node LEZ/Zebra claim corridor | A failing composition attempt proved that the Zcash graph's `crypto-common = 0.2.0-rc.1` pin cannot coexist with the official LEZ v0.1.2 graph's stable `crypto-common ^0.2` requirement through `chacha20 0.10` and `cipher 0.5.1` | ADR 0022 accepts a separately locked official-LEZ sidecar behind a bounded typed local protocol plus an in-process typed Zebra adapter. Commit `b1de754` freezes the dependency-light bridge contract, `166d3e5` proves restart-safe final Zcash revalidation before reveal, and `9235215` plus `52bdb81` provide the official transaction planner and generated upstream node-RPC core. The main workspace now has a one-attempt authenticated bridge client for all six protocol methods and a production owner-claim Zebra port that derives agreement-fixed terms, delegates only role-local signing, revalidates canonical funding, observes before exact rebroadcast, and classifies ambiguous outcomes conservatively. Nine bridge-client and 31 Zebra adapter tests pass. Existing actual LEZ and Zebra suites remain separate and are not claimed as a composed GREEN. | Add the authenticated sidecar server, official LEZ revealing-claim planner, and bounded counterparty-spend discovery; wire the SDK clients and two actor processes; then drive the both-direction happy path GREEN with restart after every effect before refund/reorg/concurrency hardening |
...
- [ ] Complete typed LEZ and Zcash actions plus atomic active-swap transition
  ...
  persists complete canonical evidence. Its ordered schema-v9 journal now
  ...
  schema-v9 SQLite close/reopen. The expected maker submission ID is still
  ...
  kill coverage remains M5. Production LEZ/Zebra claim ports must derive
  canonical identities and evidence from actual nodes; refunds remain open.
  ...
  post-submit unknown outcomes pass four additional tests. This is not yet a
  running bridge sidecar: the capability- and `RUN_ID`-bound server, revealing-
  claim planner, counterparty terms discovery, client composition, and isolated
  single-owner runner remain.
  ...
  result. The adapter's 31 tests use SDK/SQLite-built authentic contexts;
  bounded counterparty outpoint-spend discovery and actor wiring remain. The
  ...
  The main workspace bridge client now implements all six
  ...
  Server-side durable request idempotency remains mandatory before the client
  can enter the composed runner. Pin weakening, crypto patches, and hand-copied LEZ wire/RPC types are
  prohibited.
