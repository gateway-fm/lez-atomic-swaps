...
| Crash-safe ZEC first-lock intent and projection | Initial active chain capabilities were inert; a naive RPC-success transition would lose unknown outcomes and combined LEZ initialize/fund into one unsafe effect | Five RED–GREEN cases stage exact role/direction-bound bytes before a node call, conflict on changed replay, wait on unstable observation, observe before exact rebroadcast, order LEZ initialize/fund, keep core unchanged on invalid evidence/failed commit, probe unknown success exactly, and replay the committed transition after restart; full package has 78 passing tests | Implement the same contract in production SQLite, add maker-independent taker-lock observation before second lock, then extend it to maker lock, claims, and refunds |
| SQLite SDK-recovery schema | Schema v4 had no role-local accepted agreement, open/closed effect intent, separate active revision, or exact transition slot for the concrete SDK contract | Schema-v5 migration creates strict composite-role agreement, intent, and transition tables; tests prove legacy migration, two-role same-swap isolation, role/revision checks, parent enforcement, and cascade; original acceptance and active revisions are separate | Implement primitive DTO serialization plus full revalidation and the cloneable `RecoveryStore` adapter; table presence alone is not production recovery evidence |
