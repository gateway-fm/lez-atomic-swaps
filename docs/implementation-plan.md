...
| Composed actual-node LEZ/Zebra claim corridor | A failing composition attempt proved that the Zcash graph's `crypto-common = 0.2.0-rc.1` pin cannot coexist with the official LEZ v0.1.2 graph's stable `crypto-common ^0.2` requirement through `chacha20 0.10` and `cipher 0.5.1` | ADR 0022 accepts a separately locked official-LEZ sidecar behind a bounded typed local protocol plus an in-process typed Zebra adapter. Commit `b1de754` freezes the dependency-light, source-checked bridge contract, and commit `166d3e5` proves restart-safe final Zcash revalidation before reveal. The separately locked sidecar planner is now GREEN with official `PublicTransaction` construction, exact cached byte/ID submission, one checked consecutive nonce reservation, full runtime/signer/role binding, 7 locked tests, strict lints/docs, and an isolated advisory/license/source policy. Existing actual LEZ and Zebra suites remain separate and are not claimed as a composed GREEN. | Add the sidecar server and official nonce/submit/observation RPC adapters, finish the SDK-facing bridge client and Zebra claim/refund actions, then drive the two-role, both-direction happy path GREEN with restart after every effect before refund/reorg/concurrency hardening |
...
- [ ] Complete typed LEZ and Zcash actions plus atomic active-swap transition
  ...
  This is the official transaction-planning core, not yet a running sidecar:
  the capability- and `RUN_ID`-bound server, official nonce/submit/observation
  RPC adapters, client composition, and isolated single-owner runner remain.
