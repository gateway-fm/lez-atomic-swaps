...
|---|---|---|---|
| Reference-actor composition prerequisites | The first process-composition audit found no production fresh-client factory, actor-owned request-context allocator, store-derived LEZ claim funding authority, role-keyed Zcash signer, agreement-committed funding planner, or independent actor boundary; SDK ports were non-cloneable and the reusable local node exposed an all-zero channel that signed agreements reject | The private fresh-client factory, OS-random request/window source, and shared role-local crash journal are GREEN. Reopened SQLite derives canonical LEZ funding for both directions without accepting primitive transaction IDs and, in reverse, refuses until the durable Zcash second lock exists. A zeroizing role-keyed signer delegates exact funding/claim/refund construction to the existing secp256k1/librustzcash builders. The exact-outpoint planner checks the signed input-set commitment, role/key/network/branch/genesis, a stable Zebra tip, every confirmed `gettxout`, transparent-only V5 shape, contract/change policy, and canonical bytes. The checked `ZebraRpcZcashPort` composes all five SDK Zcash traits without duplicating chain behavior and fixes one identity, role, signer, RPC handle, and scan bound; the complete 66-test Zebra package plus strict Clippy/rustdoc passes. SDK activation/resume no longer requires fake discovery or negotiation capabilities after signed-wire acceptance, and `resume_all_capable` replays lock, claim, and refund history to truthful terminal status; all 132 SDK tests pass with one actual-Zebra case intentionally ignored. The reference actor's unreleased schema v2 now fixes one typed run/swap/role, exact signed-agreement SHA-256, complete sidecar and Zebra runtime identities, finite discovery windows/candidate outpoints, and isolated stores/journals/keys. Paired configs require identical agreement/runtime/Zebra terms, exactly one preimage owner and one funder, distinct sidecars/signers, and no cross-role path aliases. Command-scoped loaders enforce Unix-private regular files, detect hardlinks/replacement/version drift, keep diagnostics redacted, and let offline status load only claim-recovery/store material without effect credentials or chain ports; the SDK's terminal replay remains store-only; its hardened contract suite and strict gates pass. The real `status` command now opens only a pre-existing hardened role store and performs store-only SDK recovery with chain access impossible by type; absent state remains uncreated as `not_activated`. `activate` and `drive` still fail closed before SDK/chain-port composition. Signed runtime binding now admits only the exact local v0.1.2/NSSA and v0.2/LEE generation pairs, rejects both cross-pairs, and keeps every public v0.2 profile fail-closed until a reviewed deployment is configured. A focused locked-graph RED/GREEN changes node/config/readiness to one nonempty deterministic channel and all three readiness tests pass. The last exact full runner predates that channel correction. | Keep the v0.1.2 exact verifier as lower coverage; TDD the full local v0.2 runner and official-wire sidecar, dormant public profiles, and one-shot `activate`/`drive` commands through both actual-node directions |
...
| Architecture diagrams | 2026-07-11 completeness guard failed on the first ADR without Mermaid; a 2026-07-12 audit then found that the renderer covered architecture/M1 blocks but omitted the manual-flow diagram | The earlier 46-block architecture passed the conservative GitHub-host policy and exact Mermaid CLI 11.16.0 gate. Current ADR-0024/system/deployment additions use the same conservative subset but intentionally have not been rendered yet because the owner deferred the one full render pass to milestone end | Run static completeness/host-policy checks during edits; perform one exact full Mermaid render and GitHub visual check before the M2 evidence commit |
...
| Composed actual-node LEZ/Zebra claim corridor | A failing composition attempt proved that the Zcash graph's `crypto-common = 0.2.0-rc.1` pin cannot coexist with the official LEZ v0.1.2 graph's stable `crypto-common ^0.2` requirement through `chacha20 0.10` and `cipher 0.5.1`. The first exact external-process run then RED because it incorrectly treated upstream's static built-in `getProgramIds` map as a custom-deployment registry. | ADR 0022 accepts a separately locked official-LEZ sidecar behind a bounded typed local protocol plus an in-process typed Zebra adapter. The authenticated single-flight sidecar server binds capability/run/role before parsing, serves all eight methods, durably restores official randomized native/claim and unsigned refund bytes, and persists an unknown-submission guard before node effects. Native escrow, revealing-claim, and native-refund exact-owner/bounded-discovery observation are GREEN with conservative absence and stable clock/tip rules. The main workspace has one-attempt signed-agreement first-lock/claim/refund conversion plus a crash-safe context-owning implementation of all five LEZ SDK port traits. Production Zebra ports now submit and discover agreement-bound funding in both role directions and handle owner/counterparty claim/refund evidence; unresolved bounded scans stay `Unstable`. The corrected external exact-v0.1.2 full runner preflights the tracked ELF/ImageID, owns a fresh mode-0700 home, locates the exact deployment transaction in its canonical block, uses `getProgramIds` only for the authenticated-transfer built-in, publishes those schema-v2 facts plus two funded deterministic actors in mode-0600 readiness, and passes process, native/two-definition, Clippy, and recursive-cost gates. Existing actual LEZ and Zebra suites remain separate and are not claimed as a composed GREEN. | Wire two actor processes to the context-owning ports and checked handoff; drive the both-direction happy path GREEN with restart after every effect, then add post-lock removal/replacement and refund/reorg/concurrency hardening |
| Zcash public-testnet route and funding research | M2 required self-hosted/public node, wallet, faucet, privacy, and flakiness guidance but no supported route was selected | Primary sources select self-host Zebra 6.0.0 with loopback cookie RPC and Tatum's documented API-key-authenticated Testnet Zebrad gateway as the public-provider route. Optional Zallet alpha.4 funding and faucet/Discord fallback remain documented. No Zcash Foundation-operated public Zebra RPC or Zallet arbitrary HTLC signer was found. | Implement project-owned disposable transparent signing and the fail-closed HTTPS provider adapter as dormant M2 portability surfaces, with local exact-method/configuration contract tests. Clean-host public rehearsal, TAZ, and live evidence are deferred under ADR 0023; U10 remains incomplete |
| LEZ v0.2 public-runtime security route | Fresh 2026-07-12 audit found the official v0.2.0 runtime graph still carries Hickory 0.25 and upstream explicitly ignores RUSTSEC-2026-0118/0119; SPEL PR #238 remains open and unreviewed despite green CI | Exact LEZ v0.2.0 `a58fbce...` plus SPEL PR head `df17acd...` now build the checked guest/generated client and recursive native/two-definition/rollback suite. The exact-once fixed-URL official-RPC deployer binds channel, built-ins, ELF, ImageID, ProgramId, transaction bytes/hash, and containing block. Its unavoidable Logos common/libp2p/Hickory path is constrained by four graph-local cargo-deny policies and feature/reachability tests rather than hidden or falsely excluded. | Deploy and exercise public actor calls, retain ambiguity/no-resubmit evidence, and keep production readiness fail-closed until SPEL is reviewed/merged and Hickory is removed or explicitly accepted. Per ADR 0018, the exact Logos-owned items are disclosed production blockers but do not stop private M2 certification once repository-controlled local-v0.2 and public-capability contract evidence is green; live public execution is deferred under ADR 0023. |
| Full local LEZ v0.2 service stack | Initial audit found wildcard binds, mutable packaging, implicit native downloads, incorrect topology, and weak health-only readiness | Exact source/binary/image inputs now run on a unique no-masquerade bridge. Run `v02-stack-20260713n` proves signed key-derived channel onboarding, finalized block 2, indexer ID/hash equality, sequencer Borsh identity, channel slot 15 to 49, and fail-closed exact cleanup. Compose is configuration validation only; direct Docker lifecycle is exact-ID scoped | Service readiness is GREEN. Add owner-authorized Vault Claims, checked escrow deployment, independent actors, swap effects, and restart recovery before claiming the full runtime tuple or M2 corridor |
| Official-wire LEZ v0.2 sidecar | The final ADR-0023 corridor had only the incompatible v0.1.2/NSSA sidecar; direct `cargo --offline` still let upstream `rust-rapidsnark` attempt an implicit native-library download | A separately locked Rust 1.96.0 graph now consumes exact LEZ v0.2.0 account, public-transaction, Vault, escrow, and generated sequencer RPC types. Seventeen tests prove runtime/role/signer/channel binding, authenticated describe, exact signed native initialize/fund preparation, exact deterministic maker/taker Vault Claim preparation, canonical recovery validation, node-confirmed nonces, literal-loopback policy, and secret redaction. Both planners reserve only after complete construction and expose no submit path. Durable per-operation reservation recovery is now GREEN: each planner binds one owner-only `0700` actor directory, persists its single reservation as an fsynced create-exclusive `0600` file, and restart returns the originally signed exact bytes with zero nonce-source calls. Seven added tests prove request-drift rejection, independent maker/taker stores, complete stored request/result revalidation against role/runtime/signer/allocation/program/transaction-ID tampering, crash-partial/corruption/future-schema/unknown-field rejection, symlink/permission/hardlink refusal, and path-redacted diagnostics. The authoritative wrapper rejects missing or hash-drifted Rapisnark inputs before locked/offline format, test, strict-Clippy, rustdoc, and dependency gates. Four disclosed upstream advisory exceptions have feature/reachability assertions | Add authenticated server integration, exact/discovery observation, and ADR 0026's durable one-attempt submit; then executable maker/taker role processes and local/public signed configuration. Compose them through the real full stack; local preparation is not an on-chain effect or corridor proof |
| LEZ v0.2 submission and finality contract | Exact source audit found that `sendTransaction` proves only stateless admission, the volatile mempool exposes no status, stateful rejection has no receipt, and neither transaction query returns a containing block position | Architecture only: ADR 0026 fixes durable `AttemptStarted` before one send, observe-only reopen, bounded sequencer inclusion, and indexer finalized block/hash/account-at-block proof. No submit, recovery, finality, actor, or actual-node implementation is GREEN from this decision | RED the one-call crash matrix and exact query validators; implement official generated sequencer/indexer adapters and role-local durable effect states; then prove independent maker/taker Vault Claims against the full local v0.2 stack |
...
Owner-approved stealth certification policy (ADR 0023): the unchecked public
deployment, public Zcash-testnet, public funding-guide rehearsal, and public
recording outputs above remain honest proposal #112 gaps, but they do not block
the M2 tag. M2 instead requires a fully functional private corridor using one
...
output hashes are now attested; independent clean rebuild reproducibility,
container assembly, three-service startup,
same-ID finalized-block readiness, Vault Claim actor onboarding, checked escrow
deployment, and actor consumption remain RED. The standalone mock publisher and
...
funds, stores, journals, sidecars, and restart lifecycles. Both directions plus
happy, restart, refund, reorg, and concurrency cases must cross the actual local
nodes; contract doubles do not certify this boundary. Evidence and recordings
stay private and no public endpoint, faucet, deployment address, or transaction
...
does not pass. This gate is currently open because public LEZ activation fails
closed, actor endpoints are loopback-only, and the public Zebra route is not
composed. M2 must implement and locally contract-test those dormant public
configuration/adapters even though it does not execute them publicly. Public
...
- [x] Port and rebuild SPEL, guest, generated client, and PDA derivations for LEZ
  ...
  child-failure rollback, and tests an exact-once official-RPC deployer.
...
- [x] Source-audit and produce attested locked outputs for the full
  ...
  Restricted distroless CLI smoke passes. Independent clean rebuild reproducibility remains open. Isolated run `v02-stack-20260713n` additionally executes the pinned Bedrock, sequencer, and indexer with signed channel onboarding and non-genesis finality.
- [x] Package and execute the exact binaries with Bedrock under `.e2e/{run_id}/lez-v02`. Run `v02-stack-20260713n` proves cryptarchia advancement, exact missing-channel failure, signed runtime-channel accreditation, sequencer health/channel/built-ins/genesis, finalized ID 2, indexer by-ID/by-hash equality, sequencer Borsh-header identity, channel advancement, and fail-closed exact cleanup.
- [ ] Implement ADR 0026's durable LEZ v0.2 effect boundary. Current status is
  architecture and exact-source audit only: persist exact preparation and
  `AttemptStarted` before one `sendTransaction`, make every reopen observe-only,
  prove a unique exact sequencer inclusion inside a fixed bounded window, and
  require matching indexer transaction, finalized block ID/hash, and
  `getAccountAtBlock` state before advancing. Next is the RED crash/query matrix;
  no submission or finality implementation is claimed GREEN.
- [ ] Complete the full runtime tuple: perform each actor owner-authorized Vault Claim, deploy and observe the checked escrow, run independent role processes and swap effects, and prove restart recovery.
...
- [ ] Prove the composed native happy path preserves the ADR 0022 atomicity
  ...
  the signed refund margin but defer public-latency calibration to production
  readiness rather than treating Regtest timing as production proof.
...
- [ ] Complete peer-independent timeout recovery through public SDK ports. The
...
- [ ] Keep the stable Zebra/security pin and public-capable configuration under
...
- [x] Publish a living manual reproduction guide for the currently proven maker
  ...
  ADR 0023.
- [x] Harden the unreleased reference-actor configuration as schema v2 before
  ...
  current `activate` and `drive` commands remain fail-closed until the exact
  v0.2 sidecar/Zebra ports and effectful SDK composition are available.
...
- [ ] Publish exact local LEZ v0.2/Zebra setup and the configuration-only public
...
- [ ] Re-run formatting, strict Clippy, all tests/docs, ShellCheck, traceability,
  ...
  public smoke remains production-readiness work.
...
| Bedrock local image versus public-runtime parity is not published | The exact local image digest has immutable OCI labels binding repository, Apache-2.0 license, and source revision `d8711bbc...`, which matches the LEZ lock; no evidence yet proves that image is the current public-testnet runtime | Bind and verify the OCI labels for local M2, keep public activation fail-closed, and retain public-runtime parity as a Logos-owned production-readiness question rather than weakening local execution gates |
