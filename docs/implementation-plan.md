...
| Composed actual-node LEZ/Zebra claim corridor | A failing composition attempt proved that the Zcash graph's `crypto-common = 0.2.0-rc.1` pin cannot coexist with the official LEZ v0.1.2 graph's stable `crypto-common ^0.2` requirement through `chacha20 0.10` and `cipher 0.5.1` | ADR 0022 accepts a separately locked official-LEZ sidecar behind a bounded typed local protocol plus an in-process typed Zebra adapter. The authenticated single-flight sidecar server binds capability/run/role before parsing, durably restores official randomized native and revealing-claim bytes, and persists an unknown-submission guard before node effects. The claim planner validates role/runtime/signer/terms/preimage/funding before nonce use; the funding ID is request/cache-bound because the official guest ABI cannot embed it. Six executable methods are registered while the protocol/client define the two native-refund calls. Official native escrow and revealing-claim exact-owner/bounded-discovery observation are GREEN with conservative absence and stable-tip rules. The main workspace has the one-attempt client, exact signed-agreement native first-lock conversion, and production owner/counterparty Zebra claim/refund paths; unresolved bounded scans stay `Unstable`. Existing actual LEZ and Zebra suites remain separate and are not claimed as a composed GREEN. | Finish native LEZ refund and main claim/refund composition plus unknown-ID Zebra funding observation; wire two actor processes; then drive the both-direction happy path GREEN with restart after every effect before refund/reorg/concurrency hardening |
...
- [ ] Complete typed LEZ and Zcash actions plus atomic active-swap transition
  ...
  validation. The production LEZ refund adapter and composed actors remain.
  ...
  Six executable methods are registered; the shared protocol/client expose the
  two native-refund calls whose sidecar handlers remain RED. Official
  revealing-claim preparation now
  ...
  restart-safe native/claim cache coexistence. Refund composition, main claim
  SDK ports, actor processes, and the composed corridor remain.
  ...
  dependency-policy gates pass. Native refund and main claim/refund SDK-port
  composition remain. Pin
...
- [ ] Complete peer-independent timeout recovery through public SDK ports. The
  ...
  commits and the Zebra production refund adapter are now GREEN. The production
  LEZ refund adapter plus composed independent actors remain before this gate
  can close.
