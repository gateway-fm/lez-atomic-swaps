...
| Crash-safe ZEC first-lock intent and projection | Initial active chain capabilities were inert; a naive RPC-success transition would lose unknown outcomes and combined LEZ initialize/fund into one unsafe effect | Taker staging/retry/projection plus maker-independent direction-selected observation commit before memory changes, probe unknown outcomes, reject corrupt primitive records, and replay from separate role-local stores; forward Zcash rejects primitive ID/depth assertions and durably journals complete canonical evidence, atomic same-tip replacement, same-inclusion depth change, and affirmative removal through `ZcashObservationTracker`; reverse LEZ rejects primitive assertions and revalidates signed channel/genesis plus stable transaction/block/metadata/exact-custody snapshots after SQLite reopen; its two-phase tracker suppresses duplicates, journals monotonic Pending-Safe-Finalized updates, requires affirmative same-tip replacement, and rejects finalized removal; unchanged Zcash polls write nothing; the schema-v6 store folds history before append, rejects orphan/holey/duplicate/stale/poison histories, rolls back trigger failures, and catches stale instances up to the durable head; a distinct non-cached fresh eligibility call replays then re-queries, rejects eligibility on stable absence, instability, or an unconfirmed phase, and remains non-authorizing until a maker effect consumes it internally; SDK package has 84 ordinary tests plus one doctest and store has 23 tests | Connect official node wire types to production ports; integrate the LEZ exact-head tracker into the SDK/SQLite journal, implement reverse fresh eligibility, and the maker second-lock effect that consumes eligibility in one operation, then claims/refunds |
...
- [ ] Complete typed LEZ and Zcash actions plus atomic active-swap transition
  ...
  SQLite close/reopen; a single-field negative matrix covers chain,
  transaction, metadata, and custody substitutions. The production RPC
  adapter, official-wire transaction decoding, LEZ exact-head tracker journal integration, token-custody observation matrix, reverse
