...
| ZEC full-lifecycle SDK boundary | 2026-07-12 `sdk_lifecycle` could not import discovery, negotiation, agreement, active-swap, recovery-store, or secret types | Independent role-fixed SDKs publish/discover/negotiate the same immutable agreement, reject wrong role/pair/profile policy plus unknown schema, empty transcript, and pre-advanced state; persist separately before activation, and resume through an `ActiveZecSwap` type with no transport handles; preimage debug is redacted and storage zeroizes | Add typed LEZ/Zcash action ports, atomic transition persistence, both-direction happy/refund/restart/concurrency actors, then actual nodes |
| Concrete LEZ/ZEC agreement | Initial generic LEZ terms did not bind exact chain identities, custody, transaction destinations/fees, funding inputs, wire bounds, or both actor signatures | Seventeen focused tests prove bounded exact decoding, dual low-S signatures, both directions, fail-closed public deployment, actual LEZ/ZEC deadlines, exact PDA/ATA derivation, accepted-at resume, redacted diagnostics, and agreement-derived funding/claim/refund requests | Cross-check `/LEE/` derivation through exact upstream v0.2 types, then migrate negotiation, activation, and resume before exposing effects; adapters must independently recompute chain-derived terms |
...
- [x] Implement the concrete canonical LEZ/ZEC agreement validator and bounded
  ...
  and negotiation transcript. This validator is not yet wired into activation
  or resume and therefore does not by itself satisfy the full SDK lifecycle.
  Public-testnet validation remains fail-closed until a reviewed immutable LEZ
  deployment exists; local `/LEE/` PDA/ATA goldens still need an independent
  upstream-type cross-check in the provisional compatibility lane.
