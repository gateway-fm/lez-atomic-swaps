...
| Full local LEZ v0.2 service stack | Initial audit found wildcard binds, mutable packaging, implicit native downloads, incorrect topology, and weak health-only readiness | Exact source/binary/image inputs now run on a unique no-masquerade bridge. Run `v02-actors-finalized-20260713b` proves signed key-derived channel onboarding, finalized block 2, indexer ID/hash equality, sequencer Borsh identity, distinct maker/taker owner/Vault pre-Claim states at that exact finalized block, and fail-closed exact cleanup. Compose is configuration validation only; direct Docker lifecycle is exact-ID scoped | Service and pre-Claim actor state are GREEN. Add owner-authorized Claim submission/finality, checked escrow deployment, effect-bearing independent actors, swap effects, and restart recovery before claiming the full runtime tuple or M2 corridor |
| Official-wire LEZ v0.2 sidecar | The final ADR-0023 corridor had only the incompatible v0.1.2/NSSA sidecar; direct `cargo --offline` still let upstream `rust-rapidsnark` attempt an implicit native-library download | A separately locked Rust 1.96.0 graph now consumes exact LEZ v0.2.0 account, public-transaction, Vault, escrow, and generated sequencer RPC types. Seventeen tests prove runtime/role/signer/channel binding, authenticated describe, exact signed native initialize/fund preparation, exact deterministic maker/taker Vault Claim preparation, canonical recovery validation, node-confirmed nonces, literal-loopback policy, and secret redaction. Durable preparation recovery adds eight tests: on Linux each planner opens one owner-only `0700` actor directory with `openat2(NO_SYMLINKS)`, revalidates the held directory and state inode around every operation, persists an fsynced create-exclusive `0600` reservation, and restarts with the originally signed exact bytes and zero nonce-source calls. Request drift, independent stores, complete role/runtime/signer/allocation/program/transaction-ID tampering, crash partials, corrupt/future/unknown state, runtime permission drift, symlinks/hardlinks, and path leakage fail closed. The authoritative wrapper rejects missing or hash-drifted Rapisnark inputs before locked/offline format, 25 tests, strict-Clippy, rustdoc, and dependency gates. Four disclosed upstream advisory exceptions have feature/reachability assertions | Add authenticated server integration, exact/discovery observation, and ADR 0026's actor-wide durable one-attempt submit/nonce-phase journal; then executable maker/taker role processes and local/public signed configuration. Compose them through the real full stack; local preparation is not an on-chain effect or corridor proof |
