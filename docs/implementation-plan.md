...
| Full local LEZ v0.2 service stack | Initial audit found wildcard binds, mutable packaging, implicit native downloads, incorrect topology, and weak health-only readiness | Exact source/binary/image inputs now run on a unique no-masquerade bridge. Run `v02-actors-finalized-20260713b` proves signed key-derived channel onboarding, finalized block 2, indexer ID/hash equality, sequencer Borsh identity, distinct maker/taker owner/Vault pre-Claim states at that exact finalized block, and fail-closed exact cleanup. Compose is configuration validation only; direct Docker lifecycle is exact-ID scoped | Vault Claims, deployment, and one role-separated native lifecycle are now GREEN. Compose the reference actors and Zebra effects through both reproducible happy directions; restart recovery is post-PoC hardening |
| Official-wire LEZ v0.2 sidecar | The final ADR-0023 corridor had only the incompatible v0.1.2/NSSA sidecar; direct `cargo --offline` still let upstream `rust-rapidsnark` attempt an implicit native-library download | A separately locked Rust 1.96.0 graph now consumes exact LEZ v0.2.0 account, transaction, Vault, escrow, generated sequencer RPC, `jsonrpsee`, and bundled SQLite types. Its 42 integration tests retain runtime/prepare/recovery coverage and add seventeen ADR-0026 submission cases: immutable role/run/runtime/signer journals, canonical duplicate exact bytes, attempt-before-call, observe-only reopen, forced races, crash/ambiguity outcomes, coordinator-owned error classification, strict revisions, malicious-schema rejection, trusted-parent-chain checks, and inode substitution refusal. The wrapper rejects missing or hash-drifted Rapisnark inputs before locked/offline format, strict Clippy, rustdoc, and dependency gates. Four disclosed upstream advisory exceptions have feature/reachability assertions | Add the real generated-RPC adapter, authenticated effect server, bounded sequencer inclusion and indexer finality/account proof; then executable maker/taker processes and local/public signed configuration. Compose them through the real full stack; the green library call guard is not an on-chain effect or corridor proof |
| LEZ v0.2 submission and finality contract | Exact source audit found that `sendTransaction` proves only stateless admission, the volatile mempool exposes no status, stateful rejection has no receipt, and neither transaction query returns a containing block position | ADR 0026's Vault Claim journal and one-call crash matrix are GREEN: durable `AttemptStarted`, observe-only reopen, raw-error classification, strict revisions, schema/path hardening, and forced races pass. Bounded sequencer inclusion, indexer finalized block/hash/account-at-block proof, generated clients, actors, and actual-node execution remain unproved | Implement only the query/finality composition required to produce reproducible exact transaction, finalized block, and terminal account evidence in both PoC directions. Expand its RED-GREEN-REFACTOR matrix after owner transition |
...
balance evidence. Independent clean rebuild reproducibility, reference-actor
consumption, Zebra HTLC effects, and the composed corridor remain unimplemented
PoC work. The standalone mock publisher and existing v0.1.2 lane remain
lower-level coverage only.
...
- [ ] Complete the PoC's LEZ v0.2 effect composition. The Vault
  ...
  substitution. The full package gate has 42 integration tests. PoC work is
  reference-actor composition plus reproducible exact sequencer, finalized
  indexer, and terminal-account evidence in both happy directions. Integrated
  journal transitions and ambiguous native-effect crash reconciliation remain
...
- [ ] Complete the full runtime tuple. Vault onboarding, checked deployment,
  ...
  Still prebuild, provision just in time from a fresh tip, start explicit
  run-port role sidecars, fail fast on the 60-second LEZ and four-block Zcash
  refund headroom, run the SDK reference effects, execute Zebra HTLC effects
  and both cross-chain directions, and provide one-command scoped cleanup before
  closing this PoC item. Restart/ambiguous-outcome recovery is deferred until
...
- [ ] Prove the composed native happy path preserves the ADR 0022 atomicity
  ...
  hardening rather than treating Regtest timing as production proof.
...
- [ ] After the owner ends PoC, complete peer-independent timeout recovery
...
- [ ] After the owner ends PoC, compose cross-chain refund-margin cases through
...
- [ ] Pass both happy directions through the public SDK and actual local chain
...
- [ ] After the owner ends PoC, pass abandonment/refund, restart, reorg, and
