...
- [ ] Complete typed LEZ and Zcash actions plus atomic active-swap transition
  ...
  caching authority or changing `next_action`. The production RPC port,
  canonical LEZ adapter, second-lock effect that consumes eligibility in the
  same operation, claims, and refunds are remaining work, so this item is still
  open.
