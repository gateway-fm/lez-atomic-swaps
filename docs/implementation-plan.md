...
public route; signed configuration selects each environment's network/branch. LEZ
certification targets the full Bedrock node, indexer, and non-standalone
sequencer after their immutable pins and settlement wiring are source-verified;
that check is RED. The standalone mock publisher and existing v0.1.2 lane remain
