...
| Composed actual-node LEZ/Zebra claim corridor | A failing composition attempt proved that the Zcash graph's `crypto-common = 0.2.0-rc.1` pin cannot coexist with the official LEZ v0.1.2 graph's stable `crypto-common ^0.2` requirement through `chacha20 0.10` and `cipher 0.5.1` | ADR 0022 accepts a separately locked official-LEZ sidecar behind a bounded typed local protocol plus an in-process typed Zebra adapter. Existing actual LEZ and Zebra suites remain separate and are not claimed as a composed GREEN. | Implement the sidecar protocol/process, SDK-facing bridge client, direct Zebra adapter, and one single-owner isolated runner; drive the two-role, both-direction happy path GREEN with restart after every effect before refund/reorg/concurrency hardening |
