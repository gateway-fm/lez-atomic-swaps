...
| Full local LEZ v0.2 service stack | The first audit found wildcard service binds, mutable upstream Docker inputs, implicit Rapisnark download, no binary hashes, a false indexer-to-sequencer edge, and no finality-safe readiness contract | ADR 0024 binds clean LEZ `a58fbce2...`, Rust 1.94.0, Bedrock digest plus OCI revision `d8711bbc...`, exact r0vm/Rapisnark/distroless inputs, sequencer publication to Bedrock, and indexer polling from Bedrock. One clean-source locked offline build into a fresh target attests sequencer `3727e9aa...412f` and indexer `6ed54f04...7442`; a warm locked offline rerun performed no rebuild and retained those hashes. Both plus r0vm pass restricted distroless CLI smoke. Independent clean rebuild reproducibility remains unproven; version `0.1.0` is not provenance | Package the exact outputs, start project `lez-atomic-swaps-lez-v02-{run_id}` under `.e2e/{run_id}/lez-v02`, prove all-service same-ID finalized readiness, then perform Vault Claim onboarding, deploy escrow, and provision independent actors. Container/full-stack execution remains RED |
| Official-wire LEZ v0.2 sidecar | The final ADR-0023 corridor had only the incompatible v0.1.2/NSSA sidecar; direct `cargo --offline` still let upstream `rust-rapidsnark` attempt an implicit native-library download | A separately locked Rust 1.96.0 graph now consumes exact LEZ v0.2.0 `AccountId`, `PublicTransaction`, `LeeTransaction`, and generated sequencer RPC types. Five tests prove exact LEE role/signer/runtime binding, health plus channel identity, canonical signed-transaction decoding, literal-loopback-only node transport, and bearer/run/role rejection at an ephemeral describe-only server. The authoritative wrapper rejects missing or hash-drifted Rapisnark inputs before Cargo, then runs locked offline format/test/strict-Clippy/rustdoc/dependency gates. Four disclosed upstream advisory exceptions have feature/reachability assertions | Add the official v0.2 prepare, exact/discovery observe, and one-attempt submit families plus durable recovery, executable maker/taker role processes, and local/public signed configuration. Compose them through the real full stack; describe-only health is not an effect or corridor proof |
...
- [x] Source-audit and produce attested locked outputs for the full
  ...
  Restricted distroless CLI smoke passes. Independent clean rebuild
  reproducibility is still open; this is not a running-stack claim.
- [ ] Package and execute those exact binaries with Bedrock in project
  `lez-atomic-swaps-lez-v02-{run_id}` under `.e2e/{run_id}/lez-v02`. Readiness
  must prove Bedrock cryptarchia and channel advance, sequencer health/matching
  channel/built-ins/genesis block `1`/probe block, and an immutable same-ID finalized
  indexer block without requiring moving-tip equality. Then perform each
  actor's owner-authorized Vault Claim and deploy the checked escrow.
