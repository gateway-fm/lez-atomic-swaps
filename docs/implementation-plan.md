...
| Crash-safe ZEC first-lock intent and projection | Initial active chain capabilities were inert; a naive RPC-success transition would lose unknown outcomes and combined LEZ initialize/fund into one unsafe effect | Taker staging/retry/projection plus maker-independent direction-selected observation commit before memory changes, probe unknown outcomes, reject corrupt primitive records, and replay from separate role-local stores; forward Zcash now rejects primitive ID/depth assertions and durably revalidates complete canonical transaction/block/tip/output evidence against the signed HTLC output binding; role-local wallet construction policy is explicitly not a remote disclosure condition; seven SQLite adapter cases include exact maker replay and close/reopen recovery; SDK package has 81 passing tests and store package has 20 | Connect the canonical type to the production Zebra port; add the ordered SDK regression/removal/replacement journal and fresh eligibility check; implement canonical LEZ evidence, then later effects |
...
- [ ] Complete typed LEZ and Zcash actions plus atomic active-swap transition
  ...
  persists complete canonical evidence, while its production RPC port, the
  canonical LEZ adapter, durable regression/removal/replacement, a fresh pre-second-lock check, second
