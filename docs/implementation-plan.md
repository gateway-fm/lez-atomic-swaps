...
| Composed actual-node LEZ/Zebra claim corridor | A failing composition attempt proved that the Zcash graph's `crypto-common = 0.2.0-rc.1` pin cannot coexist with the official LEZ v0.1.2 graph's stable `crypto-common ^0.2` requirement through `chacha20 0.10` and `cipher 0.5.1` | ADR 0022 accepts a separately locked official-LEZ sidecar behind a bounded typed local protocol plus an in-process typed Zebra adapter. Commit `b1de754` freezes the dependency-light, source-checked bridge contract, and commit `166d3e5` proves restart-safe final Zcash revalidation before reveal. The separately locked sidecar now has GREEN official transaction planning plus generated upstream `getAccountsNonces`, `sendTransaction`, and bounded `getBlockRange` adapters. Eleven locked tests prove one checked nonce pair, exact cached-byte submission, conservative unknown outcomes, linked block/genesis validation, bracketed exact-owner scans, full runtime/signer/role binding, strict lints/docs, and isolated advisory/license/source policy. Existing actual LEZ and Zebra suites remain separate and are not claimed as a composed GREEN. | Add the authenticated sidecar server and official claim planner/counterparty discovery, finish SDK client/Zebra composition, then drive the two-role, both-direction happy path GREEN with restart after every effect before refund/reorg/concurrency hardening |
