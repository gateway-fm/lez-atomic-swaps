...
| Composed actual-node LEZ/Zebra claim corridor | A direct single-graph composition is impossible because the Zcash and official LEZ cryptographic pins conflict; ADR 0022 therefore requires the separately locked sidecar process | Run `m2poc-corridor-fresh-20260714o` is the first completed direction. Independent actors reached revision 4 `Completed`; LEZ init/fund/claim finalized in blocks 264/265/266 with terminal `Claimed`, custody 0, depositor 100000, and claimant 150000. Zebra funding `255b991f...dceab:0` at height 106 received two confirmations before reveal and was spent by `a2b41c5f...be16e` at height 108. Exact secret-safe evidence is checked in at `docs/evidence/m2-taker-sells-lez-corridor-20260714.json` | Reproduce reverse `TakerSellsForeign` through the same actual nodes and actor boundaries. Later hardening waits for owner transition |
...
| Official-wire LEZ v0.2 sidecar | The final ADR-0023 corridor had only the incompatible v0.1.2/NSSA sidecar; direct `cargo --offline` still let upstream `rust-rapidsnark` attempt an implicit native-library download | The exact role bridge completed initialize/fund/revealing-claim/observation/submit in 14o. A payload-free `moving_tip` observe error was retried once and succeeded; exact absence remains fixed by `0861117`. Startup now gates on non-genesis `getLastFinalizedBlockId` rather than upstream `checkHealth`, whose full-state recalculation can starve readiness. Refund is typed unavailable and the bridge itself still does not assert finality | Reuse the same bridge for the reverse direction and retain separate indexer finality. Existing gates remain enforced; expanded RED-GREEN-REFACTOR begins only after owner transition |
| LEZ v0.2 submission and finality contract | Exact source audit found that `sendTransaction` proves only stateless admission, the volatile mempool exposes no status, stateful rejection has no receipt, and neither transaction query returns a containing block position | Run 14o retained exact public tx/block membership and independent indexer finality for initialize/fund/claim in blocks 264/265/266, plus terminal account state. The retry policy was exercised once without duplicating a submission. This strengthens the corridor evidence without claiming that bridge readiness itself is finality | Retain the same finalized block/hash/account proof for the reverse direction; broaden the matrix after owner transition |
...
balance evidence. Reference actors now complete one terminal
`TakerSellsLez` corridor in run `m2poc-corridor-fresh-20260714o`: both actors
are revision 4 `Completed`, LEZ blocks 264/265/266 are finalized with terminal
balances, and the Zcash HTLC funding/claim are canonical at heights 106/108.
One bounded payload-free `moving_tip` retry succeeded. Independent clean
rebuild reproducibility and the reverse direction remain PoC work. The
standalone mock publisher and existing v0.1.2 lane remain
...
- [ ] Complete the PoC's LEZ v0.2 effect composition. The Vault
  ...
  before reveal and its `:0` output was spent at height 108. PoC work is now
  the reverse direction with equivalent evidence. Integrated
...
- [ ] Complete the full runtime tuple. Vault onboarding, checked deployment,
  ...
  three exact same-run retries. Run 14o live-proves those controls and terminal
  evidence for the first direction. Implement the second direction and preserve
  equivalent scoped cleanup before closing this PoC item. Restart/
...
- [x] Harden the unreleased reference-actor configuration as schema v2 before
  ...
  retry and terminal evidence for `TakerSellsLez`; the surface remains
  PoC-in-progress until the reverse direction passes.
- [ ] Run independent SDK reference maker and taker processes with direction-correct
  ...
  for both supported ZEC directions. Run 14o proves these boundaries for
  `TakerSellsLez`; `TakerSellsForeign` remains.
