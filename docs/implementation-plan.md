...
| Crash-safe ZEC first-lock intent and projection | Initial active chain capabilities were inert; a naive RPC-success transition would lose unknown outcomes and combined LEZ initialize/fund into one unsafe effect | Six RED–GREEN cases stage exact role/direction-bound bytes before a node call, conflict on changed replay, wait on unstable observation, observe before exact rebroadcast, order LEZ initialize/fund, keep core unchanged on invalid evidence/failed commit, probe unknown success exactly, replay after restart, and reject corrupt primitive intent/transition records without deserializing trusted domain types; full package has 79 passing tests | Implement the same contract in production SQLite, add maker-independent taker-lock observation before second lock, then extend it to maker lock, claims, and refunds |
| SQLite SDK-recovery schema | Schema v4 had no role-local accepted agreement, open/closed effect intent, separate active revision, or exact transition slot for the concrete SDK contract | Schema-v5 migration creates strict composite-role agreement, intent, and transition tables; versioned primitive JSON DTOs now reject future/unknown/substituted/corrupt fields and revalidate against the accepted agreement, revision, and exact retained closed intent; original acceptance and active revisions are separate | Implement the cloneable production `RecoveryStore` adapter with atomic close/transition/revision commits, role isolation, rollback injection, close/reopen replay, and corruption tests; tables plus DTOs alone are not production recovery evidence |
...
- [x] Add the operator/security alert outbox (introduced in schema v4 and
  retained through schema v5). Conflict warnings and
...
- [x] Prove the actual-node persistence boundary: canonical funding, immutable
  ...
  maker runtime on schema v5. Daemon-integrated polling remains open.
...
- [ ] Complete typed LEZ and Zcash actions plus atomic active-swap transition
  ...
  in-memory contract now stages exact bytes before effects, observes before
  rebroadcast, separates LEZ initialize/fund, and atomically projects/replays
  confirmed taker evidence through the store contract. Production SQLite,
