...
- [ ] Complete typed LEZ and Zcash actions plus atomic active-swap transition
  ...
  bytes, discovery windows, and typed absence/ambiguity semantics. The next
  GREEN slice is the separately locked official-wire LEZ sidecar process behind
  that capability- and `RUN_ID`-bound bridge plus one isolated single-owner
  runner.
