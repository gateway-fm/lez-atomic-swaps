...
- [ ] Complete ADR 0026's durable LEZ v0.2 effect boundary. The first Vault
  ...
  substitution. The full package gate has 42 integration tests. Still pending:
  generated-client RPC composition, unique exact sequencer inclusion in the
  fixed bounded window, matching indexer finalized block ID/hash plus
  `getAccountAtBlock` state, and actual-node maker/taker evidence. Next is the
  RED bounded inclusion/finality query matrix; no finality or actor readiness is
  claimed GREEN.
