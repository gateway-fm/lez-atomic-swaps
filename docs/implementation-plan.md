...
|---|---|---|---|
| Reference-actor composition prerequisites | The first process-composition audit found no production fresh-client factory, no actor-owned request-context allocator, non-cloneable SDK ports, and a reusable local node with an all-zero channel that signed SDK agreements reject | A bounded regular non-symlink mode-0600 capability factory creates one fresh client per attempt without retaining secret bytes. An actor-owned source now generates 128-bit OS-random one-use request IDs and invokes the injected authority only for the exact four window-bearing operations. Cloneable SDK ports share one role-local crash journal. Five factory, four allocator, and one clone regression plus the full 56-test adapter suite and strict Clippy pass. A focused locked-graph RED/GREEN changes node/config/readiness to one nonempty deterministic channel and all three readiness tests pass. The last exact full runner predates that channel correction. | Re-run the exact full standalone verifier, then implement the store-derived canonical LEZ funding source, role-keyed Zebra signers/funding planner, and one-shot maker/taker actor process |
