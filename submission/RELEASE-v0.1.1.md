# v0.1.1 — RFP-003 M1 + M3 + M6 source release

This is Gateway's corrected public source and reviewer release for the LEZ ⇄
Bitcoin vertical slice. It binds the directly browsable, buildable Rust
workspace, the M1 protocol foundation, the M3 BTC/LEZ implementation and
evidence, and the M6 Maker/Taker product surface to one versioned repository
state.

Release: <https://github.com/gateway-fm/lez-atomic-swaps/releases/tag/v0.1.1>

The default `main` branch and this tag contain `Cargo.toml`, `Cargo.lock`,
`crates/`, `compat/`, `apps/`, tests, and build scripts directly. No patch
series, reconstruction script, personal-remote checkout, or generated binary
is needed to inspect the source.
